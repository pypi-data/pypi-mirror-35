...
class Backbone(object):
    def __init__(self, api_mapping, active_config, **kwargs):
        server_address = active_config.general.ws_server
        jwt_str = active_config.general.jwt
    ...
    async def do_task(self, msg, admin=False):
        ...
        if isinstance(data, dict):
            logger.debug('%s(%s)', command, data)
            return await command(send_command, self.active_config, **data)
        else:
            if data is not None:
                logger.warning('No valid data in %s', json.dumps(data))

            logger.debug('%s(%s)', command, None)
            return await command(send_command, self.active_config, **{})
    ...
    async def serve(self):
        ...
        self.disconnect_future.set_result(True)
        logger.info('Single mode %s', self.single_job)
