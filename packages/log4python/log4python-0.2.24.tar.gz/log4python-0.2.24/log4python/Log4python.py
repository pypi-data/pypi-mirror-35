...
class log:
        ...
        if self.config_logger.has_key("AppenderRef"):
            ...
            for appender_name in appenders:
                if self.config_all.has_key("appenders") and self.config_all['appenders'].has_key(appender_name):
                    config_appender = dict(self.config_appender_default, **self.config_all['appenders'][appender_name])
                    self.debugLog("NewConfig:[%s]" % json.dumps(config_appender, encoding=False))
    ...
    def update_logger_config(self):
        ...
        self.debugLog("self.config_appender_list:[%s]" % json.dumps(self.config_appender_list, encoding=False))
        # set log's Level
