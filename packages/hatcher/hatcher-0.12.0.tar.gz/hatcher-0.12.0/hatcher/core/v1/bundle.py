...
class BundleResourceController(object):
    ...
    def canonical_filename(self):
        ...
        Note that this is **NOT** a path and does **NOT** indicate that
        the bundle is associated with a real file on the filesystem.

        Returns
        -------
        str
            The expected filename for the bundle
    ...
    def save(self, directory, overwrite=False):
        ...
        This calls :ref:`metadata`, which is a remote operation.

        The bundle filename is automatically calculated based on the
        bundle name and version (see :ref:`canonical_filename` for
        details).
