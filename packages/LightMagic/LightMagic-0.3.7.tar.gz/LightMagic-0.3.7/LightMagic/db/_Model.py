...
class _Model(_Tools, _SqlGenerator):
    ...
    def _update(self, fields=None) -> (str, list):
        """ Обновляем объект.
        :param fields: Список полей, которые будут обновлены. Если пустой, обновляем все поля.
        """
        if fields is not None:
            assert isinstance(fields, collections.Iterable), 'fields param must be instance of `Iterable`'

        keys = []

        # Ключ для кеширования запроса.
        query_key = 'update_keys' if not fields else ';'.join(sorted(fields))

        if self._check_cache(query_key) is None:
            if not fields:
                # Исключаем первичные ключи из обновления
                filter_func = lambda x: x not in self._get_primary_keys()
            else:
                # Оставляем только поля, которые передали явно.
                filter_func = lambda x: (x not in self._get_primary_keys() and x in fields)

            updated_fields = list(filter(filter_func, self.get_model_fields()))
