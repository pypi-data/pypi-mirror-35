...
class magic(object):
    ...
    def __init__(self, modulesOrClasses, generator=None, verbose=False):
        ...
        print('Auger: files %s' % (self._file_names))
    ...
    def __exit__(self, exception_type, value, tb):
        ...
        if self.verbose:
            print('=' * 100)
            print('Auger: Saving tests')
            print('Auger: Files tracked:')
            for n, filename in enumerate(self._file_names):
                print('Auger:   %d: %s' % (n, filename))
            print('Auger: Function calls tracked:')
            for n, call in enumerate(self._calls):
                print('Auger:   %d: %s' % (n, call))
        for filename, functions in self.group_by_file(self._file_names, self._calls).items():
            if self.verbose:
                print('Auger: test for %s' % filename)
    ...
    def _trace(self, frame, event, args):
        ...
        if top.startswith('<ipython-input-') or top in self._file_names:
