...
class MetastoreCreateTableRequest(RpcRequest):
	...
	def get_PartitionLists(self):
		return self.get_query_params().get('PartitionLists')

	def set_PartitionLists(self,PartitionLists):
		for i in range(len(PartitionLists)):	
			if PartitionLists[i].get('Name') is not None:
				self.add_query_param('PartitionList.' + str(i + 1) + '.Name' , PartitionLists[i].get('Name'))
			if PartitionLists[i].get('Comment') is not None:
				self.add_query_param('PartitionList.' + str(i + 1) + '.Comment' , PartitionLists[i].get('Comment'))
			if PartitionLists[i].get('Type') is not None:
				self.add_query_param('PartitionList.' + str(i + 1) + '.Type' , PartitionLists[i].get('Type'))
