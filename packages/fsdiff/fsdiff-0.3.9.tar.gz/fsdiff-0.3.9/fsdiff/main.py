...
def is_same(path1, path2):
    ...
    """
    compared = dircmp(path1, path2, ignore=['common_files', 'common_dirs', 'common_funny', 'same_files'])
...
def is_same_display(from_loc, to_loc):
    ...
    colorp("LOCATIONS {} and {} are SIMILAR".format(from_loc, to_loc) if ret == True else "LOCATIONS DIFFER",
           "green" if ret == True else "red",
           "white")
    return ret
...
def main(from_loc, to_loc, image=False, extract=False, ):
    ...
    if image:
        if extract:
            ...
            # There is a race condition that affects the loop files
            # as don't exist immediatly after created from kpartx
