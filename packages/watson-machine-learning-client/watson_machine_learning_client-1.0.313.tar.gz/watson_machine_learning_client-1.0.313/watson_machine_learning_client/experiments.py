...
class Experiments(WMLResource):
    ...
    def _store(self, meta_props):
        ...
        payload = self.ConfigurationMetaNames._generate_resource_metadata(meta_props)
    ...
    def _update_experiment(self, experiment_uid, changes):
        ...
        self.ConfigurationMetaNames._check_types_only(changes)

        details = self.get_details(experiment_uid)
        ...
        patch_payload = []
        _update_patch_payload(patch_payload, changes, [u'settings', u'name'], self.ConfigurationMetaNames.NAME)
        _update_patch_payload(patch_payload, changes, [u'settings', u'description'],
                              self.ConfigurationMetaNames.DESCRIPTION)
        _update_patch_payload(patch_payload, changes, [u'settings', u'author', u'name'],
                              self.ConfigurationMetaNames.AUTHOR_NAME)
        _update_patch_payload(patch_payload, changes, [u'settings', u'evaluation_definition', u'method'],
                              self.ConfigurationMetaNames.EVALUATION_METHOD)
        _update_patch_payload(patch_payload, changes, [u'training_data_reference'],
                              self.ConfigurationMetaNames.TRAINING_DATA_REFERENCE)
        _update_patch_payload(patch_payload, changes, [u'training_results_reference'],
                              self.ConfigurationMetaNames.TRAINING_RESULTS_REFERENCE)

        if self.ConfigurationMetaNames.EVALUATION_METRICS in changes:
            prepared_metrics = [{u'name': x} for x in changes[self.ConfigurationMetaNames.EVALUATION_METRICS]]
            _update_patch_payload(patch_payload, changes, [u'settings', u'evaluation_definition', u'metrics'],
                                  self.ConfigurationMetaNames.EVALUATION_METRICS, prepared_metrics)

        if self.ConfigurationMetaNames.TRAINING_REFERENCES in changes:
            prepared_references = copy.deepcopy(changes[self.ConfigurationMetaNames.TRAINING_REFERENCES])
            for ref in prepared_references:
                if u'name' not in ref or u'command' not in ref:

                    training_definition_response = requests.get(
                        ref[u'training_definition_url'].replace(u'/content', u''),
                        headers=self._client._get_headers())
                    result = self._handle_response(200, u'getting training definition', training_definition_response)
                    ...
                    if not u'command' in ref:
                        ref.update({u'command': result[u'entity'][u'command']})
            _update_patch_payload(patch_payload, changes, [u'training_references'],
                                  self.ConfigurationMetaNames.TRAINING_REFERENCES, prepared_references)
