...
class GSOEngine(Engine):
    ...
    def initialize(self, *args, **kwargs):
        plt.ion()
        super(GSOEngine, self).initialize(*args, **kwargs)
    ...
    def analysis(self, *args, **kwargs):
        ...
        plt.ioff()
        plt.figure(1)
        plt.plot(self.gbest.indv[0], self.gbest.indv[1], 'ro')
        plt.figure(2)
    ...
    def record(self, *args, **kwargs):
        ...
        plt.clf()
        ax = plt.subplot(111)
        [plt.plot(each.indv[0], each.indv[1], 'g*') for each in self.agents]
        plt.plot(self.gbest.indv[0], self.gbest.indv[1], 'ro')
        [ax.add_patch(plt.Circle(xy=each.indv, radius=self.r, alpha=0.5, edgecolor='r', fill=False)) for each in
         self.agents]
        plt.pause(0.2)
