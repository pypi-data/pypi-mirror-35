...
class Glowworm(Agent):
    def __init__(self, indv=None, dim=2, bound=None, fitness_function=None, *, fitness_value=-np.inf,
                 init_method='random', rho=0.1, s=0.5, gamma=0.1, beta=0.1, rs=1, nt=8, l0=None, **kwargs):
        super().__init__(indv, dim, bound, fitness_function, fitness_value=fitness_value, init_method=init_method,
                         **kwargs)
    ...
    def update(self, glowworms: list, self_ind, *args, **kwargs):
        ...
        if neighbors:
            ...
            self.indv = self.indv + self.s * (neighbors[ind][1].indv - self.indv) / (euclidean(neighbors[ind][1].indv, self.indv) + 1e-6)

            ind_low = self.indv < self.bound[:, 0]
            self.indv[ind_low] = self.bound[ind_low, 0]
            ind_high = self.indv > self.bound[:, 1]
            self.indv[ind_high] = self.bound[ind_high, 1]
    ...
    def get_neighbors(self, glowworms: list, self_ind):
        ...
        for ind, each_glowworm in enumerate(glowworms):
            if self.l <= each_glowworm.l and euclidean(self.indv, each_glowworm.indv) <= self.r and ind != self_ind:
