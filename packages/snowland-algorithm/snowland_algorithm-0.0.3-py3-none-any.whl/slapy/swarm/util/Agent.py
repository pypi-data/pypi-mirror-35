...
class Agent(object):
    ...
    def __init__(self, indv=None, dim=2, bound=None, fitness_function=None, *,
        ...
        if indv:
            ...
            if init_method == 'rand' or init_method == 'random':
                self._indv = npr.random(dim) * (bound[:,1]-bound[:, 0]).flatten() + bound[:,0].flatten()
            else:
                self._indv = npr.randn(dim) * (bound[:,1]-bound[:, 0]).flatten() + bound[:,0].flatten()
