...
class SAEngine(Agent):
    def __init__(self, steps=10000, eps=0.01, chromosome=None, dim=2, bound=None, fitness_function=None, *,
                 fitness_value=-np.inf, init_method='random', t0=10, r=0.05,k=100,d=0.95, **kwargs):
        super(SAEngine, self).__init__(steps, eps, chromosome, dim, bound, fitness_function, fitness_value=fitness_value,
    ...
    def update(self, *args, **kwargs):
        ...
        new_agent_chromosome = self.chromosome
        ind = npr.randint(self.dim)
        new_agent_chromosome[ind] += np.random.rand() * self.r - self.r/2
