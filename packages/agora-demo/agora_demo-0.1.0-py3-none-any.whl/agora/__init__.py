...
def get_dataset(dataset_name):
    try:
        ds = client.getter(ds_prepend + dataset_name)
    except:
        raise DatasetNotFoundError(dataset_name)
    return json.loads(ds)

def get_datasets():
    ct = pd.read_json(json.loads(client.getter(category_table)))
...
def post_dataset(category, dataset_name, filepath):
    ...
    client.setter(category_table, ct)
    client.setter(ds_prepend + dataset_name, str(dataset.to_json()))
    client.setter(sch_prepend + dataset_name, str(pd.DataFrame(dataset.dtypes).to_json()))

def get_schema(dataset_name):
    try:
        sch = client.getter(ds_prepend + dataset_name)
    except:
        raise DatasetNotFoundError(dataset_name)
