...
def compile_source_file(file_path='Database.sol'):
...
def get_contract_address():
    try:
        sample_data = pd.read_sql_query("select * from {}".format(table), db.engine)
    except: 
        print('No contract address exists')
    return sample_data.to_json()

def post_contract_address(contact_address, json):
    sample_data = pd.DataFrame()
    sample_data['contact_address'] = [contact_address]
    sample_data['json'] = [json]
    sample_data.to_sql(name='contactaddress', con=db.engine, if_exists='replace', index=False)
