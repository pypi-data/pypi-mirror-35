...
DispersalCoordinator::DispersalCoordinator() : dispersal_prob_map(), raw_dispersal_prob_map(), NR(nullptr),
											   ...
											   reproduction_map(make_shared<ReproductionMap>()), generation(nullptr),
...
void DispersalCoordinator::setMaps(shared_ptr<Landscape> landscape_ptr, shared_ptr<ReproductionMap> repr_map_ptr)
{
	landscape = landscape_ptr;
	reproduction_map = repr_map_ptr;
	if(!landscape_ptr)
	{
...
void DispersalCoordinator::importDispersal(const unsigned long &dispersal_dim, const string &dispersal_file)
{
	...
	*generation = 0.0;
	addDensity();
//	addReproduction(); // TODO add this back in at a later point
	fixDispersal();
...
void DispersalCoordinator::updateDispersalMap()
{
	if(dispersal_prob_map.getRows() > 0)
	{
		...
		addDensity();
//		addReproduction(); // TODO add this in at a later point
	}
}
...
void DispersalCoordinator::disperseNullDispersalMap(Step &this_step)
{
	...
	this_step.oldy = floor(NR->d01() * (ydim - 1));
}
