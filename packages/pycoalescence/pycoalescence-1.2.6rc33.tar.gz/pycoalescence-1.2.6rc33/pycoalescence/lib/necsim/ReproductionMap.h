...
public:
	ReproductionMap() : reproduction_map(), offset_x(0), offset_y(0), x_dim(0), y_dim(0),
						reproductionMapChecker_fptr(nullptr)
	{
		map_file = "none";
		max_val = 0;
		null_map = true;
	 ...
	 * @param rm the ReproductionMap object to copy from
	 * @return the self ReproductionMap object
	 */
	ReproductionMap &operator=(const ReproductionMap &rm)
	{
		this->reproduction_map = rm.reproduction_map;
		...
		this->x_dim = rm.x_dim;
		this->y_dim = rm.y_dim;
		this->reproductionMapChecker_fptr = rm.reproductionMapChecker_fptr;
		return *this;
	}
