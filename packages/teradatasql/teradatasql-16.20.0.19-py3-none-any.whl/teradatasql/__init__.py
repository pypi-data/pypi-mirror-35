    ...
    def __init__ (self, sConnectParams, **kwargs):
        ...
        try:
            ...
            if not bInitDone:
                ...
                prototype(None, goside.goInitDriver              , ctypes.c_char_p, ctypes.c_char_p, ctypes.POINTER(ctypes.POINTER(ctypes.c_char)))
                ...
                goside.goInitDriver (vernumber.sVersionNumber.encode ('utf-8'), sConnectParams.encode ('utf-8'), ctypes.byref (pcError))
                if pcError:
        ...
        if seqOfParams:
            ...
            for i in range (0, len (seqOfParams)):
                ...
                for j in range (0, len (seqOfParams [i])):
                    ...
                    elif isinstance (seqOfParams [i][j], datetime.datetime):
                        abyBindValues += _serializeString (seqOfParams [i][j].isoformat (' '))
                    elif isinstance (seqOfParams [i][j], datetime.timedelta):
                        abyBindValues += _serializeString (_formatTimedelta (seqOfParams [i][j]))
                    else:
