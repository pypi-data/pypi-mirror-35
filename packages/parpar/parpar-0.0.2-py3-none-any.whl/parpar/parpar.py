...
class ParPar:
    ...
    def shard(self,
        ...
        with open(input_file, 'r') as f:
            for line in f:
                ...
                files_made.add(dest)
                if not os.path.isdir(os.path.join(output_dir, dest)):
                    os.makedirs(os.path.join(output_dir, dest))
                with open(os.path.join(output_dir, dest, basename), 'a') as o:
                    o.write(linemend(fields, delim, newline))
                sts.tick(suffix=f'\t{len(files_made)} files made')
    ...
    def shard_apply(self,
        ...
        with Pool(processes=processess) as pool:
            ...
            pool.starmap(self._shard_apply, sargs)
    ...
    def _shard_apply(self,
        ...
        with open(file, 'r') as f:
            for line in f:
                ...
                try:
                    self._shared_current.value += 1
                    status.update(current=self._shared_current.value, suffix=f'\tprocess: {cp}')
                finally:
