...
class EmailParser(object):
    ...
    def parse(self, exclude_attachment_extensions=None):
        result = {
            "result": "failure", "attachments_sha1": "", "attachments_md5": "",
            "attach_info": "", "headers": "",
        ...
        if exclude_attachment_extensions:
            exclude_attachment_extensions = tuple(map(str.lower, exclude_attachment_extensions))
            attachments_data = filter(
                lambda attachment: not bool(attachment["filename"].lower().endswith(exclude_attachment_extensions)),
                attachments_data
            )
