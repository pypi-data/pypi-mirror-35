...
class UtilityImage:
    ...
    def draw_rectangle(image,
        ...
        for box in batch_rectangle:
            x_1 = int(round(max(box[0], 0)))
            y_1 = int(round(max(box[1], 0)))
            x_2 = int(round(min(box[2], width - 1)))
            y_2 = int(round(min(box[3], height - 1)))
    ...
    def save_image(image,
        ...
        save image
        """
        image_folder_1 = int(index / stride_folder)
        image_folder_2 = int(image_folder_1 / stride_folder)
        image_name = Path('%d/%d/%d.jpg' % (image_folder_1, image_folder_2, index))
        ...
        label_folder_1 = int(index / stride_folder)
        filename = Path('%d/labels-%d' % (label_folder_1,
                                          int(label_folder_1 / stride_label)))

        label_path = save_dir / 'label' / filename

        if label_path.parent.exists() is False:
            with (save_dir / 'label_file_list.txt').open('a') as file:
