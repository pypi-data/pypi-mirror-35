...
class UtilityImage:
    ...
    def cut_out_rectangles(image,
        ...
        c_x_1 = numpy.zeros((batch_rectangle.shape[0],), numpy.int32)
        c_y_1 = numpy.zeros((batch_rectangle.shape[0],), numpy.int32)
        ...
        for i in range(rectangle_number):
            ...
            images_out[i, :, :, :] = cv2.resize(container,
                                                (size_out, size_out))
    ...
    def save_image(image,
        ...
        save image
        """
        image_name = Path('%d/%d.jpg' % (int(index / stride_folder), index))
        ...
        filename = Path('%d.txt' % int(index / stride_label))
