...
class ManagementSystemHandler(tornado.web.RequestHandler):
	...
	def verify(self):
		...
		if self.request.body:
			...
			logging.debug("\n [+] -- Missed request body and arguments")
			data = {}
		...
		try:
			...
			logging.debug(" ** Exception with signature verification. ")
			flag = None

		if not flag:
			try:
				logging.debug("\n * Try block with replaced.")
				flag = Qtum.verify_message(message.replace(" ", ""), signature, public_key)
				pass
			except:
				...
				#raise tornado.web.Finish
				logging.debug("\n Exception with signature verification")
				self.set_status(403)
				self.write({"error":403, "reason": "Bad signature"})
				raise tornado.web.Finish
