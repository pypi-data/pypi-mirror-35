...
def get_services(name):
    ...
    strip = {"battery" : "battery", "temper_invalid_code" : "larm_burglar"}
    ...
    multi = {"battery" : "battery", "temper_removed_cover" : "alarm_burglar", "C" : "sensor_temp", "F" : "sensor_temp", "Lux" : "sensor_lumin", "%" : "sensor_humid", "index" : "sensor_uv"}
    ...
    lock = {"battery" : "battery", "locked": "door_lock", "rf_not_locked" : "alarm_lock"}

    climax = {"battery" : "battery", "temper_removed_cover" : "alarm_burglar", "ac_on" : "alarm_power", "ac_off" : "alarm_power", "overheat" : "alarm_heat", "CO" : "alarm_gas",
             "smoke" : "alarm_fire", "smoke_test" : "alarm_fire", "C" : "sensor_temp", "%" : "sensor_humid",
             "on" : "siren_ctrl", "off" : "siren_ctrl", "heat" : "siren_ctrl", "smoke" : "siren_ctrl", "CO" : "siren_ctrl"}
...
def battery_map(state):
    if (state == "full"):
        ...
        return {"event" : "low_battery", "status" : "activ"}
    elif (state == "dead"):
        return {"event" : "low_battery", "status" : "inactiv"}

# {"battery" : "battery", "locked": "door_lock", "rf_not_locked" : "alarm_lock"}
def dump_inclusion_report(req, address=1):
    Log("dump_inclusion_report : address" + address)
    Log("dump_inclusion_report : address" + req["val"]["address"])
    Log("dump_inclusion_report : comm_tech" + req["val"]["comm_tech"])
...
def dump_trigger_report(device, address, event, status, service, s_value=""):
    ...
    if (value.find("lock") > -1):
        return \
            ...
            "val": {"door_is_closed" : True },
            "val_t": "bool_map",
            "ver": 1,
            "uid": str(uuid.uuid4())
        }
    elif (value.find("battery") > -1):
        s_value = battery_map(s_value)
