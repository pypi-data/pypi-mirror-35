...
# Convert a base 64 string into a pem formatted string
def write_pem(der_bytes, pem_type):
    header = "-----BEGIN %s-----\r\n" % pem_type
...
# Read the certificate from the passed in JKS and write it into a temp file
def create_cert(path, password, cert_path=expanduser('~') + '/.pyhivecert.pem'):
    if not path:
        return cert_path

    ks = jks.KeyStore.load(path, password)
    cert = open(cert_path, mode='w')
    cert.write(write_pem(ks.private_keys.get('presto').cert_chain[0][1], 'CERTIFICATE'))
    cert.close()
    return cert_path
...
class Connection(object):
    ...
    def __init__(self, *args, **kwargs):
        if 'keystore_path' in kwargs:
            keystore_path = kwargs.pop('keystore_path')
        else:
            keystore_path = None

        if 'keystore_password' in kwargs:
            keystore_password = kwargs.pop('keystore_password')
        else:
            keystore_password = None

        if 'cert_path' in kwargs:
            cert_path = kwargs.pop('cert_path')
            self._cert = create_cert(keystore_path, keystore_password, cert_path)
        else:
            self._cert = create_cert(keystore_path, keystore_password)

        if self._cert:
            self._session = requests.Session()
            self._session.verify = self._cert
            kwargs.update([('requests_session', self._session)])
...
class Cursor(common.DBAPICursor):
    ...
    def description(self):
        ...
        return [(col['name'], col['type'], None, None, None, None, True) for col in self._columns]
