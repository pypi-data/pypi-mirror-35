"""
Data helper classes for constructing Globus Auth virtual clients.
"""
...
class ClientData(object):
    ...
    **Examples**
    ...
    """
    def __init__(self, name, public, visibility, grant_types,
                 redirect_uris, **kwargs):
        logger.info('Creating a new ClientData object')

        self.client = {}

        name = safe_stringify(name)
        self.client['name'] = name
        logger.info(
            'client.name = {}'.format(name))

        self.client['public_client'] = public
        logger.info(
            'client.public_client = {}'.format(public))

        visibility = safe_stringify(visibility)
        self.client['visibility'] = visibility
        logger.info(
            'client.visibility = {}'.format(visibility))

        self.client['grant_types'] = grant_types
        logger.info(
            'client.grant_types = {}'.format(grant_types))

        self.client['redirect_uris'] = redirect_uris
        logger.info(
            'client.redirect_uris = {}'.format(redirect_uris))

        self.client.update(kwargs)
        for option, value in kwargs.items():
            logger.info("client.{} = {} (option passed in via kwargs)"
                        .format(option, value))

        self._scopes = []

    def add_scope(self, name, description, scope_suffix):
