...
class Pin(Content):
    ...
    def init(self, id=''):
        self.load(zhihu.json(f'https://api.zhihu.com/pins/{id}'))
    ...
    def load(self, JSON):
        ...
        datas = {
            ...
            'voteup_count': JSON.get('top_reactions', {}).get('like'),
            'content': JSON.get('content', [''])[0],
            'img_urls': []if len(JSON.get('content', [''])) < 2 else list(map(lambda x: x['url'], JSON['content'][1:])),
            'img_infos': []if len(JSON.get('content', [''])) < 2 else list(map(lambda x: x, JSON['content'][1:])),
