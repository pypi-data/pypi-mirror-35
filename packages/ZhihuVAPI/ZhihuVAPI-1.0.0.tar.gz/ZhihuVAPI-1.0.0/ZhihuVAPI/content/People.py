...
class People(Container):
    ...
    def is_waterman(self):
        '''没有收藏夹(正常人至少会有一个),没有联通免流,没有想法,没有回答(一个也没有?),那就是三无小号了'''
        if self.favorite_count == 0 and self.is_unicom_free == False and self.pins_count == 0 and self.answer_count == 0:
            return True
        else:
            return False
