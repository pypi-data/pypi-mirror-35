...
def cc_live_callback(request):
    ...
    cc_live.save()
    # 录制成功
    if cc_live.record_status == "10" and cc_live.call_count == 7:
        user = cc_live.user
        ...
        if timedelta >= 60 and user:
            # 更新课件学习时间
            user_course_section = UserCourseSection.objects.filter(user_course__user=user, status=1).order_by('-updated_at')
