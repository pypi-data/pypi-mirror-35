...
def default_captcha_handler(image_url):
    ...
    data = requests.get(image_url, proxies={'https' : 'https://127.0.0.1:1080'}).content
...
class PCSBase(object):
    ...
    def get_fastest_pcs_server_test():
        ...
        """
        ret = requests.get(
                'https://pcs.baidu.com/rest/2.0/pcs/manage?method=listhost', proxies={'https' : 'https://127.0.0.1:1080'}).content
        serverlist = [server['host'] for server in json.loads(ret)['list']]
        url_pattern = 'http://{0}/monitor.jpg'
        time_record = []
        for server in serverlist:
            start = time.time() * 1000
            requests.get(url_pattern.format(server), proxies={'https' : 'https://127.0.0.1:1080'})
            end = time.time() * 1000
    ...
    def get_fastest_pcs_server():
        ...
        ret = requests.get(url, proxies={'https' : 'https://127.0.0.1:1080'}).content
    ...
    def _initiate(self):
        if not self._load_cookies():
            self.session.get('http://www.baidu.com', proxies={'https' : 'https://127.0.0.1:1080'})
    ...
    def _get_token(self):
        ...
        ret = self.session.get(
                'https://passport.baidu.com/v2/api/?getapi&tpl=mn&apiver=v3&class=login&tt=%s&logintype=dialogLogin&callback=0' % int(
                        time.time()), proxies={'https' : 'https://127.0.0.1:1080'}).text.replace('\'', '\"')
        foo = json.loads(ret)
    ...
    def _get_publickey(self):
        url = 'https://passport.baidu.com/v2/getpublickey?token=' + \
              self.user['token']
        content = self.session.get(url, proxies={'https' : 'https://127.0.0.1:1080'}).content
    ...
    def _login(self):
        ...
        while True:
            ...
            result = self.session.post(
                    'https://passport.baidu.com/v2/api/?login', data=login_data, proxies={'https' : 'https://127.0.0.1:1080'})
            ...
            if resp.ok:
                while 1:
                    ...
                    if vresp_data['errno'] == 110000:
                        ...
                        loginproxy_resp = self.session.get(urlparse.unquote(loginproxy_url.decode()), proxies={'https' : 'https://127.0.0.1:1080'})
    ...
    def _request(self, uri, method=None, url=None, extra_params=None,
        ...
        if data or files:
            ...
            if uri == 'filemanager' or uri == 'rapidupload' or uri == 'filemetas' or uri == 'precreate':
                response = self.session.post(
                        api, params=params, verify=False, headers=headers, proxies={'https' : 'https://127.0.0.1:1080'}, **kwargs)
            else:
...
class PCS(PCSBase):
    ...
    def __err_handler(self, act, errno, callback=None, args=(), kwargs=None):
        ...
        def err_handler_download():
            if errno == 112:
                ...
                self.session.get(url, proxies={'https' : 'https://127.0.0.1:1080'})
    ...
    def get_sign(self):
        ...
        url = 'http://pan.baidu.com/disk/home'
        resp = self.session.get(url, proxies={'https' : 'https://127.0.0.1:1080'})
        html = resp.content
