...
class Error(ValueError):
    "Exception raised by HTML parser on unhandled cases."
...
class Parser:
    ...
    def make_heading(self, level, element):
        ...
        if partial:
            logger.warning('Unhandled elements left: %r',
                           [{'tag': e.tag, 'line': e.sourceline, 'text': e.text}
                            for e in children])
            raise Error('Incomplete parse of <Hx>')
    ...
    def handle_h1(self, element):
        return self.make_heading(1, element)

    def handle_h2(self, element):
        return self.make_heading(2, element)

    def handle_h3(self, element):
        return self.make_heading(3, element)

    def handle_h4(self, element):
        return self.make_heading(4, element)

    def handle_h5(self, element):
        return self.make_heading(5, element)

    def handle_h6(self, element):
        return self.make_heading(6, element)
    ...
    def make_paragraph(self, element):
        ...
        for elt in children:
            if elt.tag == 'br':
                ...
                if elt.tag not in ('a', 'b', 'div', 'em', 'i', 'span', 'strong'):
                    partial = True
                    break
    ...
    def handle_div(self, element):
        if not self.in_div and element.text and element.text.strip():
            yield from self.handle_p(element)
        else:
            self.in_div += 1
            children = element.getchildren()
            if children:
                if children[0].tag in ('br', 'strong', 'em', 'span', 'a'):
                    yield from self.handle_p(element)
                else:
                    for elt in children:
    ...
    def handle_p(self, element):
        ...
        if remaining is not None:
            logger.warning('Unhandled elements left: %r',
                           [{'tag': e.tag, 'line': e.sourceline, 'text': e.text}
                            for e in remaining])
            raise Error('Incomplete parse of paragraph')
        return elts
...
def parse_html(html, fallback_to_plain_text=True):
    ...
    try:
        ...
        if fallback_to_plain_text:
            logger.warning('Could not parse HTML, %s: %r', e, html)
