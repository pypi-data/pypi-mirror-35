...
class ptArtifact:
    def __init__(self, pt_server_api_url, uuid1=None):
        self.uuid = uuid1 if uuid1 else uuid.uuid1()
        self.url = pt_server_api_url.rstrip("/") + "/0/artifact/"
    ...
    def link(self, obj_uuid1):
        # fixme
        pass

    def unlink(self, obj_uuid1):
        # fixme
        pass

    def update(self, filename, mime, description):
        # fixme
        pass

    def upload(self, data):
        try:
            response = requests.post(self.url, data=data, headers={'Content-Type': 'application/octet-stream'})
        except requests.exceptions.ConnectionError as exc:
            logging.error("Connection error: %s" % str(exc))
            return False

        if response.status_code != httplib.OK:
            logging.error("job json uploaded to %s: status %d, %s" % (self.url, response.status_code, response.text))
...
class ptBlob:
    def __init__(self, title, content, mime_type='application/octet-stream'):
        self.uuid = uuid.uuid1()
        assert type(val) == str
        self.title = title
        self.mime_type = mime_type
        self.content = content
        self.linked_uuids = []  # uuids of linked tests/suites/hosts/etc... artefacts

    def linkToTest(self, test):
        assert isinstance(test, ptTest)
        self.linked_uuids.append(test.uuid)
...
class ptTest:
    ...
    def add_blob(self, blob):
        assert isinstance(blob, ptBlob)
        blob.linkToTest(self)
...
class ptSuite:
    ...
    def genApiUrl(self, url):
        ...
        api_url = "%s/api/v%s" % (self.pt_server_url, API_VER)
        return "%s/%s" % (api_url, url.lstrip("/")) if url else api_url

    def validateProjectName(self):
        if not self.project_name:
            return

        url = self.genApiUrl("/0/project/")
        try:
