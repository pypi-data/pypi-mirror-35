...
def datasource_link(app=None):
    ...
    print api.endpoints
    api.app = app
    #if not "datasourcelinkresource" in app.view_functions.keys():
    # After init_app() is called, this should be set by Flask (but it's not) so that further calls to add_resource() are handled immediately.
    api.add_org_resource(DataSourceLinkResource, '/api/data_sources/<data_source_id>/link', endpoint="datasourcelinkresource")
    print "this finished"
