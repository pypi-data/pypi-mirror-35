...
def datasource_link(app=None):
    ...
    print api.endpoints
    if not api.owns_endpoint("datasourcelinkresource"):
        api.app = app # After init_app() is called, this should be set by Flask (but it's not) so that further calls to add_resource() are handled immediately.
