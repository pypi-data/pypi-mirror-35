...
class ApiClient(object):
    ...
    def __init__(self, app_key, app_sid, base_path=None, configuration=None, header_name=None, header_value=None, cookie=None):
        if configuration is None:
            configuration = Configuration()
        self.configuration = configuration
        if base_path is not None:
            self.configuration.host = base_path
        self.configuration.app_sid = app_sid
    ...
    def deserialize(self, response, response_type):
        ...
        try:
            data = json.loads(response.data.decode('utf8') if PY3 else response.data)
        except ValueError:
            data = response.data
    ...
    def __microsoft_date_as_datetime(self, string):
        import datetime
        if not re.search('^/Date\(.*\)/$', string):
            return None
        sign = string[-7]
        if sign not in '-+' or len(string) == 13:
            millisecs = int(string[6:-2])
        else:
            millisecs = int(string[6:-7])
            hh = int(string[-7:-4])
            mm = int(string[-4:-2])
            if sign == '-': mm = -mm
    ...
    def __deserialize_date(self, string):
        ...
        """
        try:
            dt = self.__microsoft_date_as_datetime(string)
            if None != dt:
                return dt
