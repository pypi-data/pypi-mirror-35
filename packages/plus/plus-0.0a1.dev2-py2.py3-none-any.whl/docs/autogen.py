...
import plus
from plus import visual
from plus.visual import plotter
from plus.utils import debugger, utils
...
PAGES = [
    {
        'page': 'visual/plotter.md',
        'methods':[
            plotter.Plotter.register_plot_type,
...
def post_process_signature(signature):
    ...
    if len(parts) >= 4:
        if parts[1] == 'layers':
            signature = 'plus.layers.' + '.'.join(parts[3:])
        if parts[1] == 'utils':
            signature = 'plus.utils.' + '.'.join(parts[3:])
        if parts[1] == 'backend':
            signature = 'plus.backend.' + '.'.join(parts[3:])
    return signature


def clean_module_name(name):
    if name.startswith('plus_applications'):
        name = name.replace('plus_applications', 'plus.applications')
    if name.startswith('plus_preprocessing'):
        name = name.replace('plus_preprocessing', 'plus.preprocessing')
    assert name[:5] == 'plus.', 'Invalid module name: %s' % name
    return name
...
def class_to_source_link(cls):
    ...
    link = ('https://github.com/SHTUPLUS/'
            'plus/blob/master/' + path + '#L' + str(line))
    return '[[source]](' + link + ')'
...
if __name__ == '__main__':
    ...
    print('Generating docs for PLUS %s.' % plus.__version__)
