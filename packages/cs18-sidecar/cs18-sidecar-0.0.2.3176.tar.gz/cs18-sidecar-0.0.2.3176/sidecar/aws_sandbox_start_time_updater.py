...
class AwsSandboxStartTimeUpdater(ISandboxStartTimeUpdater):
    ...
    def _update_stack(self, stack_name: str):
        ...
        time = str(self._date_time_provider.get_current_time_utc())
        stack.update(
            UsePreviousTemplate=True,
            Tags=[AwsTagHelper.create_tag(Const.SANDBOX_START_TIME, time),
                  AwsTagHelper.create_tag(Const.SANDBOX_START_TIME_v2, time)])
