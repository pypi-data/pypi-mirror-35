...
class SosbackupsScanStartEven(SosBackupsScanEven):
...
class SosBackupsScanCompleteEven(SosBackupsScanEven):
    def __init__(self, pathname, caller, caller_id, plugins = None, sync = False):
        SosBackupsScanEven.__init__(self, pathname, caller, caller_id, plugins, sync)
        self.completed = True
...
class SosBackupsCronScan(object):
    def __init__(self, name, xpath, options = {}):
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(xpath, topdown = True, followlinks = followlinks):
            ...
            for filename in xfiles:
                ...
                if not last_cfg_path and not last_filepath:
                    start_event = SosbackupsScanStartEven(filepath, self.caller, self.caller_id, plugins)
                    self.handler.call_plugins(cfg_path, start_event, include_plugins = self.include_plugins)
                    start_event.tevent.wait()
        ...
        if last_cfg_path and last_filepath:
            complete_event = SosBackupsScanCompleteEven(last_filepath, self.caller, self.caller_id, plugins)
            self.handler.call_plugins(last_cfg_path,
                                      complete_event,
                                      include_plugins = self.include_plugins,
                                      exclude_filter  = False)
            complete_event.tevent.wait()
...
class SosBackupsCronSyncPlugin(DWhoPluginBase):
    ...
    def safe_init(self):
        ...
        self.cron_pool = workerpool.WorkerPool(max_workers = int(self.plugconf.get('max_workers') or len(self.tabs)),
