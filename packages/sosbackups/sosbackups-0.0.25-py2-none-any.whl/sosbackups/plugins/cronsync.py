...
class SosBackupsScanEven(object):
    def __init__(self, pathname, caller, caller_id, plugins = None, last_file = None, sync = True):
...
class SosBackupsCronScan(threading.Thread):
    ...
    def run(self):
        ...
        (last_cfg_path, last_event) = (None, None)
        ...
        for root, dirs, files in os.walk(xpath, topdown = True):
            ...
            for filename in files:
                ...
                event     = SosBackupsScanEven(filepath, self.caller, self.caller_id, self.plugins)
                cfg_path  = self.inotify.get_cfg_path(event.pathname)
                if cfg_path:
                    last_cfg_path = cfg_path
                    last_event    = copy.copy(event)
                    self.handler.call_plugins(cfg_path, event, include_plugins = self.include_plugins)

        if last_cfg_path and last_event:
            last_event.last_file = last_event.pathname
            last_event.sync      = False
            self.handler.call_plugins(last_cfg_path, last_event, include_plugins = self.include_plugins)
