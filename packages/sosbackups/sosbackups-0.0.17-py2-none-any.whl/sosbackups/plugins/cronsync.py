...
class SosBackupsScanEven(object):
    def __init__(self, pathname, plugins = None):
...
class SosBackupsCronScan(threading.Thread):
    ...
    def _parse_time_opt(self, value, utc = False):
        if value == 'start':
            ...
            xtime = self._parse_time_arg(value['when'], utc)

            if isinstance(xtime, datetime.datetime):
                if 'args' in value:
                    return datetime.timedelta(*args) + xtime
                elif 'kwargs' in value:
                    return datetime.timedelta(**kwargs) + xtime
    ...
    def run(self):
        ...
        for root, dirs, files in os.walk(xpath):
            ...
            if self.exclude_patterns and self.exclude_patterns(root):
                LOG.debug("exclude path from scan. (filepath: %r)", root)
                continue
            ...
            for filename in files:
                ...
                event    = SosBackupsScanEven(filepath, self.plugins)
