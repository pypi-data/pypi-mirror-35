...
class SosBackupsScanCompleteEven(SosBackupsScanEven):
    def __init__(self, pathname, caller, caller_id, plugins = None, meta = True):
        SosBackupsScanEven.__init__(self, pathname, caller, caller_id, plugins, meta)
    ...
    def run(self, xpath):
        ...
        for root, dirs, files in os.walk(xpath, topdown = True, followlinks = followlinks):
            ...
            if last_filepath and last_cfg_path:
                event         = SosBackupsScanCompleteEven(last_filepath, self.caller, self.caller_id, plugins)
