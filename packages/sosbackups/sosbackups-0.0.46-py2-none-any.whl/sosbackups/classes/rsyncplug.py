...
class SosBackupsRsyncPlug(DWhoInoEventPlugBase):
    ...
    def _set_state(self, filepath, errors = None, nb_files = None, total_size = None):
        ...
        if nb_files:
            state['stats']['files'][status]  += nb_files.group('files')
            state['stats']['dirs'][status]   += nb_files.group('dirs')
            state['stats']['total'][status]  += nb_files.group('total')
            state['stats']['total']['total'] += nb_files.group('total')

        if total_size:
            state['stats']['files']['size']  += total_size.group('size')
            state['stats']['total']['size']  += total_size.group('size')
