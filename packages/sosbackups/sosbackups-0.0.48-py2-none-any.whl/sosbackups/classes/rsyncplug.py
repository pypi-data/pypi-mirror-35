...
class SosBackupsRsyncPlug(DWhoInoEventPlugBase):
        ...
        for x in stdout.splitlines():
            if not x:
                if stats_matched:
                    stats_matched = 3
                continue
            elif not self.with_stats or stats_matched > 2:
                LOG.info(x)
                continue
    ...
    def _set_state(self, filepath, errors = None, nb_files = None, total_size = None):
        ...
        if nb_files:
            state['stats']['files'][status]  += long(nb_files.group('files'))
            state['stats']['dirs'][status]   += long(nb_files.group('dirs'))
            state['stats']['total'][status]  += long(nb_files.group('total'))
            state['stats']['total']['total'] += long(nb_files.group('total'))

        if total_size:
            state['stats']['files']['size']  += long(total_size.group('size').replace(',', ''))
            state['stats']['total']['size']  += long(total_size.group('size').replace(',', ''))
