...
class SosBackupsCronScan(object):
    ...
    def run(self, xpath):
        ...
        for plug_name, plug_opts in plugins.itervalues():
    ...
    def __call__(self):
        ...
        self.clean_events(wait = True)

        self.ended_at = time.strftime(_DATE_FORMAT)
        started       = datetime.datetime.strptime(self.started_at, _DATE_FORMAT)
    ...
    def run(self):
        while not _KILLED:
            for tab, tasks in self.tabs.iteritems():
                ...
                for params in tasks:
                    options = params.copy()
                    if isinstance(options['path'], list):
                        paths = list(options.pop('path'))
                    else:
                        paths = [options.pop('path')]
