...
class SosBackupsRsyncPlug(DWhoInoEventPlugBase):
    ...
    def get_event_params(self, event):
        if hasattr(event, 'plugins') \
           ...
           and self.PLUGIN_NAME in event.plugins:
            return event.plugins[self.PLUGIN_NAME].copy()
    ...
    def try_exec_rsync(self, params, event, filepath, notify = True):
        ...
        try:
            errors = self.exec_rsync(params, event, filepath, False)
        except Exception, e:
            errors = e
    ...
    def send_notifications(self, filepath, event, errors = None):
        ...
        if getattr(event, 'started', False):
        ...
        if not getattr(event, 'completed', False) or event.pathname != filepath:
            return
