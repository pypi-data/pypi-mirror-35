...
class SosBackupsCronScan(object):
    ...
    def run(self, xpath):
        ...
        for root, dirs, files in os.walk(xpath, topdown = True, followlinks = followlinks):
            ...
            for filename in xfiles:
                ...
                event         = SosBackupsScanEvent(filepath, self.caller, self.caller_id, plugins)
            ...
            if last_filepath and last_cfg_path:
                event         = SosBackupsScanCompleteEvent(last_filepath, self.caller, self.caller_id, plugins)
                self.handler.call_plugins(last_cfg_path,
                                          event,
                                          include_plugins = self.include_plugins,
                                          exclude_filter  = False)
                LOG.debug("starting complete scan event")
                event.tevent.wait()
                LOG.debug("stopping complete scan event")
    ...
    def __call__(self):
        ...
        LOG.debug("starting clean events")
        self.clean_events(wait = None)
        LOG.debug("stopping clean events")
