...
class SosBackupsRsyncPlug(DWhoInoEventPlugBase):
    ...
    def init(self, config):
        ...
        if self.config['general'].get('notifiers_path'):
            self._notifiers = DWhoPushNotifications(config_path = self.config['general']['notifiers_path'])
    ...
    def _get_rsync_params(self, params, event, filepath):
        ...
        if 'notifications' in params:
            self.notifications = bool(params.get('notifications', True))
    ...
    def send_notifications(self, filepath, event, errors = None):
        ...
        if not getattr(event, 'caller_id'):
        ...
        if caller_state.get('started_at') and caller_state.get('ended_at'):
            ...
            ended   = datetime.datetime.strptime(caller_state['ended_at'], _DATE_FORMAT)
            caller_state['stats']['duration'] = (ended - started).total_seconds()
