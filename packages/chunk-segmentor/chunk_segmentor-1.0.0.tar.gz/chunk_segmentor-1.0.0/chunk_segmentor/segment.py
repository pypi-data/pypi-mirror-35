...
class Chunk_Segmentor(object):
    def __init__(self, model_name='', pos=True, mode='accurate', qualifier=False):
        self.pos = pos
        self.mode = mode
        ...
        # model变量
        weight_file = os.path.join(self.path, 'data/model/%s_weights.h5' % self.model_name)
        param_file = os.path.join(self.path, 'data/model/%s_parms.h5' % self.model_name)
        preprocess_file = os.path.join(self.path, 'data/model/%s_preprocess.h5' % self.model_name)
        if self.qualifier:
        ...
        if self.mode == 'accurate':
            ...
            self.labeler = Chunk_Labeler.load(self.model_name, weight_file, param_file, preprocess_file)
    ...
    def cut(self, data, batch_size=512):
