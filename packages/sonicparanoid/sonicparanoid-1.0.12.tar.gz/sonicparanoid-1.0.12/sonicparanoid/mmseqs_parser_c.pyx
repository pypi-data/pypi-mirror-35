...
def extract_inparanoid_scores(hspDict, qid, sid, debug=False):
    ...
    cdef int fQlen, fSlen, fCol6, fCol7, fCol8, fCol9
...
# NOTE: multiple hits with xx_0, xx_1 are not handled
# this would cause inpyranoid to crash
# fix by loading the sequence headers on the fir st place
def mmseqs_parser_convertalis_format_2_NNEDS_FIX(alignFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    ...
    The conversion of the MMseqs2 output must be done using convertalis with --format-mode 2
    """
    if debug:
        print('inparanoid_like_parser_mmseqs :: START')
    ...
    '''
    qidLenDict = load_seq_lengths(queryFile, debug)
    sidLenDict = qidLenDict
    if queryFile != targetFile:
      sidLenDict = load_seq_lengths(targetFile, debug)
    '''
    ...
    while True:
        ...
        sstart += 1
        send = atoi(flds[9])
        # old way of extracting the score
        ##tmpHitScore = int(flds[-1].decode().rstrip('\n'))
        tmpHitScore = atoi(flds[-3])
        #sys.exit('DEBUG')
        ...
        if currentHitId != prevHitId: #then it is a new hit
            #print(currentHit)
            #print(prevHitId)
            #sys.exit('DEBUG :: hit id comparison')
            ...
            currentHit['%d:%d'%(qstart, qend)] = (qlen, slen, qstart, qend, sstart, send, tmpHitScore)
    ...
    outLn = ''
    #calculate InParanoid like scores
    hitScore = extract_inparanoid_scores(currentHit, qid, sid, debug=debug)
    #create the final string if the the bitscore for the hit is higher than the cutoff
    #tmpHitScore = <double>float(hitScore[2])
    #if tmpHitScore >= scoreCoff_c:
    if int(hitScore[2]) >= scoreCoff_c:
        #print(hitScore[2])
        outLn = '\t'.join(hitScore[:-1])
        outLn += '\t%s\n'%('\t'.join(hitScore[-1]))
...
def mmseqs_parser(alignFile, queryFile, targetFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    """Parse MMseqs2 results converted to BLAST-like tab-separated files
    The parsing is similar to how InParanoid extract results from XML
    The conversion of the MMseqs2 output must be done using convertalis with --format-mode 0
    """
    if debug:
        print('inparanoid_like_parser_plastp :: START')
        print('Outdir:%s'%outDir)
        print('Bit-score cutoff below which hits (the sum of the bit-scores) are discarded:%s'%str(scoreCoff))
    # load the dictionaries with sequence lengths
    qidLenDict = load_seq_lengths(queryFile, debug)
...
def mmseqs_parser_f0_9flds(alignFile, queryFile, targetFile, outDir=os.getcwd(), outName=None, scoreCoff=40, debug=False):
    ...
    while True:
        ...
        if currentHitId != prevHitId: #then it is a new hit
            if not currentHit is None: #then it is not the first line
                ...
                prevqid, prevsid = prevHitId.split('@')
                #calculate InParanoid like scores
                hitScore = extract_inparanoid_scores_format0_9flds(currentHit, prevqid, prevsid, debug=False)
                #create the final string if the the bitscore for the hit is higher than the cutoff
                if int(hitScore[2]) >= scoreCoff_c:
                    '''
                    outLn = '\t'.join(hitScore[:-1])
    ...
    outLn = ''
    #calculate InParanoid like scores
    hitScore = extract_inparanoid_scores_format0_9flds(currentHit, qid, sid, debug=False)
    #create the final string if the the bitscore for the hit is higher than the cutoff
    #tmpHitScore = <double>float(hitScore[2])
    #if tmpHitScore >= scoreCoff_c:
    if int(hitScore[2]) >= scoreCoff_c:
...
'''
def load_seq_lengths_flat_fasta(inSeq, debug=False):
    """Read a FASTA sequence and returns a dictionary with the sequence length for each id."""
    if debug:
        print('load_seq_lengths_flat_fasta :: START')
        print('Input sequence:%s'%inSeq)
    lenDict = {} # contain the length associated to each id
    seqid = None
    seqCnt = 0
    for ln in open(inSeq):
        if ln[0] == '>':
            seqCnt += 1
            seqid = ln[1:-1]
            # add the id in the dictionary
            lenDict[seqid] = 0
        else: # increment the length of the current sequence
            lenDict[seqid] += len(ln[:-1])
    if debug:
        print('Input sequences:\t%d'%seqCnt)
    return lenDict
'''
