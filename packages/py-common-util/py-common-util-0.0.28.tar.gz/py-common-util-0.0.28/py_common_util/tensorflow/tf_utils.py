...
class TFUtils:
    ...
    @staticmethod
    def check_available_gpus(show_info=True):
    ...
    @staticmethod
    def cuda_is_available(show_info=True):
    ...
    def device(device="gpu", device_index=0):
        ...
        if TFUtils.cuda_is_available(show_info=False) and device == 'gpu':
            return tf.device("/" + device + ":" + str(device_index))
        else:
            return tf.device("/cpu:" + str(device_index))
