...
class TFRecordsUtils:
    ...
    def read_and_decode(tfrecords_file_path,
        ...
        data_raw = tf.reshape(data_raw, shape_channels_height_width)
    ...
    def load_np_data(tfrecords_file_path,
        ...
        :param dense_labels_to_one_hot_classes: 0-不用处理，>0就处理labels数据为TFUtils.dense_to_one_hot(label, dense_labels_to_one_hot_classes)
        :return:
        usage:
        np_data_labels = TFRecordsUtils.np_load_data(tfrecords_path + "/valid.tfrecords",
            ...
            allow_smaller_final_batch=False,
            category=category,
            shape_channels_height_width=[1, height, width],
            should_use_labels=False,
            data_is_normalized=True,
            using_shuffle_batch=True)
        ...
        ...
        data_raw, labels = \
            TFRecordsUtils.read_and_decode(tfrecords_file_path=tfrecords_file_path,
                                                                category=category,
                                                                batch_size=batch_size,
                                                                shape_channels_height_width=shape_channels_height_width,
            ...
            try:
                for i_batch in range(total_num_batch):
                    if not coord.should_stop():
                        ...
                        data_labels.append((np.asarray(_data, dtype=np.float32),
                                            np.asarray(_labels, dtype=np.int64) if dense_labels_to_one_hot_classes<1 else TFUtils.dense_to_one_hot(np.asarray(_labels, dtype=np.int64), dense_labels_to_one_hot_classes)
                                            ))
            except:
