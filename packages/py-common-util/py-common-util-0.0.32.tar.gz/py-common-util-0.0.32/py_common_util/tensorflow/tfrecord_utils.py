                ...
                for i_batch in range(total_num_batch):
                    if not coord.should_stop():
                        ...
                        data_labels.append((np.array(_data, dtype=np.float32),
                                            np.array(_labels, dtype=np.int64) if dense_labels_to_one_hot_classes<1 else TFUtils.dense_to_one_hot(np.asarray(_labels, dtype=np.int64), dense_labels_to_one_hot_classes)
                                            ))
            except:
