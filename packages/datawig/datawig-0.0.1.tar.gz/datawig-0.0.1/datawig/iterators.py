...
class ImputerIter(mx.io.DataIter):
    ...
    def __init__(self,
        ...
        self.df_iterator: mx.io.DataIter = None
        self.indices: Any = None
        self.start_padding_idx: int = 0
    ...
    def mxnet_iterator_from_df(self, data_frame: pd.DataFrame) -> mx.io.NDArrayIter:
        ...
        for col_enc in self.data_columns:
            ...
            logger.info(f'Data Encoding - Encoded {len(data_frame)} rows of column '
                        f'{",".join(col_enc.input_columns)} with {col_enc.__class__} into '
                        f'{type(data_array_numpy)} of shape {data_array_numpy.shape} '
                        f'and then into shape {data[col_enc.output_column].shape}')

        # transform labels into mxnet nd arrays
        labels = {}
        for col_enc in self.label_columns:
            assert len(col_enc.input_columns) == 1, f'Number of encoder input columns for labels ' \
                                                    f'must be 1, was {len(col_enc.input_columns)}'
            ...
            if col_enc.input_columns[0] in data_frame.columns:
                ...
                logger.info(f'Label Encoding - Encoded {len(data_frame)} rows of column '
                            f'{col_enc.input_columns[0]} with {col_enc.__class__} into '
                            f'{type(labels_array_numpy)} of shape {labels_array_numpy.shape} and '
                            f'then into shape {labels[col_enc.output_column].shape}')
            else:
                labels[col_enc.input_columns[0]] = mx.nd.zeros((n_samples, 1))
                logger.info(f'Could not find column {col_enc.input_columns[0]} in DataFrame, '
                            f'setting {n_samples} labels to missing')
...
class ImputerIterDf(ImputerIter):
    ...
    def __init__(self,
        ...
        self.indices: List[int] = data_frame.index.tolist()
        data_frame = data_frame.reset_index(drop=True)

        # custom padding for having to discard the last batch in mxnet for sparse data
        padding_n_rows: int = self._n_rows_padding(data_frame)
        ...
        self.df_iterator: mx.io.NDArrayIter = self.mxnet_iterator_from_df(data_frame)
        self.df_iterator.reset()
