...
def _pct_change(df, timedelta, dateset):
    df_t = df.T.copy()
    ...
    for col in sorted(dateset):
        if col not in columns_set:
            continue
        next_i = np.searchsorted(columns, col + timedelta, side='left')
        if next_i >= len(columns):
            df_t[col] = np.nan
        else:
            next_col = columns[next_i]
            df_t[col] = (df_t[next_col] - df_t[col]) / df_t[col]
    return df_t.T
...
def compute_forward_returns(factor,
    ...
    for period in sorted(periods):
        ...
        #
        fwdret = (_pct_change(prices, period_timedelta, factor_date_set)
...
def _freq_to_timedelta(freq):
    ...
    if freqstr in {'B', 'C', 'D'}:
        return 'D'
    elif freqstr in {'M', 'MS', 'BM', 'CBM', 'CBMS'}:
        return 'M'
    else:
        # TODO: Add mapping for other freq
        return freqstr
...
def add_custom_calendar_timedelta(input, timedelta, freq):
    ...
    pd.DatetimeIndex or pd.Timestamp
        input + timedelta
    """
    if freq.freqstr in {None, 'B', 'C', 'D'}:
        days = timedelta.components.days
        offset = timedelta - pd.Timedelta(days=days)
        return input + freq * days + offset
    else:
        freq_unit = _freq_to_timedelta(freq)
        step = round(timedelta / pd.Timedelta(1, unit=freq_unit))
        return input + freq * step
