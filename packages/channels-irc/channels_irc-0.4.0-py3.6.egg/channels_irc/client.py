...
class ChannelsIRCClient(AioSimpleIRCClient, BaseServer):
    ...
    async def from_consumer(self, message):
        ...
        if "type" not in message:
            ...
            await handler(message, self.connection)

        else:
            raise ValueError("Cannot handle message type %s!" % message["type"])

    async def _handle_status(self, msg, connection):
        """
    ...
    async def _handle_join(self, msg, connection):
        """
        ...
        if channel:
            connection.join(self.format_channel_name(channel))

    async def _handle_message(self, msg, connection):
        """
        ...
        connection.privmsg(self.format_channel_name(channel), message)

    async def _handle_names(self, msg, connection):
        """
        ...
        if channel:
            connection.names(self.format_channel_name(channel))

    async def _handle_part(self, msg, connection):
        """
        ...
        if channel:
            connection.part(self.format_channel_name(channel))
