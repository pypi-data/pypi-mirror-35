...
from msxf_flow_engine.actions.action import ActionRestart, ACTION_LISTEN_NAME
from msxf_flow_engine.channels.channel import UserMessage, InputChannel
from msxf_flow_engine.channels.direct import CollectingOutputChannel
from msxf_flow_engine.dispatcher import Dispatcher
from msxf_flow_engine.domain import Domain
from msxf_flow_engine.events import ReminderScheduled, Event
from msxf_flow_engine.events import SlotSet
from msxf_flow_engine.events import UserUttered, ActionExecuted, BotUttered
from msxf_flow_engine.interpreter import (
    NaturalLanguageInterpreter,
    INTENT_MESSAGE_PREFIX)
from msxf_flow_engine.interpreter import RegexInterpreter
from msxf_flow_engine.policies.ensemble import PolicyEnsemble
from msxf_flow_engine.tracker_store import TrackerStore
...
TIME_OUT_EACH_REQ = 60

class ResCodeParser(object):
    """Parser for http response json."""

    def __init__(self, resp):
        super(ResCodeParser, self).__init__()
        self.resp = resp
        self.json = resp.json()
    ...
    def success(self):
        ...
        if (self.resp.status_code == 200 and
                self.json['code'] == '10000'):
            return True
        else:
            return False
...
class MessageProcessor(object):
    ...
    def handle_message(self, message):
        ...
        message.sender_id = self.processerId
        # we have a Tracker instance for each user
        # which maintains conversation state
        tracker = self._get_tracker(message.sender_id)

        self._handle_urlmessage_with_tracker(message, tracker)

        return_text = self._predict_and_execute_next_action(message, tracker)
        # save tracker state to continue conversation from this state
        self._save_tracker(tracker)
        return return_text,self.proba
    ...
    def _get_intents_json(self,message):
        exception = None
        intent_result = ''
        try:
            json_data = {
            ...
            url = message.url + '/predict/intent'
            resp = requests.post(
                    url,
                    json=json_data,
                    timeout=TIME_OUT_EACH_REQ)
            res_json = resp.json()
            code_parser = ResCodeParser(resp)
            success = code_parser.success
            msg = code_parser.err_msg
            data = res_json['data']
            if not data:
                raise Exception('{}:no intentin'.format(message.text))
            intent_result = str(data['result'])
            logger.info(
                'intents success:: get intents res: {}'
                .format(res_json))
        ...
        if not success:
            logger.error(
                'intents faild:: intents info failed: {}.'
                .format(msg),
                exc_info=True)
    ...
    def _get_entity_json(self,message):
        exception = None
        entities = []
        try:
            json_data = {
            ...
            data = res_json['data']
            if not data:
                raise Exception('{}:no entities'.format(message.text))
            for d in data:
                ets = {'value':d['entity'],'entity':d['type']}
                entities.append(ets)
            logger.info(
                'Ner success:: get ner res: {}'
                .format(res_json))
        except Exception as e:
            success = False
            msg = '{}'.format(e)
    ...
    def  _handle_urlmessage_with_tracker(self, message, tracker):

        parse_data = {}
        parse_data['intent'] = {'name':self._get_intents_json(message),'confidence':1}
        entities =  self._get_entity_json(message)
        parse_data['entities']= []
        for e in entities:
            parse_data['entities'].append({'value':e['value'], 'entity': e['entity']})
    ...
    def _log_action_on_tracker(self, tracker, action_name, events):
        ...
        # print("Action '{}' ended with events '{}'".format(
        #         action_name, ['{}'.format(e) for e in events]))
    ...
    def _get_next_action(self, tracker):
        ...
        idx,proba = self.policy_ensemble.predict_next_action(tracker, self.domain)
        self.proba = proba
