...
import msxf_flow_engine.utils as utils
import msxf_flow_engine.training as training
from  msxf_flow_engine.events import SlotSet, ActionExecuted
from  msxf_flow_engine.featurizers import MaxHistoryTrackerFeaturizer
...
class PolicyEnsemble(object):
    ...
    def predict_next_action(self, tracker, domain):
        ...
        # second = 0
        # second_index = 0
        # for i,pro in enumerate(probabilities):
        #     if pro < probabilities[max_index]:
        #         if pro > second:
        #             second = pro
        #             second_index = i
        ...
        # third_index = 0
        # for i,pro in enumerate(probabilities):
        #     if pro < probabilities[second_index]:
        #         if pro > third:
        #             third = pro
        #             third_index = i
        ...
        # print("Predicted next action '{}' with prob {:.2f}.".format(
        #         domain.action_for_index(max_index).name(),
        #         probabilities[max_index]))
        # print("Predicted next action '{}' with prob {:.2f}.".format(
        #         domain.action_for_index(second_index).name(),
        #         probabilities[second_index]))
        # print("Predicted next action '{}' with prob {:.2f}.".format(
        #         domain.action_for_index(third_index).name(),
        #         probabilities[third_index]))
    ...
    def ensure_model_compatibility(metadata):
        ...
        #         "".format(model_version,  __version__))
