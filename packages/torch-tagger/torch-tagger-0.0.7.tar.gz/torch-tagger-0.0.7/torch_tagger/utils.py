...
def build_vocabulary(x_data, y_data):
...
def batch_flow(x_data, y_data, word_to_ix, tag_to_ix, batch_size=32):
    ...
    while True:
        if len(x_batch) == batch_size:
            ...
            yield torch.stack(x_batch), torch.stack(y_batch), torch.stack(len_batch)
            x_batch, y_batch = [], []
...
def extrat_entities(seq):
    """
    ['B', 'I', 'I', 'O', 'B', 'I'] -> [(0, 3, ''), (4, 6, '')]
    ['B-loc', 'I-loc', 'I-loc', 'O', 'B-per', 'I-per'] -> [(0, 3, '-loc'), (4, 6, '-per')]
    """
    ret = []
