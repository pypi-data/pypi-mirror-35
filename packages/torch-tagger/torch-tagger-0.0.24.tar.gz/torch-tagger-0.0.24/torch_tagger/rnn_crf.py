...
class RNNCRF(nn.Module):
    ...
    def __init__(self,
        ...
        self.hidden = None # hidden state of RNN
        ...
        for i in range(batch_size):
            ...
            for feat in feats:
                ...
                for next_tag in range(self.tagset_size):
                    ...
                    next_tag_var = trans_score
                    next_tag_var = next_tag_var + forward_var
    ...
    def _score_sentence_batch(self, feats, tags, lengths):
        ...
        for j, feat in enumerate(feats):
            ...
            score = score + self.transitions[tag_next, tag_cur] + feat[:, tag_next]
    ...
    def _viterbi_decode_batch(self, feats):
        """Inference result"""
        # feats dim: [batch_size, seq_len, target_dim]
        batch_size = feats.size()[0]
        ...
        for feat in feats:
            ...
            t = torch.cat(viterbivars_t)
            t = t.permute(1, 0)
            forward_var = (t + feat).view(batch_size, -1)
        ...
        # Pop off the start tag (we dont want to return that to the caller)
        start = best_path.pop()
        assert np.sum(start.cpu().detach().numpy() - np.array([self.tag_to_ix[START_TAG]] * batch_size)) <= 1e-5
        best_path.reverse()
    ...
    def neg_log_likelihood(self, sentence, tags, lengths):
        ...
        forward_score = self._forward_alg_batch(rnn_feats, lengths)
        # gold_score = self._score_sentence(rnn_feats, tags, lengths)
        gold_score = self._score_sentence_batch(rnn_feats, tags, lengths)
