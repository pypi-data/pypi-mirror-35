...
class RNNCRF(nn.Module): # pylint: disable=too-many-instance-attributes
    ...
    def init_embedding(self):
        ...
        if self.pretrained_embedding is not None:
            assert len(self.pretrained_embedding.shape) == 2
            assert self.pretrained_embedding.shape[0] == self.vocab_size
            assert self.pretrained_embedding.shape[1] == self.embedding_dim
            self.word_embeds.load_state_dict({
                'weight': torch.from_numpy(self.pretrained_embedding)
            })
        else:
