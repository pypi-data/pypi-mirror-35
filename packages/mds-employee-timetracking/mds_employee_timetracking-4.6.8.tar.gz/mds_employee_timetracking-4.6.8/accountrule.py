...
class AccountRule(ModelSQL, ModelView):
    ...
    presence = fields.Many2One(string=u'Type of presence', depends=['company'],
        ...
        required=True, ondelete='RESTRICT', domain=[('company', '=', Eval('company', None)),])
    ...
    tariffmodel = fields.Function(fields.One2Many(string=u'used in Tariff model', readonly=True, field=None,
                    help=u'This account rule is used in the specified tariff models.',
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints.extend([
            ('uniq_name', 
            Unique(tab_accountrule, tab_accountrule.name, tab_accountrule.company), 
            u'This name is already in use.'),
            ('uniq_short', 
            Unique(tab_accountrule, tab_accountrule.shortname, tab_accountrule.company), 
    ...
    def on_change_with_tariffmodel(self, name=None):
        ...
        l1 = Tariff.search([('accountrule', '=', self.id)])
    ...
    def get_periods_to_check_by_rule(cls, accountrule, startpos, endpos):
        ...
        start_loc = cls.get_localtime(startpos, accountrule.company.timezone)
        end_loc = cls.get_localtime(endpos, accountrule.company.timezone)
    ...
    def get_match_periods(cls, accountrule, startpos, endpos):
        ...
        for i in check_list:
            (beg1, end1) = i
            beg_utc = cls.get_utctime(beg1, accountrule.company.timezone)
            end_utc = cls.get_utctime(end1, accountrule.company.timezone)
    ...
    def add_item_by_rules(cls, accountrules, perioditem):
        ...
        for i in accounts.keys():
            ...
            accounts[i] = cls.optimize_items2create(l1, ar_obj.company.timezone)
        ...
        for i in accounts.keys():
            ...
            for k in accounts[i]:
                ...
                beg_loc = cls.get_localtime(beg1, ar_obj.company.timezone)
