...
from heresuperset import app, cache, get_css_manifest_files, utils
from heresuperset.exceptions import NullValueException, SpatialException
from heresuperset.utils import DTTM_ALIAS, JS_MAX_INTEGER, merge_extra_filters
...
class TableViz(BaseViz):
    ...
    credits = 'a <a href="https://github.com/airbnb/heresuperset">Superset</a> original'
    is_timeseries = False
...
class MultiLineViz(NVD3Viz):
    ...
    def get_data(self, df):
        ...
        from heresuperset.models.core import Slice
        from heresuperset import db
...
class WorldMapViz(BaseViz):
    ...
    def get_data(self, df):
        from heresuperset.data import countries
        fd = self.form_data
        cols = [fd.get('entity')]
        metric = self.get_metric_label(fd.get('metric'))
        secondary_metric = self.get_metric_label(fd.get('secondary_metric'))
