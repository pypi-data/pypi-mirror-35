...
from .exceptions import ServersideProgrammingError, BenignException
from . import basics
...
def execute_image(exec_env, image_name, path, execution_id, step_num, max_execution_duration):
    ...
    try:
        ...
        ServersideProgrammingError("this container already exists:\n%s" % container_name)
    ...
    try:
        ...
        with CurrentlyDisplayedStatus(exec_env, "downloading Docker program %s" % image_name):
            client.images.pull(image_name)
    with CurrentlyDisplayedStatus(exec_env, "running Docker program %s" % (image_name,)):
    ...
    def _prepare_step_for_execution(self, object_manager, event_identifier, program_identifier, input_arguments_dict):
        ...
        for arg_index, file_object in enumerate(input_file_objects):
            if file_object.creation_step >= object_manager.get_current_step():
                ServersideProgrammingError("objects used in an execution must be from an earlier step")
