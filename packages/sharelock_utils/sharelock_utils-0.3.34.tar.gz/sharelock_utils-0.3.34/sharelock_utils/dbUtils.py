...
def strip_url(url):
    ANNOYING_DOMAINS = {'dronelife.com': 'p', 'abcnews.go.com': 'id', 'suasnews.com': 'p', 'youtube.com': 'v',
                        'youtu.be': 'v'}

    URL_PATTERN = r"https?://(www\.)?"
    if not url:
        return None
    is_annoying_domain = False
    for domain, legit_param in ANNOYING_DOMAINS.items():
        if domain in url:
            parsed_url = urlparse.urlparse(url)
            all_params = urlparse.parse_qs(parsed_url.query)
            url = clean_after_q_mark(url)
            if legit_param in all_params:
                url = '{}?{}={}'.format(url, legit_param, all_params[legit_param][0])

            is_annoying_domain = True
            break

    if not is_annoying_domain:
        url = clean_after_q_mark(url)

    url = re.sub(URL_PATTERN, '', url).strip().strip('/')
    return url
