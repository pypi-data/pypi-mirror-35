...
class getDataByArea(object):
    def __init__(self, tagList=['poi','dianping','population','anjuke','anjukeRent','segmentation'], 
                 aliasList=[],rmTagList=['categories.'], MongoDB='Claudius', collection='TileData'):
        client16 = pmi.connOcm16()
        self.collection = client16[MongoDB][collection]
        self.matchDict = {'meters': 1000, 'processTime':'201711'}
        self.showDict = {'province':1, 'city': 1, 'region': 1, 'township': 1, 'tileName': 1, '_id': 0}
    ...
    def agg(self, area:dict): # poi、dianping {'province':'江苏省'}
        ...
        if not len(self.aliasList)==len(self.tagList):
            for tag in self.tagList:
                alias = tag.replace('.categories.', '_').replace('.size', '').replace('.', '_')
...
if __name__ == '__main__':
    # 测试1
    gda1 = getDataByArea(['poi'], rmTagList=[])
    df1=gda1.proc({'province':'江苏省'})
    l1 = list(df1)
    df_tag1 = pd.DataFrame(l1, columns=['poi'])
    df_tag1.to_excel('江苏省通路标签筛选.xlsx')
    
    # 测试2
    df_ret = pd.read_excel('江苏省通路标签筛选.xlsx', sheetname='零售')
    df_spec = pd.read_excel('江苏省通路标签筛选.xlsx', sheetname='特通')
    
    gada1 = getAggDataByArea(tagList=df_ret['tag'].values.tolist(), minLevel='region')
    df=gada1.agg({'province':'江苏省'})
    
    gada2 = getAggDataByArea(tagList=df_spec['tag'].values.tolist(), minLevel='region')
    df2=gada2.agg({'province':'江苏省'})
    gada3 = getAggDataByArea(tagList=['population.totalSize'], minLevel='region')
    df3=gada3.agg({'province':'江苏省'})
    
    writer = pd.ExcelWriter('江苏省通路点数.xlsx')
    df.to_excel(writer,sheet_name='零售')
    df2.to_excel(writer,sheet_name='特通')
    df3.to_excel(writer,sheet_name='人口')
    writer.save()
