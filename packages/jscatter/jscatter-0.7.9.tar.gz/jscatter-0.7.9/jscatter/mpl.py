...
Example 2::
    ...
    for pp in range(10):
        fig[0].Plot(aa.X,aa.Y*pp,legend='sin',sy=[-1,0.8,-1,''],li=0,markeredgewidth =1)
    fig[1].Plot(bb,legend='sqr=$qq ',sy=2,li=0)
    fig[0].Title('test')
    fig[0].Legend(x=1.2,y=1)
    fig[1].Legend(x=1.2,y=1)
...
def test(keepopen=True):
    ...
    bb.qq=123
    for pp in range(5):
        fig[0].Plot(aa[pp].X,-1*aa[pp].Y,legend='some stufff',sy=[1,pp/10.],li=0)
