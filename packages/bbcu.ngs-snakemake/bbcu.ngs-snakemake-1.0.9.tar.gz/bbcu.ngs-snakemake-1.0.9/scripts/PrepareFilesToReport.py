...
class ComparingFiles(object):
    def __init__(self, samples, samples_deseq, factors, batches, output_dir, job_name):
        self.samples = samples
        self.samples_deseq = samples_deseq
        self.factors = factors
        self.batches = None if batches == ['no-deseq'] else batches
        self.desc_out_file = os.path.join(output_dir, 'sample_desc'+job_name+'.csv')
        self.comp_out_file = os.path.join(output_dir, 'comparisons'+job_name+'.csv')
...
def parse_args():
    ...
    parser.add_argument('--job-name', type=str, help='This name incorporate to name of output file', default='', required=False)
...
if __name__ == '__main__':
    ...
    try:
        steps_args = args.samples, args.samples_deseq, args.factors, args.batches, args.output_dir, args.job_name
        ComparingFiles(*steps_args).print_files()
    except Exception as e:
        logging.info(e.message)
    try:
        steps_args = create_args.getargs('%d_umi_counts', '%s.deDup_counts.corrected.txt',
                                         'countsCorrectedMatrix.txt')
