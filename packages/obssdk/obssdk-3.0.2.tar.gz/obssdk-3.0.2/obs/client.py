...
class _BasicClient(object):
    ...
    def _make_request(self, methodType, bucketName, objectKey=None, pathArgs=None, headers=None, 
        ...
        try:
            result = self._make_request_with_retry(methodType, bucketName, objectKey, pathArgs, headers, 
                       entity, chunkedMode, methodName, readable, parseMethod)
        except _RedirectException as e:
            result = self._make_request_with_retry(methodType, bucketName, objectKey, pathArgs, headers, 
                       entity, chunkedMode, methodName, readable, parseMethod, e.location)
    ...
    def _make_request_with_retry(self, methodType, bucketName, objectKey=None, pathArgs=None, headers=None, 
                       entity=None, chunkedMode=False, methodName=None, readable=False, parseMethod=None, redirectLocation=None):
        flag = 0
        conn = None
        while True:
            try:
                conn = self._make_request_internal(methodType, bucketName, objectKey, pathArgs, headers, entity, chunkedMode, redirectLocation)
    ...
    def _rename_request_headers(self, headers, method):
        ...
        if isinstance(headers, dict):
            for k, v in headers.items():
                if k == const.CONTENT_DISPOSITION_HEADER:
                    new_headers[k] = v
                    continue
                if k is not None and v is not None:
                    ...
                    if(k.lower().startswith(self.ha._get_header_prefix())):
