...
class Connection(object):
    ...
    def _print_request(self, method, url, payload=None):
        if self._trace in [Connection.TRACE_REQUEST, Connection.TRACE_REQUEST_RESPONSE]:
            logging.debug('%s %s %s %s' % (datetime.datetime.now().isoformat(), method, url, payload))
    
    def _print_response(self, response):
        if self._trace == Connection.TRACE_REQUEST_RESPONSE:
            logging.debug('%s %s %s %s' % (datetime.datetime.now().isoformat(), response.status_code, response.reason, response.raw.data))
