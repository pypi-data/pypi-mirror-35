...
class Pcc(Base):
	...
	def BackupPceStart(self, Arg1):
		"""Executes the backupPceStart operation on the server.

		Start Backup-PCEs

		Args:
			Arg1 (list(str[None|/api/v1/sessions/1/ixnetwork/topology])): This parameter requires a list of /topology/./pcepBackupPCEs object references

		Raises:
			NotFoundError: The requested resource does not exist on the server
			ServerError: The server has encountered an uncategorized error condition
		"""
		return self._execute('backupPceStart', child='pcepBackupPCEs', payload=locals(), response_object=None)

	def BackupPceStart(self, Arg1, SessionIndices):
		"""Executes the backupPceStart operation on the server.
	...
	def BackupPceStop(self, Arg1):
		...
		Raises:
			...
			ServerError: The server has encountered an uncategorized error condition
		"""
		return self._execute('backupPceStop', child='pcepBackupPCEs', payload=locals(), response_object=None)

	def BackupPceStop(self, Arg1, SessionIndices):
		"""Executes the backupPceStop operation on the server.
