...
import corenlp
import dateparser
# os.environ["CORENLP_HOME"]="C:\stanford-corenlp-full-2018-02-27"
import argparse
...
class consumer(threading.Thread):
    def __init__(self, q,cli_args,multi_log_lock,identifier,session,out,threadLock,nlp):
        global Thread_id
...
#                 self.threadLock.acquire()
                text=sent_tokenize(task['content'])
                ...
                for txt in text:
                    ...
                    if len(txt)<500:
                        try :
                            txt.replace("-"," ")             
                            tree0 = self.nlp.parse(txt)
                            tree0=tree0.strip()
                            tree0=tree0.replace("\n","")
                            tree0=tree0.replace("\r","")
                            tree0=tree0.replace("  "," ")
                            tree0=tree0.replace("  "," ")
                            tree0=tree0.replace("  "," ")
                            ner = self.nlp.ner(txt)
                            ner_list = ""
                            nerlist=[]
                            ner_list2={}
                            ner_list3={}
        ...
        #                     print(ner_list)
                            ...
                            last["ner_type"]=""
                            for i in ner:               
                                if i[1] != "O" and last["ner_type"]=="":
                                    ner_list2["ner_name"] = i[0] 
                                    ner_list2["ner_type"] = i[1]
                                    tmp=str(ner_list2)
                                    nerlist.append(eval(tmp))
                #                     print nerlist 
                                elif i[1] != "O" and last["ner_type"]!="":
                                    if i[1]==last["ner_type"]:
                                        nerlist[-1]["ner_name"]=nerlist[-1]["ner_name"]+" "+i[0] 
                                    else:
                                        ner_list3["ner_name"] = i[0] 
                                        ner_list3["ner_type"] = i[1]
                                        tmp= str(ner_list3)
                                        nerlist.append(eval(tmp))
                                last["ner_name"]=i[0]
                                last["ner_type"]=i[1]
                            if nerlist:    
                                for one_ner in nerlist:
                                    if one_ner["ner_type"]=="DATE":
                                        date0 =  one_ner["ner_name"]
                                        print("==========date0",date0)
                                        try: 
                                            time0 = dateparser.parse(date0)
                                            print("===========",time0)
                                        except:
                                            time0=""
                                        if time0 :
                                            try:
                                                time0=time0.strftime('%Y-%m-%d %H:%M:%S')
                                            except:
                                                time0=""
        #                             print(date0)
                            filterner={}
                            filterner["sentner"] = ner_list
                            filterner["paraNo"]=0
                            filterner["sentNo"]=k
                            filterner["sent"]=txt
                            filterner["tree"]=tree0
                            filterner["nerlist"]=nerlist
                #             print(filterner)
                            core_nlp.append(filterner)
                            k += 1
                        except:
                            continue
                core["corenlp"]=core_nlp
#                 self.threadLock.release()
                if core:                     
                    process_task(task, self.out, self.multi_log_lock, self.session, self.identifier,core,time0)
...
#         self.threadLock=threadLock
    def run(self):
        while True: 
            ...
            tmp_list = access_solr.read_stories(1,self.id,self.i)
...
#     write_multiprocess_log(multi_log_lock, '{}Process {}: {}'.format(u'', os.getpid(), u'started.'))
    ...
    while True:
        try:
            task = q.get(block = True) #不设置阻塞的话会一直去尝试获取资源                
#             task = q.get(block = True, timeout = 50) #不设置阻塞的话会一直去尝试获取资源
#                 print("cosumer",task)
#                 self.threadLock.acquire()
            content1='''Ambassador Joanna Wronecka, who heads the Polish mission at the United Nations in New York, talks to Pawel Kryszczak about Poland's activities in Africa and the priorities of Polish development assistance.'''
            # text=sent_tokenize(task['content'])
            text = sent_tokenize(content1)
            k=0
            core={}
            core_nlp=[]
            time0=""
...
#                 self.threadLock.release()
            if core:                     
                process_task(task, out, session, identifier,core,time0)
...
#     pool.apply_async(read, (q,cli_args, ))
    p1 = Process(target=read,args=(q,cli_args,))
    p2 = Process(target=read,args=(q,cli_args,))
    p3 = Process(target=read,args=(q,cli_args,))
    p4 = Process(target=read,args=(q,cli_args,))
#     p5 = Process(target=read,args=(q,cli_args,))
#     p6 = Process(target=read,args=(q,cli_args,))
...
def process_task(one_task, out_file, session, identifier,core,time0):
    events = {}
...
#         print("======>databasewriter.write_events() function to start running.")
        databasewriter.write_events(time0, updated_events, session, True)
