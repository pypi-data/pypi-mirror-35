...
def select_hard_sample(anchor_feature, positive_feature, negative_feature, function, margin):
    ...
    while selected_indexes[(selected_indexes == 1).squeeze()].squeeze().dim() == 0:
        margin = margin + 0.1
        print("sdf")
        selected_indexes = l2 - l1 < margin
...
class TripletTrainer:
    ...
    def train(self, triplet_batch):
        ...
        anchor_pred = self.model.seq[-1](selected_anchor_feature)
        positive_pred = self.model.seq[-1](selected_positive_feature)
        negative_pred = self.model.seq[-1](selected_negative_feature)
