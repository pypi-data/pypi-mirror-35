...
class MariaSQL(object):
    ...
    def __init__(self, host = 'localhost', port = 3306, user = 'root', password = 'password', db = 'mysql', charset = 'utf8'):
        self.host = host
        self.port = port
        self.user = user
    ...
    def create_table(self, name, tabledef = None):
        sql = """
            create table if not exists {name} ({columns}) ENGINE=InnoDB CHARACTER SET utf8;
        """
