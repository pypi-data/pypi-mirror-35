...
@cli.group(name='theme', short_help='Manage themes.', help='This set of sub-commands help you to manage themes.')
def theme_cli():
    pass
...
def list_command():
    themes = list(filter(lambda x: os.path.isdir(os.path.join(get_themes_dir(), x)), os.listdir(get_themes_dir())))
    if not themes:
...
@theme_cli.command('install', short_help='Install a theme from GitHub.',
                   help='This command will install theme from GitHub for you. '
                        ...
                        '  $ veripress theme install someone/the-theme --branch the-theme-branch\n\n'
                        'If you want to customize the theme name installed, use "--name" parameter.')
@click.argument('theme', nargs=1)
@click.option('--branch', '-b', default='master',
              help='From which branch to clone the theme. '
                   'Only matters when a string like "user/repo" is passed in as the theme name.')
@click.option('--name', '-n', help='Specify the theme name. If this is not specified, a default one will be used.')
...
def uninstall_command(theme):
    ...
    if os.path.isdir(theme_path):
        ...
        click.echo('"{}" theme has been uninstalled successfully.'.format(theme))
    else:
...
def uninstall_command(theme):
    ...
    if os.path.isdir(theme_path):
        ...
        os.chdir(cur_dir)
        if exit_code == 0:
            click.echo('\n"{}" theme has been updated successfully.'.format(theme))
        else:
            click.echo('\nSomething went wrong. Do you forget to install git? Or did you modify the theme by yourself?')
    else:
        click.echo('There is no such theme.')
