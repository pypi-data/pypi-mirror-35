...
class Binary_Sentiment_Analysis_Model(Model):
    ...
    def init_input_data(self, sentiment_texts):
        x = np.zeros( (len(sentiment_texts), self.input_shape[0]) , dtype='uint16' )
        for i, text in enumerate(sentiment_texts):
            ...
            for word in word_list:
                if word in self.model.input_dictionary:
                    x[i][x_elem_size] = self.model.input_dictionary[word]
                    x_elem_size += 1
        x = sequence.pad_sequences(x, maxlen = self.input_shape[0])
        return x
    
    def prepare_train_data(self, sentiment_texts, boolean_sentiment_values, skip_top, num_of_words_to_consider, max_seq_len = 400):
        ...
        self.y = np.asarray(boolean_sentiment_values)
    ...
    def create_model(self, input_dim):
        ...
        output_layer = Dense(1, activation = 'sigmoid')(act1)
    ...
    def train(self,
            ...
            callbacks = None,
            shuffle = True):
            self.compile_model(optimizer = 'adam', loss = 'binary_crossentropy', metrics = ['accuracy'])
