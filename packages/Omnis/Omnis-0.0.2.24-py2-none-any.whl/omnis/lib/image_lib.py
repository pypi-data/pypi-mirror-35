...
def cast_to_float_array(img_list):
    try:
        array_of_images = numpy.asarray(img_list, dtype=numpy.float32)
    except:
        left_array = cast_to_float_array( img_list[: len(img_list)/2] )
        right_array = cast_to_float_array( img_list[len(img_list)/2 :] )
        array_of_images = numpy.concatenate((left_array, right_array), axis = 0)
...
def directory_images_to_arrays(directory_path, target_size):
    ...
    array_of_images = cast_to_float_array(img_list)
    array_of_images /= 255
