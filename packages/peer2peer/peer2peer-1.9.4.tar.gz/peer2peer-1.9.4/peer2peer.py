...
def pipe(sub_from, from_channel, pub_to, to_channel=''):
    if   to_channel.startswith(':'): addr = 'localhost' + to_channel
    if from_channel.startswith(':'): addr = 'localhost' + from_channel
