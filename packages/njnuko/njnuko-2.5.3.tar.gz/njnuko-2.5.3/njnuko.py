...
def move_dup(folder):
    if not os.path.exists(os.path.join(os.path.dirname(folder),'duplicate')):
        os.mkdir(os.path.join(os.path.dirname(folder),'duplicate'))
    desfold = os.path.join(os.path.dirname(folder),'duplicate')
    check_files(folder,desfold)

def check_files(folder,desfold):
    global all_md5
    ...
    for i in os.listdir(folder):
        f = open(log,'a')
        if os.path.isdir(os.path.join(folder,i)):
            check_files(os.path.join(folder,i),desfold)
        else:
            #print("test"+e.lower())
            # 
