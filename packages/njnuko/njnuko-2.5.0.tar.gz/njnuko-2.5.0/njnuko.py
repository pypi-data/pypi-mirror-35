...
def filecount(source_folder):
    filecount=int(os.popen(source_folder).read())
    return(filecount)
...
def check_files(folder):
    global all_md5
    if not os.path.exists(os.path.join(os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir)),'duplicate')):
        os.mkdir(os.path.join(os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir)),'duplicate'))
    desfold = os.path.join(os.path.abspath(os.path.join(os.path.dirname("__file__"),os.path.pardir)),'duplicate')
    log = os.path.join(desfold,'check.log')
    if not os.path.exists(log):
        f = open(log,'a')
        f.write("Total File Number:" + filecount +'\n')
        global all_md5
        all_md5={}
    for i in os.listdir(folder):
        f = open(log,'a')
        if os.path.isdir(os.path.join(folder,i)):
            check_files(os.path.join(folder,i))
        else:
            #print("test"+e.lower())
            # 
            if md5sum(os.path.join(folder,str(i))) in all_md5.values():
                shutil.move(os.path.join(folder,str(i)),os.path.join(desfold,str(i)))
                f.write("File" + os.path.join(folder,str(i)) + ', is duplicate')
                print("File" + os.path.join(folder,str(i)) + ' is duplicate')
                f.write('\n')
            else:
                k = len(all_md5) + 1
                all_md5[k]=md5sum(os.path.join(folder,str(i)))
                f.write("File" + os.path.join(folder,str(i)) + ', is new' + ' And MD5 is,' + all_md5[k])
                print("File" + os.path.join(folder,str(i)) + ', is new' + ' And MD5 is,' + all_md5[k])
                f.write('\n')
        f.close()
