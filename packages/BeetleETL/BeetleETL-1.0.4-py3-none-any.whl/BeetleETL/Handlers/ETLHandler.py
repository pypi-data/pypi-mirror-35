...
class ETLHandler():
    ...
    def setup_handles(self):
        ...
        if "mongoFilter" in self.config_handler.config:
            filter_dict = {}
            for filter_alias in self.config_handler.config['mongoFilter']:
                
                # get mongo search query from mongotsql config
                new_query = self.mongo_handler.get_mongo_search_query(
                    alias=filter_alias, 
                    input_value="replace",
                    cast_to=None
                    )
                ...
                MongoHandler.deepmerge_dicts(filter_dict, new_query)

            self.mongo_handler.mongo_filter = filter_dict
            print(' * * Mongo Filter:  {}'.format(str(filter_dict)))


    def get_from_mongo(self, filter_dict={}, add_index=False):
        ...
        if filter_dict == {} and self.mongo_handler.mongo_filter != {}:
