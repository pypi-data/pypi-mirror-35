...
class MongoHandler():
    ...
    def pull_collection(self, query_filter={}, add_index=False):
        ...
        for doc in cur:
            ...
            if str(doc["_id"]) == self.config['data']['last_mongo_id_pulled']:
                break
        ...
        if docs_pulled > 0:
            self.config['data']['last_mongo_id_pulled'] = newest_pulled_id
            return package_list          
        else:
            return package_list
    ...
    def get_mongo_search_query(self, alias, input_value, cast_to=""):
        """ looks through config interjectFilter and attempts to return a
            search query to use in a mongo query

            Arguments:
                alias {string} -- the key in interjectFilter to look for
        ...
        try:
            if 'interjectFilter' not in self.config:
                query = {alias : input_value}
            else:
                # open interject filter dictionary from config file and parse to string
                dictionary = json.loads(json.dumps(self.config['interjectFilter'][alias]))
