...
class BaseApi:
    ...
    def run(self, *args, **kargs):
        ...
        if self.name == 'repo-set':
            ...
            if url.endswith(".git"):
                path = os.path.join(self.__class__.BASE_DIR ,url.split("/")[-1].split(".git")[0])
            else:    
                path = os.path.join(self.__class__.BASE_DIR, url.split("/")[-1])
