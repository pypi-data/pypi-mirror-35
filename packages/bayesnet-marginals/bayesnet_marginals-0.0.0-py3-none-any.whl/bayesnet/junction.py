...
class Marginals(object):
    def __init__(self, pmfs: Sequence[ProbabilityMassFunction]):
        ...
        for junction_tree in junction_trees:
            ...
            local_marginals, marginals_update_list = _marginals(junction_tree)
...
def _collect(junction_tree: JunctionTreeNode) \
    ...
    for node in postorder(descendants(junction_tree)):
        ...
        if node.parent is not None:
            ...
            for variable in node.variables - node.boundary:
                ...
                if len(subtables) > 1:
                    ...
                    def product(left, right):
                        new_table = ProbabilityMassFunctionProduct(left, right)
                ...
                marginal = ProbabilityMassFunctionMarginal(subtable, variable)
            ...
            if len(tables) > 1:
                table = reduce(ProbabilityMassFunctionProduct, tables)
...
def _distribute(junction_tree: JunctionTreeNode) \
        -> List[ProbabilityMassFunction]:
...
def _marginals(junction_tree: JunctionTreeNode) \
        ...
        if len(new_variables) > 0:
            ...
            for variable in new_variables:
                ...
                for to_marginalize in node.variables - {variable}:
                    marginal = ProbabilityMassFunctionMarginal(marginal,
                                                               to_marginalize)
                    update_list.append(marginal)
