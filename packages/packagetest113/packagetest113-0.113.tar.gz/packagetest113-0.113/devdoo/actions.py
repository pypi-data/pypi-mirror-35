...
class Actions(object):
    ...
    def __config(self):
        ...
        if self.datarest.ready(): # and self.service.ready() and self.database.ready():
            ...
            self.status.error("INVALID_SERVICE", None, ["NAO ESTA PRONTO PARA EXECUTAR O SERVICO", "PARAMETROS FALTANDO"])
    ...
    # insert
    # --------------------------------
    # Prepara documento para ser inserido na base de dados
    ...
    def update_config(self, scheme, fields_default):
        ...
        print "LIST_SCHEME_CLEAN", list_scheme_clean, self.datarest.body()
        ...
        # Retorna a lista de dados que deverá ser enviada para o servidor de base de dados
    ...
    #
    # data - Objeto contendo dados existentes do mesmo grupo ou um objeto vazio para receber novos elementos
    ...
    def __prepare_insert_data_fields(self, list_data):
        ...
        for item in list_data:
            ...
            if (item["field"]) in result.keys():
                # Caso não esteja na lista então o campo é incluído
                # Prepara objetos-filhos no padrão permitido para registrar na base de dados
                result[(item["field"])] = self.__insert_data_join(result[(item["field"])], item["value"])
            else:
                # Registra o campo e valor na lista de campos válidos do documento
                result[(item["field"])] = item["value"]
        ...
        # Retorna dicionário de dados do documento
        return result
