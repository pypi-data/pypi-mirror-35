...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['label']:
            items = {k: v for k, v in _registry.items() if k == options['label']}
        if options['contains']:
            items = {k: v for k, v in _registry.items() if options['contains'] in k}
