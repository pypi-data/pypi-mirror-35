...
class Formula(object):
    ...
    def process_ast(
        ...
        if type(t).__name__ == 'BinOp':
            ...
            if t.func.id == 'C':
                ...
                subterms = []
                self.process_ast(t.args[0], terms=subterms, has_intercept=has_intercept, ops=[t.func.id] + ops,
                                 rangf=rangf)
                terms += subterms
    ...
    def process_irf(
        ...
        if ops is None:
            ops = []
        assert t.func.id in Formula.IRF or spline.match(
            t.func.id) is not None, 'Ill-formed model string: process_irf() called on non-IRF node'
        irf_id = None
        coef_id = None
        cont = False
        ranirf = False
            ...
            for k in t.keywords:
                if k.arg == 'irf_id':
                    ...
                    if type(k.value).__name__ == 'Num':
                        ...
                        assert type(
                            k.value.op).__name__ == 'USub', 'Invalid operator provided to to IRF parameter "%s"' % k.arg
                        assert type(
                            k.value.operand).__name__ == 'Num', 'Non-numeric initialization provided to IRF parameter "%s"' % k.arg
    ...
    def compute_2d_predictor(
        ...
        if predictor_name in ['cosdist2D', 'eucldist2D']:
            ...
            assert len(
                embedding_colnames) > 0, 'Model formula contains vector distance predictors but no embedding columns found in the input data'
            ...
            for i in range(0, len(first_obs), minibatch_size):
                sys.stderr.write(
                    '\rProcessing batch %d/%d' % (i / minibatch_size + 1, math.ceil(len(first_obs) / minibatch_size)))
                sys.stderr.flush()
    ...
    def __str__(self):
        ...
        if None in terms:
            ...
            term_strings.append(' + '.join(
                ['C(%s, %s)' % (' + '.join([x.name() for x in y['impulses']]), y['irf']) for y in new_terms]))
        ...
        for rangf in terms:
            ...
            if not self.has_intercept[rangf]:
                new_terms_str += '0 + '
            new_terms_str += ' + '.join(
                ['C(%s, %s)' % (' + '.join([x.name() for x in y['impulses']]), y['irf']) for y in
                 new_terms]) + ' | %s)' % rangf
    ...
    def to_lmer_formula_string(self, z=False):
        ...
        for gf in random:
            out += ' + (' + ('1 + ' if self.has_intercept[gf] else '0 + ') + ' + '.join(
                [x for x in random[gf]]) + ' | ' + gf + ')'
...
class IRFNode(object):
    ...
    def __init__(
        ...
        if family is None or family in ['Terminal', 'DiracDelta']:
            assert irfID is None, 'Attempted to tie parameters (irf_id=%s) on parameter-free IRF node (family=%s)' % (
            irfID, family)
        if family != 'Terminal':
            assert coefID is None, 'Attempted to set coef_id=%s on non-terminal IRF node (family=%s)' % (coefID, family)
            assert impulse is None, 'Attempted to attach impulse (%s) to non-terminal IRF node (family=%s)' % (
            impulse, family)
