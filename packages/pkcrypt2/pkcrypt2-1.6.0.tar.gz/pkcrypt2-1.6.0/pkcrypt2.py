...
def main():
    ...
    parser.add_argument('-sign2',    action='store_true',    help='sign a message, no date, no new verify line')
    ...
    if args.mine0:
        ...
        x = env_class()(sys.stdin.read(), suppress_date=False).sign(key())
        if args.r:
            print('- ' + x.hexdigest)
        if args.full:
            sys.stdout.write(repr(x))
