...
def load_user_mapping():
    if not os.path.isfile('mangled_users_mapping.txt'):
        return
    with open('mangled_users_mapping.txt', 'rU') as f:
        reader = csv.reader(f)
        for r in reader:
            login = r[0].decode('utf-8')
            mangled_name = r[1].decode('utf-8')
            new_user = youtrack.User()
            new_user.fullName = mangled_name
            new_user.login = new_user.fullName.lower().replace(' ', '.')
            new_user.email = new_user.login + '@fake.com'
            mangled_users[login] = new_user


def append_user_mapping(login, mangled_name):
    with open('mangled_users_mapping.txt', 'a') as f:
        users_csv = csv.writer(f)
        users_csv.writerow([utf8encode(login), utf8encode(mangled_name)])


def mangle_user(user):
    if user.login == 'guest':
        return user
    if user.login not in mangled_users:
        new_user = youtrack.User()
        new_user.fullName = names.get_full_name()
        new_user.login = new_user.fullName.lower().replace(' ', '.')
        new_user.email = new_user.login + '@fake.com'
        mangled_users[user.login] = new_user
        append_user_mapping(user.login, new_user.fullName)
    return mangled_users[user.login]
...
def create_bundle_from_bundle(source, target, bundle_name, bundle_type, user_importer):
    ...
    if bundle_name in target_bundle_names:
        ...
        if isinstance(source_bundle, youtrack.UserBundle):
            ...
            user_importer.importUsersRecursively(set([mangle_user(u) for u in source_bundle.get_all_users()]))
            ...
            mu = [mangle_user(u) for u in source_bundle.users]
            users_to_add = [user for user in mu if
        ...
        if isinstance(source_bundle, youtrack.UserBundle):
            ...
            source_bundle.users = [mangle_user(u) for u in source_bundle.users]
            ...
            for elem in source_bundle.values:
                if elem.owner is not None:
                    elem.owner = mangle_user(source.getUser(elem.owner)).login
        user_importer.importUsersRecursively([mangle_user(u) for u in users])
...
def create_project_stub(source, target, projectId, user_importer):
    ...
    lead = mangle_user(source.getUser(project.lead))
    project.lead = lead.login
    ...
    for projectId in project_ids:
        ...
        for pcf_ref in project_custom_fields:
            ...
            if hasattr(pcf, "bundle"):
                # create_bundle_from_bundle(source, target, pcf.bundle,
                #                           source.getCustomField(pcf.name).type,
                #                           user_importer)
                try:
        ...
        while True:
            try:
                ...
                for issue in issues:
                    ...
                    reporter = mangle_user(source.getUser(issue.reporterName))
                    issue.reporterName = reporter.login

                    users.add(reporter)
                    if issue.hasAssignee():
                        if isinstance(issue.Assignee, (list, tuple)):
                            assignee = [mangle_user(source.getUser(u))
                                        for u in issue.Assignee]
                            issue.Assignee = [u.login for u in assignee]
                            users.update(assignee)
                        else:
                            assignee = mangle_user(source.getUser(issue.Assignee))
                            issue.Assignee = assignee.login
                            users.add(assignee)
                    # TODO: http://youtrack.jetbrains.net/issue/JT-6100
                    updater = mangle_user(source.getUser(issue.updaterName))
                    issue.updaterName = updater.login
                    users.add(updater)
                    if issue.hasVoters():
                        if isinstance(issue.voterName, (list, tuple)):
                            voters = [mangle_user(source.getUser(u))
                                      for u in issue.voterName]
                            issue.voterName = [u.login for u in voters]
                            users.update(voters)
                        else:
                            voter = mangle_user(source.getUser(issue.voterName))
                            issue.voterName = voter.login
                            users.add(voter)
                    for comment in issue.getComments():
                        comment_author = mangle_user(source.getUser(comment.author))
                        comment.author = comment_author.login
                    ...
                    for comment in issue.getComments():
                        ...
                        def repl_login(m):
                            l = m.group()[1:]
                            if l in mangled_users:
                                return '@' + mangled_users[l].login
                            return m.group()
                        comment.text = re.sub('@[\.@,]?\w+(?:[\.,@]+?\w+)*', repl_login, comment.text, re.U)
                ...
                for issue in issues:
                    ...
                    for a in issue.getAttachments():
                        ...
                        if a.authorLogin == '<no user>':
                            author = None
                        else:
                            author = source.getUser(a.authorLogin)
                        if author is not None:
                            author = mangle_user(author)
                            a.authorLogin = author.login
                            users.add(author)
                    user_importer.importUsersRecursively(users)
