...
class tko(BaccSys):
    def __init__(self, sys_name=""):
        parent_ret = super(tko,self).__init__("tko") #first, call parent w/name
    def hand_pre(self, hand_num):
        parent_ret = super(tko,self).hand_pre(hand_num) #first, call parent
        if not self.quit_shoe:
            pass
            '''
            preTIcount... last 5 hands, who has the most P or B? that's the base
            '''
        return ""
    def end_shoe(self):
