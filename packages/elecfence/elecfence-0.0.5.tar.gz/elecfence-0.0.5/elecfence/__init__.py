...
class Area(object):
    def __init__(self, area):
        ...
        if isinstance(area, str):
            ...
            points = area_json['coordinates']
        if isinstance(area, list):
            points = area

        self._gener_line(points)

    def _gener_line(self, points):
        self._points = [Point(pt) for pt in points]
        self._lines = list()
        first_pt = None
        last_pt = None
    ...
    def to_geojson(self):
        from geojson import Polygon
        point = [(float(x._long), float(x._lat)) for x in self._points]
