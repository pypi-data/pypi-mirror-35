...
class Brokers:
    ...
    '''
    def __init__(self, service_port, brokers, status, has_connection_backend=False, has_connect_frontend=False):
        # Variaveis privadas
        self.__brokers = []
        ...
        if len(self.brokers) > 0:
            for broker_id in self.brokers:
                self.__brokers.append(
                    Broker(broker_id, self.status, self.service_port, self.has_connection_backend, self.has_connect_frontend))
        else:
