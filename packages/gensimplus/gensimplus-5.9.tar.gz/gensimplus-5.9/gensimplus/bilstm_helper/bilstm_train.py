    ...
    def train(self):
        ...
        for epoch in range(self.max_max_epoch): # total epoch
            ...
            for batch in range(self.tf_batch_num):
                ...
                X_batch, y_batch = self.datahelper.next_batch()
                feed_dict = {self.X_inputs:X_batch, self.y_inputs:y_batch, self.lr:self._lr, self.batch_size:10, self.keep_prob:0.5}
                X_batch, y_batch = self.datahelper.next_batch()
...
class Bilstm_Att(object):
    ...
    def __init__(self, data_helper):
        ...
        self.datahelper = data_helper
        self.gen=self.datahelper.gen_train_data(name='train')
...
# ====================== model inout=====================
    ...
    def input_placeholder(self):
        with tf.variable_scope('Inputs'):
            self.X_inputs = tf.placeholder(tf.float32, [10, self.timestep_size,self.embedding_size], name='X_input')
    ...
    def init_attention(self):
        with tf.variable_scope('attentions'):
            #pdb.set_trace(
            att_inputs = tf.reshape(tf.cast(self.X_inputs,tf.float32), (10, 200*128))
            #att_inputs = tf.reshape(tf.nn.embedding_lookup(self.embedding, self.X_inputs), (10, 200*128))
            # 10 * 200 * 128
            # 10 * 200 * 9
            softmax_w_att = self.weight_variable([200*128, 200*3])
            softmax_b_att = self.bias_variable([10, 200*3])
            ...
            softmax_w_att2 = self.weight_variable([600, 600])
            softmax_b_att2 = self.bias_variable([10, 600])
            y_pred_trans = tf.reshape(self.y_pred,(10, 600))
            rnn_output = tf.matmul(y_pred_trans, softmax_w_att2) + softmax_b_att2


            r = tf.reshape(rnn_output,[-1])
            d = tf.reshape(dnn_output,[-1])
            rd_tensor = tf.concat((r,d),0)
            print(rd_tensor)
            rdl = 12000#rd_tensor.shape[0]
            print(rdl)
            w0 =  self.weight_variable([rdl,rdl])
            b0 =  self.bias_variable([1,rdl])
            attention = tf.matmul(tf.reshape(rd_tensor,(1,rdl)), w0) + b0

            rd_tensor = tf.reshape(rd_tensor, (1,rdl))
...
if __name__ == "__main__":
    ...
    如何开始一段训练
    """
    train_bilstm_ner_ins =  Train_Bilstm_Ner() #实例化训练类
    train_bilstm_ner_ins.model.fit_train(train_bilstm_ner_ins.sess)#开始训练
