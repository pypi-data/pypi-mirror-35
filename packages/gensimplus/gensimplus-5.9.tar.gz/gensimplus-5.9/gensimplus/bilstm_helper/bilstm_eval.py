...
class Eval_Ner(object):
    ...
    def predict_sent(self, sent):
        ...
        #pdb.set_trace()
        X_batch, y_batch = np.array(sent_data).reshape(10,200), np.array([0]*2000).reshape(10,200)
    ...
    def predict(self):
        ...
        start_time = time.time()
        while(1):
          X_batch, y_batch = datasrc.next_batch('eval')
          #pdb.set_trace()
          _print(X_batch.shape)
          _print(y_batch.shape)
          feed_dict = {self.X_inputs:X_batch, self.y_inputs:y_batch, self.lr:1e-4, self.batch_size:10, self.keep_prob:1.0}
          ...
          #pdb.set_trace()
          import gensim
          w2vm = gensim.models.word2vec.Word2Vec.load("/home/siyuan/bilstm/model/w2vm")

          for m in range(10000):
              for i in range(200):
                  print(_y_pred_meta[m][i], y_batch[m][i], w2vm.similar_by_vector(X_batch[m][i])[0][0])
        #return result, y_batch, _y_pred_meta.reshape(2000,8)
