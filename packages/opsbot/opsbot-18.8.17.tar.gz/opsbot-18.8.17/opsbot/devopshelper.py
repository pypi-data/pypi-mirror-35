...
class OpsbotHelper:
    ...
    def opsbot_setting(self, params):
        ...
        if field == "password_length": 
            ...
            scripts.append("echo Auto Backup : {1}")
    ...
    def opsbot_site(self, params):
        ...
        if self.auto_backup == 1 and source_git != "" :             
            scripts.extend(self.opsbot_make_contab_daily("sourcebackup {site}".format(site=site)))
            comment_line_1 = ""
        else :
            comment_line_1 = "#"
        
        if self.auto_backup == 1 and database_git != "" :
            scripts.extend(self.opsbot_make_contab_daily("dbackup {site}".format(site=site)))
            comment_line_2 = ""
        else:
            comment_line_2 = "#"
        #tool deploysite.                
    ...
    def opsbot_tool(self, params):
        ...
        if cmd == "certbotsafe":
            ...
            source = self.template("global-tool/certbotsafe.sh").format(admin = self.admin).replace("$","\\$")
            ...
            source = self.template("global-tool/dbbackup.sh").replace("$","\\$")
            ...
            source = self.template("global-tool/sourcebackup.sh").replace("$","\\$")
            ...
            source = self.template("global-tool/dbdump.sh").replace("$","\\$")
