...
class OpsbotHelper:
    ...
    def opsbot_site(self, params):
        ...
        text = self.template("site/vhost.sh").format(
            site=site, 
        ...
        text = self.template("site-tool/dbimport.sh" ).format(
            site=site, 
            db_username=owner,
            db_password=self.user_passwords[owner]["mysql"],
        )
        scripts.extend(self.make_site_tool(site, "dbimport.sh", text, owner ))

        #tool backup.
        if self.auto_backup == 1 and source_git != "" :             
            scripts.extend(self.opsbot_make_contab_daily("sourcebackup {site}"))
            comment_line_1 = ""
        else :
            comment_line_1 = "#"
        
        if self.auto_backup == 1 and database_git != "" :
            scripts.extend(self.opsbot_make_contab_daily("dbackup {site}"))
            comment_line_2 = ""
        else:
            comment_line_2 = "#"
    ...
    def opsbot_tool(self, params):
        ...
        if cmd == "certbotsafe":
            ...
            source = self.template("global-tool/certbotsafe.sh").format(admin = self.admin)
            scripts =  self.make_tool_available_global(source,CONSTANT.TOOL_SAFE_CERTBOT_NAME) 
        elif cmd == "dbbackup":
            source = self.template("global-tool/dbbackup.sh")
            scripts =  self.make_tool_available_global(source, "dbbackup")
        elif cmd == "sourcebackup":
            source = self.template("global-tool/sourcebackup.sh")
            scripts =  self.make_tool_available_global(source, "sourcebackup")
        return scripts
    ...
    def make_owner_can_sudo_site_tool(self, site, tool_file, owner):
        scripts = []
        text = self.template("util/make_owner_can_sudo_site_tool.sh").format(site=site, tool_file = tool_file, owner=owner)
        scripts.append(text)
        return scripts
