...
class OpsbotHelper:
    ...
    def opsbot_site(self, params):
        ...
        for param in params:
            ...
            elif str(param).startswith("--public-directory="):
        ...
        access_log = "/var/www/{site}/log/access-{site}.log".format(site=site)
        error_log = "/var/www/{site}/log/error-{site}.log".format(site=site)
    ...
    def opsbot_tool(self, params):
        ...
        if cmd == "allow_normal_user_add_site":
            ...
            print( "DEPRECATED. use logratate instead")
            #write tool
            reset_log_sh = self.template("tool/tool_reset_log.sh")
            scripts = self.make_tool_available_global(reset_log_sh, CONSTANT.TOOL_RESET_LOG)
            ...
            #     script = "{resetlog} \"{log}\" {maximum_bytes}".format(resetlog = CONSTANT.TOOL_RESET_LOG, log = log, maximum_bytes = maximum_bytes)
