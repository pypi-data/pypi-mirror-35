...
def count_parameters(model_or_dict_or_param):
    ...
    Usage:
    1. to count trainable+intrainbale params
    ...
    if isinstance(model_or_dict_or_param,torch.nn.Module):
        w = [i for i in model_or_dict_or_param.parameters()]
    ...
    for k in w:
        num += np.prod(k.shape)
        dtype = k.dtype if k.dtype in dtype_size else torch.float32
        KBs += num * dtype_size[dtype]
    print('Torchfun:parameters:',Bytes/1024/1024,'MB',num,'params')
    return num
