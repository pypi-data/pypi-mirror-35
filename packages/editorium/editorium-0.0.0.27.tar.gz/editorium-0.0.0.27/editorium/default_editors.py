...
from mhelper import EFileMode, FileNameAnnotation, Filename, HReadonly, SwitchError, abstract, ignore, override, sealed, virtual, Password, array_helper, exception_helper, Dirname, AnnotationInspector, Documentation
...
class StringEditor( AbstractEditor ):
    ...
    def on_set_value( self, value: str ):
        ...
        exception_helper.assert_type( "value", value, str )
...
class PasswordEditor( AbstractEditor ):
    ...
    @classmethod
    def on_can_handle( cls, info: EditorInfo ):
        return info.annotation.is_direct_subclass_of( Password )
    
    
    def on_set_value( self, value: Password ):
        if value is None:
            value = Password( "" )
    ...
    def on_get_value( self ) -> Password:
...
class FallbackEditor( AbstractEditor ):
    ...
    def on_clicked( self, _ ):
        ...
        txt = qt.FrmGenericText.request( parent = window,
                                         ...
                                         text = "",
                                         continuous = True,
                                         details = stringcoercion.get_default_collection().get_descriptive_text(),
...
class StringCoercionEnumEditor( AbstractEnumEditor ):
    ...
    def __get_coercer( cls, info: EditorInfo ) -> Optional[Tuple[stringcoercion.CoercionInfo, AbstractEnumCoercer]]:
        dc = stringcoercion.get_default_collection()
...
class FilenameEditor( AbstractBrowserEditor ):
    ...
    def on_browse( self, value: Filename ) -> str:
        ...
        if self.info.annotation.is_mannotation_of( Dirname ):
            d.setFileMode( qt.QFileDialog.Directory )
        else:
            t: FileNameAnnotation = self.info.annotation.mannotation
    ...
    @classmethod
    def on_can_handle( cls, info: EditorInfo ) -> bool:
        return info.annotation.is_mannotation_of( Filename ) or info.annotation.is_mannotation_of( Dirname )
...
class UnionEditor( AbstractEditor ):
    ...
    @classmethod
    def on_get_priority( cls ) -> int:
        # Anything taking the type directly gets first dibs
        return cls.Priority.LOW
    ...
    def __init__( self, info: EditorInfo ) -> None:
        ...
        self.combo = qt.ComboBoxWrapper( qt.QComboBox(), options = [x for x in info.annotation.union_args if info.editorium.can_handle( x )], namer = lambda x: str( AnnotationInspector( x ) ) )
