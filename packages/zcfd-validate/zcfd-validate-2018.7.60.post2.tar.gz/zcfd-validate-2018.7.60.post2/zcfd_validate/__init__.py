...
def validate(d):
    ...
    for k in ic_keys:
        try:
            ...
            config.logger.error(Fore.RED + "Error in IC configuration for key %s: field %s did not validate (either missing or invalid value)" % (k, e.path) + Fore.RESET)
            raise e
    ...
    for k in bc_keys:
        try:
            ...
            try:
                ...
                config.logger.error(Fore.RED + "Error in BC configuration for key %s: field %s did not validate (either missing or invalid value)" % (k, e.path) + Fore.RESET)
                raise e

    for k in fz_keys:
        if d[k]['type'] in ('rotating', 'translating'):
            config.logger.error(Fore.RED + "Rotating fluid zones are not supported at the moment" + Fore.RESET)
