...
def request_write(filename, bucket_name):	
	'''
	...
	d = {"config":{"encoding":"FLAC","languageCode":"hi-IN","enableWordTimeOffsets":False},"audio":{"uri":"gs://" + bucket_name + "/" + filename}}
	fp = open('sync-request.json','w')
	json.dump(d,fp,indent=4)
	fp.close()
#********************************************************---------------------------------------------------------------*********************************************************

def get_transcript(path_audio_file, bucket_name):
	'''
	...
	if not os.path.exists(path):
		os.makedirs(path)
	d,r = sf.read('mono.flac', frames = rate * 14, start = 0)
	sf.write(os.path.join(path, 'chunk0.flac'), d, rate)
	if(rate * 14 < len(data)):
		de ,r = sf.read('mono.flac', frames = rate * 15, start = rate * 14)
		sf.write(os.path.join(path, 'chunk1.flac'), de, rate)
	j = 2
	...
	for chunk in chunks:
		request_write(chunk, bucket_name)
