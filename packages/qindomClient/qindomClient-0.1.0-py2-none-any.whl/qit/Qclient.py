...
class Qclient:
    ...
    def do_prediction(self, training_data, label, test_data, clf_type, n_estimators=None, random_state=None,
        ...
        r = requests.post('%s/%s' % (SERVICE_URL, self.username), json=request_dict, headers=headers)
        return r.text

    def getMethods(self):
        # r = requests.get('%s/%s' % (SERVICE_URL, 'mlmethods'))
        return CLF_LIST
