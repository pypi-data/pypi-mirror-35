    ...
    def check_commented(self):
        ...
        for line in self.lines:
            ...
            if line.entry.comment_type == 'entry':
                ...
                if host.mac.replace('-', '') and host.hosttype not in ['ip_alias']:
                    if host.mac in self.macs:
                        error = 'Duplicate Commented MAC Address: %s' % (host.mac)
                        line.warnings.append(error)
                    else:
                        self.macs[host.mac] = line.entry
