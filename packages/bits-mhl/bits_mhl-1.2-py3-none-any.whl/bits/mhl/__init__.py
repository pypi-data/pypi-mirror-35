...
from bits.mhl.types import CnameRecord, Comment, Host, MxRecord, NsRecord
...
REGULAR_HOSTS.add(netaddr.IPRange(
    '192.168.141.150', '192.168.141.255'))
...
class MHL(object):
    ...
    def __init__(
        ...
        self.mhlfile = MHLFile(self, path, lockfile, verbose=self.verbose)

class MHLFile(object):
    """MHL class."""

    def __init__(self, mhl, path, lockfile, verbose=False):
        """Initialize the object."""
        self.mhl = mhl
        self.path = path
        self.lockfile = lockfile
    ...
    def check_commented(self):
        ...
        for line in self.lines:
            if line.entry and line.entry.ip:
                ...
                if not last_ip:
                    last_ip = line.entry.ip
                else:
                    if netaddr.IPAddress(last_ip) > netaddr.IPAddress(line.entry.ip):
                        error = 'IP Address out of order: %s' % (line.entry.ip)
                        line.warnings.append(error)
                    last_ip = line.entry.ip
            ...
            # check the comment entry
            line.entry.check()

            # check for full lines
            if line.entry.comment_type == 'entry':
                # check line as if it was uncommented
                newline = MHLLine(line.line.lstrip('#'), line.linenum)
                host = Host(newline)
                host.check()

                # check for duplicate hostnames
                if host.hostname in self.hostnames:
                    error = 'Duplicate Commented Hostname: %s' % (host.hostname)
                    line.warnings.append(error)

                # check for duplicate cnames
                for cname in host.cnames:
                    if cname in self.hostnames:
                        error = 'Duplicate Commented Hostname [cname]: %s' % (cname)
                        line.warnings.append(error)

            if line.entry.comment_type in ['available_ip', 'entry', 'reserved_ip']:
                # check for duplicate IP address
                if line.entry.ip in self.ips:
                    error = 'Duplicate Commented IP Address: %s' % (line.entry.ip)
                    line.warnings.append(error)
                else:
                    self.ips[line.entry.ip] = line.entry
    ...
    def register_cname_record(self, line):
        """Add an entry to the internal cnames dictionary"""
        error = []
        hostname = line.entry.hostname

        if hostname not in self.records:
            self.records[hostname] = line.entry
        else:
            error.append('Duplicate Record [CNAME]: %s' % (hostname))

        if hostname not in self.cnames:
            self.cnames[hostname] = line.entry
        else:
            error.append('Duplicate CNAME: %s' % (hostname))

        return error

    def register_mx_record(self, line):
        """Add an entry to the internal mxs dictionary"""
        error = None
        hostname = line.entry.hostname
        ...
        if hostname not in self.records:
            ...
            error.append('Duplicate Record [MX]: %s' % (hostname))

        if hostname not in self.mxs:
            self.mxs[hostname] = line.entry
        else:
            self.mxs[hostname].targets += line.entry.targets

        return error

    def register_ns_record(self, line):
        """Add an entry to the internal nss dictionary"""
        error = None
        hostname = line.entry.hostname
        ...
        if hostname not in self.records:
            ...
            error.append('Duplicate Record [NS]: %s' % (hostname))

        if hostname not in self.nss:
            self.nss[hostname] = line.entry
        else:
            self.nss[hostname].targets += line.entry.targets

        return error

    def register_host_record(self, line):
        """Add an entry to the internal hosts dictionary"""
        error = None
        hostname = line.entry.hostname
        ...
        if hostname not in self.records:
            ...
            error.append('Duplicate Record [Host]: %s' % (hostname))

        if hostname not in self.hosts:
            self.hosts[hostname] = line.entry
        else:
            error.append('Duplicate Host: %s' % (hostname))
    ...
    def get_records(self):
        ...
        for line in self.lines:
            ...
            # Construct the method name using entry.type
            reg_record = getattr(self, 'register_%s_record' % line.entry.type)
            # Call the method
            err = reg_record(line)

            if err:
                # Use extend instead of append because err will always be
                # returned as a list of errors
                line.errors.extend(err)
