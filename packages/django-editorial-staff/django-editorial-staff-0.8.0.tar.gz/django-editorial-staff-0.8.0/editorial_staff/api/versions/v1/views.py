...
class StaffFetch(View):
    def get(self, request, email):
        ...
        """
        staffer = Staffer.objects.filter(email__iexact=email).first()
        if staffer:
...
class StaffRescrape(View):
    def get(self, request, email):
        if DATA_PROVIDER is not None:
            staffer_raw = DATA_PROVIDER.get_staffer(
                email,
                excluded_email_domains=PROVIDER_OPTIONS.get(
        ...
        return JsonResponse({
            ...
            'msg': ' '.join([
                ...
                '`settings.EDITORIAL_STAFF_DATA_PROVIDER`.'
            ])
        })
