...
class microservicio(db):
    ...
    def __functionBifurcacion(self,configService,jsonArguments):
        ...
        try:
            loop.create_task(self.log("[ "+ID_TRANSACCION + " ] - input - ["+metadata["owner"]+"] \nHeaders : " + str(headers) +"\nData : "+ str(data) +"\nResponse : "+ str(response)+ "\n" ))
            ...
            if "grabar_metadata" in cursor_conf:
                ...
                if success == True:
                    loop.create_task(self.log("[ "+ID_TRANSACCION + " ] - "+metadata["owner"] +" SUCCESS"+ "\n"))
                    if cursor_conf["end"] == False:
                        loop.create_task(self.log("[ "+ID_TRANSACCION + " ] - "+metadata["owner"] +" NO ES SERVICIO FINAL"+ "\n"))
                        uWorker_async = metadata["uworker"]
                        string = str(time.time()).replace('.', '')
                        jsonArguments["metadata"]["id_operacion"] = int(string)
                        jsonArguments["metadata"]["uowner"] = uWorker_async
                        ...
                        loop.create_task(self.log("[ "+ID_TRANSACCION + " ] - "+metadata["owner"] +" ES SERVICIO FINAL"+ "\n"))
                        JSON_RESPUESTA_FIN = {}
                    ...
                    loop.create_task(self.log("[ "+ID_TRANSACCION + " ] - "+metadata["owner"] +" ERROR"+ "\n"))
