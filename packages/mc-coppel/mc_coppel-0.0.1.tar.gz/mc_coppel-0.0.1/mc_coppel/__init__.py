...
class microservicio(db):
    ...
    def __functionBifurcacion(self,configService,jsonArguments):
        ...
        try:
            ...
            if "grabar_metadata" in cursor_conf:
                ...
                if success == True:
                    print("ANTERIOR SUCCESS")
                    print(cursor_conf["end"])
                    if cursor_conf["end"] == False:
                        print("NO ES SERVICIO FINAL")
                        print(metadata["uworker"])
                        uWorker_async = metadata["uworker"]
                        ...
                        print(self.__inicializaciones["configuraciones"])
                        print(data_inicial)
                        print(jsonArguments["response"])
            ...
            self.__escribirColaMensajeria("Errores_criticos",error)
    ...
    async def escribir(self,topico,respuesta):
        msj = self.__json_to_b64(respuesta)
        self.__producer.send(topico,key=str.encode(str(self.__id_transaction)),value=msj)
        self.__producer.flush()
        print("Escribe en topico ["+topico+"]"+" : \n"+str(respuesta))

    def __escribirColaMensajeria(self,topico,respuesta):
        #loop.create_task(self.send_one(topico,respuesta))
        loop.create_task(self.escribir(topico,respuesta))
    ...
    async def __functionBridge(self,function,*args):
        ...
        if "smoketest" in args[1]["data"]:
            ...
            if(metadata["bifurcacion"] == True):                                                                                                                                                                                                                                        
                ...
                self.__escribirColaMensajeria(TOPICO,respuestax)
            else:
                ...
                self.__escribirColaMensajeria(TOPICO,respuesta2)
        else:                
            if self.__esWorker == True:
                ...
                self.__id_transaction = args[1]["metadata"]["id_transaction"]
                ...
                self.__escribirColaMensajeria(TOPICO,respuesta)
        ...
        try:
            async for msg in self.__consumer2:
                try:
                    ...
                    if msg.value.decode('utf-8') ==  "smoketest":
                        msj = str.encode("smoketest ok")
                        self.__producer.send(self.__inicializaciones["topicName"],value=msj)
                        self.__producer.flush()
                        pass
                    else:
                        #print("fallo al decodificar b64")
                        pass
        finally:
    ...
    def __streamMessageQueue(self,function):
        ...
        configService = {
            "configGral":{"timeout_internos":self.__inicializaciones["configuraciones"]["timeout_internos"],"mongoUri" : self.__inicializaciones["configuraciones"]["mongo_uri"]},
