...
class DynamoController(object):
    ...
    def __init__(self, table_name, filter_app=True, debug=False):
    ...
    def get(self, primary_keys, request=None, return_raw=False):
        ...
        """
        if self.filter_app and not request:
    ...
    def filter(self, key, value, order_by=None, limit=50, next=None, index=None, order_type=ORDER_DESC, filter_expression=None, request=None, return_raw=False):
        ...
        if self.filter_app == True:
            if not request:
                raise Exception("A Active HTTP request is required for filtering by app_id")
            ...
            if type == self.TYPE_GET:
                ...
                if self.filter_app:
                    if not "app_id" in data:
                        raise ExceptionInvalidDynamoControllerNotFound("Not found for this app_id")
                    app_id = self.controller_app.get_app_id(request=request)
