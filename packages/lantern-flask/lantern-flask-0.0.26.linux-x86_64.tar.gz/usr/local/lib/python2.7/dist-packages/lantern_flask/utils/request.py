...
def http_response(code, message, data, count=None, next=None, response=None, cleanup_system_cols=True):
    ...
    data_json = {
        "status": code,
        "message": message,
        "results": cleanup_system_cols(data) if cleanup_system_cols else data,
    }
    if count:
