...
class DeIdField(object):
    ...
    @staticmethod
    def _hash(profile, value, output_format='hex'):
        """Hash a value according to profile rules"""
        return util.hash_value(value, algorithm=profile.hash_algorithm, salt=profile.hash_salt, output_format=output_format)
    ...
    def get_value(self, profile, state, record):
        ...
        if original:
            ...
            if profile.uid_suffix_fields > 0:
                result_parts += orig_parts[-profile.uid_suffix_fields:]
