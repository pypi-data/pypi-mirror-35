...
class _RawPFile(object):
    ...
    def __init__(self, filepath, encoding='ascii'):
        ...
        attrs, self.offsets = self.parse(filepath)

        self.exam_no = self.decode(attrs['exam_no'], encoding)
        self.exam_uid = self.decode(attrs['exam_uid'], encoding)
        self.patient_id = self.decode(attrs['patient_id'], encoding)
        self.accession_no = self.decode(attrs['accession_no'], encoding)
        self.series_no = self.decode(attrs['series_no'], encoding)
        self.series_desc = self.decode(attrs['series_desc'], encoding)
        self.series_uid = self.decode(attrs['series_uid'], encoding)
        self.acq_no = self.decode(attrs['acq_no'], encoding)
