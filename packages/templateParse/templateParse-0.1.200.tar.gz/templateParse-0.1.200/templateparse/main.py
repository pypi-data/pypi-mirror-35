...
def extract_data(invoicefile, templates=None, input_module=pdftotext):
    ...
    Notes
    -----
    Import required `input_module` when using templateparse as a library
    ...
    Examples
    --------
    When using `templateparse` as an library

    >>> from templateparse.input import pdftotext
    >>> extract_data("templateparse/test/pdfs/oyo.pdf", None, pdftotext)
