...
@task(name='push')
def push_twine(ctx, username, password):
    """Upload build files to pypi or devpi archive."""
    dist_dir = Path('src/{}/dist'.format(ctx.app.pkg_name))
    options = ('--username {}'.format(username),
               '--password {}'.format(password),
               )

    cmd = 'twine upload {} {}/*'.format(' '.join(options), dist_dir)
    print(cmd)
    # ctx.run(cmd)
...
def test(ctx, errors=False, unit=False, integration=False, regression=False,  # pylint: disable=too-many-arguments
    ...
    pytest_cmd = 'pytest src/{}/tests/{}'

    if unit:
        cmd = pytest_cmd.format(ctx.app.pkg_name, 'unit')
        ctx.run(cmd)

    if integration:
        cmd = pytest_cmd.format(ctx.app.pkg_name, 'integration')
        ctx.run(cmd)

    if regression:
        cmd = pytest_cmd.format(ctx.app.pkg_name, 'regression')
        ctx.run(cmd)
