...
class Client:
    ...
    def put_playback_configuration(
        ...
        *,
        ad_decision_server_url: str = autoboto.ShapeBase._NOT_SET,
        cdn_configuration: shapes.CdnConfiguration = autoboto.ShapeBase.
        _NOT_SET,
        name: str = autoboto.ShapeBase._NOT_SET,
        slate_ad_url: str = autoboto.ShapeBase._NOT_SET,
        video_content_source_url: str = autoboto.ShapeBase._NOT_SET,
        ...
        if _request is None:
            _params = {}
            if ad_decision_server_url is not autoboto.ShapeBase._NOT_SET:
                _params['ad_decision_server_url'] = ad_decision_server_url
            if cdn_configuration is not autoboto.ShapeBase._NOT_SET:
                _params['cdn_configuration'] = cdn_configuration
            if name is not autoboto.ShapeBase._NOT_SET:
                _params['name'] = name
            if slate_ad_url is not autoboto.ShapeBase._NOT_SET:
                _params['slate_ad_url'] = slate_ad_url
            if video_content_source_url is not autoboto.ShapeBase._NOT_SET:
