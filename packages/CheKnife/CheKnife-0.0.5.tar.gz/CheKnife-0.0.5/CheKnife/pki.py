...
class ServerCertificate(Certificate):
    def __init__(self, dn, ca=None, key_size=4096, days=3650, working_dir='.', **kwargs):
        self.ca = ca
        self.dn = dn
        self.key_size = str(key_size)
        self.days = str(days)
        ...
        self.kwargs = kwargs
        self.key_path = os.path.join(working_dir, 'private', dn.file_friendly_cn + self.key_ext)
        self.csr_path = os.path.join(working_dir, 'csr', dn.file_friendly_cn + self.csr_ext)
        self.crt_path = os.path.join(working_dir, 'certs', dn.file_friendly_cn + self.crt_ext)
        self.srl_path = os.path.join(working_dir, 'srl', dn.file_friendly_cn + self.srl_ext)
        self.create_dirs()
