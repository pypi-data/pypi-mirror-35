...
def resolve_consequences(consequence_string):
    ...
    for entry in vcf_reader:
        ...
        for transcript in entry.INFO['CSQ']:
            ...
            for key, value in zip(csq_format, transcript.split('|')):
                transcript_info[key] = value
            if len(set(['upstream_gene_variant', 'synonymous_variant', 'intron_variant', 'stop_gained']) & set(resolve_consequences(transcript_info['Consequence']))) > 0:
                continue
            if transcript_info['Feature'] != '' and not transcript_info['Feature'].startswith('ENSR'):
                transcript_ids.add(transcript_info['Feature'])
            if args.format == 'kallisto':
                if transcript_info['SYMBOL'] != '':
                    genes.add(transcript_info['SYMBOL'])
            else:
                if transcript_info['Gene'] != '':
                    genes.add(transcript_info['Gene'])
