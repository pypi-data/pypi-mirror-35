...
class SubVarSet(object):
	...
	def substitute_all(self, any_text):
		if isinstance(any_text, stringtypes):
			subbed = True
			while subbed:
	...
	def select_rowsource(self, sql):
		...
		def decode_row():
			...
			if row:
				if self.encoding:
					if sys.version_info < (3,):
						return [c.decode(self.encoding) if isinstance(c, stringtypes) else c for c in row]
					else:
						return [c.decode(self.encoding) if type(c) == type(b'') else c for c in row]
				else:
					return row
			else:
				return None
...
def get_password(dbms_name, database_name, user_name, server_name=None, other_msg=None):
	...
	if script_name is None:
		script_name = ""
	prompt = u"The execsql script %s wants the %s password for" % (script_name, dbms_name)
