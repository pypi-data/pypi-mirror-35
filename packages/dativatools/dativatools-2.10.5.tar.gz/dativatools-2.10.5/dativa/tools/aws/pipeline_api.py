...
class PipelineClient:
    ...
    def __init__(self,
        ...
        self.dest_bucket = self.data['destination']['s3_url'].split("/")[3]

    @classmethod
    def _force_s3_policy(cls, bucket):
        s3_client = boto3.client("s3")
        policy = deepcopy(cls.policy)
    ...
    def run_job(self):
        ...
        """
        self._force_s3_policy(self.source_bucket)
        self._force_s3_policy(self.dest_bucket)
    ...
    def run_job_with_callback(self, callback, response=None, call_no=0, *args):
        '''
        ...
        if self.check_status(response).json()['status'] in ('ERROR', 'COMPLETED'):
            ...
            threading.Timer(3, self.run_job_with_callback,
                            [callback, response, call_no] + list(args)).start()
        return response
    ...
    def set_s3_bucket_policy(self, policy, policy_dest=None):
        ...
        if self.source_bucket != self.dest_bucket:
