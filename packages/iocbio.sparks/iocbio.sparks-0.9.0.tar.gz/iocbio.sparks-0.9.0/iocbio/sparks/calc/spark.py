...
Results = {
    ...
    "amplitude_f0": RDesc("Amplitude/F0", "", "amplitude/f0"),
    "amplitude_blurred": RDesc("Amplitude blurred", "AU", None),
    "amplitude_blurred_f0": RDesc("Amplitude blurred/F0", "", "amplitude_blurred/f0"),
    "f0": RDesc("F0", "AU", None),
    "time_spline_amplitude": RDesc("Time: spline amplitude", "AU", None),
    "time_spline_amplitude_f0": RDesc("Time: spline amplitude/F0", "", "time_spline_amplitude/f0"),
    ...
    "space_spline_amplitude_f0": RDesc("Space: spline amplitude/F0", "", "space_spline_amplitude/f0"),
...
class SparkAnalyzer(worker.Job):
    ...
    def run_job(self):
        ...
        if waterlabel.max() < 0.5: # no labels left
            ...
            self.signals.sigStats.emit(self.id,  "Spark not found", 0)
            return
        ...
        self.signals.sigStats.emit(self.id, "amplitude_f0", self.corrected_mean[self.location[0], self.location[1]] / self.F0)
        ...
        self.signals.sigStats.emit(self.id, "amplitude_blurred_f0",
                                   self.corrected_mean_medblu[self.location[0], self.location[1]] / self.F0)
        ...
        self.signals.sigStats.emit(self.id, "time_spline_amplitude_f0", self.spline_time.max() / self.F0)
        ...
        self.signals.sigStats.emit(self.id, "space_spline_amplitude_f0", self.spline_space.max() / self.F0)
        ...
        if create_view and not db.has_view(Spark.database_view):
            ...
            for k in keys:
                if Results[k].view is not None:
                    math += Results[k].view + " AS " + k + ","
            math = math[:-1]
            db.query("CREATE VIEW " + db.table(Spark.database_view) + " AS SELECT *, " +
                     math + " FROM " + db.table(Spark.database_table))
