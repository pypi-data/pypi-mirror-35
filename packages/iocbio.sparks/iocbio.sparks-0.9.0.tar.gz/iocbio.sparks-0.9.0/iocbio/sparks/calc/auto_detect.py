...
def get_sparks(blurred, std, accept_as_possible_peak, accept_as_possible_area, min_area, mean=0,
               get_rois=False,
               largest_only=False):
    ...
    maxima = ndimage.measurements.maximum(blurred, label, range(numlabels+1))
...
class AutoDetect(worker.Job):
    ...
    def __init__(self, image, std, accept_as_possible_peak, accept_as_possible_area, min_area):
    ...
    def run_job(self):
        filtered, regions = get_sparks(self.image, self.std,
                              ...
                              min_area=self.min_area, get_rois=True)
