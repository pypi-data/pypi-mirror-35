"""
 * $Id: 0.6+16 utils.py 907fe5427293 2018-08-31 od $
 *
...
class Client(object):
    ...
    def __init__(self, data: Dict = None, metainfo: Dict = None, 
        ...
        if metainfo is not None:
            self.url = url or metainfo.get(self.META_KEY_SERVICE_URL, None)
            # this is for existing catalogs deployed services, to be removed later
            if self.url is None:
                self.url = metainfo.get("path", None)
        else:
            self.url = url
    ...
    def get_catalog(url:str) -> List['Client']:
        ...
        for service in response_json:
            s.append(Client(metainfo=service))
        return s
    ...
    def execute(self, url: str = None, sync:bool = True, files:List[str]=[]) -> 'Client':
        ...
        for i, file in enumerate(files, start=1):
            ...
            if os.path.isdir(file):
                att = make_archive(file, 'zip', file)
            else:
                att = file 
            multipart[self.REQ_FILE + str(i)] = open(att, 'rb')
    ...
    def execute_async(self, url: str = None, files:List[str]=[], first_poll:int = 2, next_poll:int = 2,
        ...
        """Executes a service as HTTP/POST. This method runs the service asynchronously and
           ...
           :first_poll: time in sec to poll the first time
           :next_poll: time in sec to poll from then on
        """
        def cb(c):
            pass
    ...
    def get_entries_asstr(self) -> str:
        ...
        for i in self.get_data_names():
            s = s + self.get_entry_asstr(i)
        return s
            
    def __str__(self):
        """String representation."""
        return ('SERVICE: {name} ({suid})\n url: {url}\n parent: {parentsuid} \n metainfo: {metainfo}\n data:\n{data}'.format(
               name = self.get_name(), suid=self.get_suid(),  url=self.url, 
