...
from protocol import SafeBrowsingApiClient, URL
from storage import SqliteStorage, ThreatList, HashPrefixList
...
class SafeBrowsingList(object):
    ...
    def __init__(self, api_key, db_path='/SDUrl.db', discard_fair_use_policy=False,
                        platforms = None):
        """Constructor.
    ...
    def _sync_hash_prefix_cache(self):
        ...
        for response in self.api_client.get_threats_update(client_state):
            ...
            if self._verify_threat_list_checksum(response_threat_list, expected_checksum):
                OutputDebugStringA('Local cache checksum matches the server: {}'.format(to_hex(expected_checksum)))
    ...
    def _lookup_hashes(self, full_hashes):
        ...
        if len(matching_expired_threat_lists) == 0 and sum(map(int, matching_prefixes.values())) == 0:
            OutputDebugStringA('Negative cache hit.')
            return []
