...
class ThreatList(object):
    ...
    """
    def __init__(self, threat_type, platform_type, threat_entry_type, malware_threat_type='null'):
        self.threat_type = threat_type
        self.platform_type = platform_type
        self.threat_entry_type = threat_entry_type
        self.malware_threat_type = malware_threat_type
    ...
    def as_tuple(self):
        return (self.threat_type, self.platform_type, self.threat_entry_type, self.malware_threat_type)
...
class SqliteStorage(object):
    ...
    def __init__(self, db_path):
        ...
        OutputDebugStringA('Opening SQLite DB {}'.format(db_path))
        self.db = sqlite3.connect(db_path)
        if do_init_db:
            OutputDebugStringA('SQLite DB does not exist, initializing')
            self.init_db()
        if not self.check_schema_version():
            OutputDebugStringA("Cache schema is not compatible with this library version. Re-creating sqlite DB {}".format(db_path))
            self.db.close()
            os.unlink(db_path)
            self.db = sqlite3.connect(db_path)
            self.init_db()
    ...
    def check_schema_version(self):
        ...
        with self.get_cursor() as dbc:
            try:
                ...
                OutputDebugStringA('Can not get schema version, it is probably outdated.')
                return False
    ...
    def lookup_full_hashes(self, hash_values):
        ...
        q = '''SELECT threat_type,platform_type,threat_entry_type,malware_threat_type, expires_at < current_timestamp AS has_expired
        ...
        with self.get_cursor() as dbc:
            ...
            for h in dbc.fetchall():
                threat_type, platform_type, threat_entry_type, malware_threat_type, has_expired = h
                threat_list = ThreatList(threat_type, platform_type, threat_entry_type, malware_threat_type)
    ...
    def store_full_hash(self, threat_list, hash_value, cache_duration, malware_threat_type):
        ...
        OutputDebugStringA('Storing full hash {} to list {} with cache duration {}'.format(to_hex(hash_value), str(threat_list), cache_duration))
    ...
    def cleanup_full_hashes(self, keep_expired_for=60*60*12):
        ...
        '''
        OutputDebugStringA('Cleaning up full_hash entries expired more than {} seconds ago.'.format(keep_expired_for))
        with self.get_cursor() as dbc:
    ...
    def delete_threat_list(self, threat_list):
        ...
        """
        OutputDebugStringA('Deleting cached threat list "{}"'.format(repr(threat_list)))
    ...
    def update_threat_list_client_state(self, client_state):
        OutputDebugStringA('Setting client_state in Sqlite')
    ...
    def populate_hash_prefix_list(self, threat_list, hash_prefix_list):
        OutputDebugStringA('Storing {} entries of hash prefix list {}'.format(len(hash_prefix_list), str(threat_list)))
    ...
    def get_hash_prefix_values_to_remove(self, threat_list, indices):
        OutputDebugStringA('Removing {} records from threat list "{}"'.format(len(indices), str(threat_list)))
        indices = set(indices)
        q = '''SELECT value FROM hash_prefix
                WHERE threat_type=? AND platform_type=? AND threat_entry_type=?
                ORDER BY value
    ...
    def rollback(self):
        OutputDebugStringA('Rolling back DB transaction.')
        self.db.rollback()
