...
class SearchQuery:
  ...
  def parse(self, arglist):
    ...
    if "-closed" in arglist:
      if self.listopts is not None and self.listopts.source == "all":
        self.listopts.source = "closed"
      else:
        self.closed = True
      arglist.remove("-closed")
    if "-sent" in arglist:
      if self.listopts is not None and self.listopts.source == "all":
        self.listopts.source = "sent"
      else:
        self.sent = True
      arglist.remove("-sent")
  ...
  def do_compile(self):
    ...
    if len(self.tagset_list) > 0 or len(self.not_tagset_list) > 0:
      slist.append("  data_tagset = tags.get_tagset_from_str(data[mailfile.TAGS_L])\n")
    ...
    if len(self.subjectlist_list) > 0:
      slist.append("  if not any([all([s in data[mailfile.SUBJECT_L] for s in subjectlist]) for subjectlist in self.subjectlist_list]):\n")
      slist.append("    continue\n")
  ...
  # convert relative dates and time zones to absolute
  def interpret_dates(self):
    ...
    if self.orig_after_str is not None:
      if self.orig_after_str.startswith("p") or self.orig_after_str.startswith("m"):
        self.after_str = datestr.get_absstr_from_datestr(self.orig_after_str)
        if self.listopts is not None:
          self.listopts.after_str = self.after_str[:10]
        if not any([s in self.orig_after_str for s in ["H","M","."]]):
          self.after_str = None  # let listmail take care of it
      else:
        if self.listopts is not None:
          self.listopts.after_str = self.orig_after_str[:10]
        if len(self.orig_after_str) > 10:
          self.after_str = self.orig_after_str

    if self.orig_before_str is not None:
      if self.orig_before_str.startswith("p") or self.orig_before_str.startswith("m"):
        self.before_str = datestr.get_absstr_from_datestr(self.orig_before_str)
        if self.listopts is not None:
          self.listopts.before_str = self.before_str[:10]
        if not any([s in self.orig_before_str for s in ["H","M","."]]):
          self.before_str = None  # let listmail take care of it
      else:
        if self.listopts is not None:
          self.listopts.before_str = self.orig_before_str[:10]
        if len(self.orig_before_str) > 10:
          self.before_str = self.orig_before_str
  ...
  def filter(self, mgr, filelist):
    ...
    namespace ={"self": self, "addr": addr, "datestr_matches": datestr.datestr_matches, "mailfile": mailfile, "mgr": mgr, "tags": tags, "filelist": filelist}
