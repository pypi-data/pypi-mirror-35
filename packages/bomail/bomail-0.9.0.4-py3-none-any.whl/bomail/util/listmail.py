...
def list_open(after, before, mgr):
  ...
  with open(config.openlist_file) as f:
    for line in f:
      ...
      if datestr.datestr_matches(datestr.get_date_from_filename(filename), after, before):
...
def list_scheduled(after, before, mgr):
  ...
  with open(config.scheduledlist_file) as f:
    for line in f:
      ...
      if filename[-1] == "\n":
        filename = filename[:-1]
      ds = datestr.get_date_from_filename(filename)
      if datestr.datestr_matches(ds, after, before):
        output.append(filename)
  return output

def list_drafts(after, before, mgr):
...
class ListOpts:
  ...
  def listmail(self, mgr):
    if self.source == "all":
      ...
      return list_closed(self.after_str, self.before_str, mgr)
