...
def generate(howMany):
    alist=[]
    import random as r
    while howMany != 0:
        while True:
            aadharNum = str(r.randint(1111,9999)) + str(r.randint(0000,9999)) + str(r.randint(0000,9999))
            if validate(aadharNum) == True:
                #print(aadharNum)
                alist.append(aadharNum)
                break
        howMany -= 1
    return alist
