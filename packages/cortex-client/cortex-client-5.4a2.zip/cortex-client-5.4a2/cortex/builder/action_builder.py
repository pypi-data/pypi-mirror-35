...
class ActionBuilder:
    ...
    def _build_daemon(self, dry_run=False):
        if self._code_s:
            ...
            # Deploy existing docker image
            if not dry_run:
                self._client.deploy_action(self._name, self._kind, self._get_docker_repository(), action_type=_action_daemon)
    ...
    def _build_job(self, dry_run=False):
        if self._code_s:
            ...
            # Deploy existing docker image
            if not dry_run:
                self._client.deploy_action(self._name, self._kind, self._get_docker_repository(), action_type=_action_job)
            ...
            with Timer() as t:
                ...
                for obj in json_stream(stream):
                    if 'stream' in obj:
                        ...
                        match = re.match(r'^Step\s+(\d)/(\d+)', line)
                        if match:
                            log_message(obj['stream'].strip(), log, logging.INFO)
                    elif 'error' in obj:
    ...
    def _deploy_daemon_image(self, source, func_name, global_code):
        ...
        try:
            ...
            self._client.deploy_action(self._name, self._kind, self._get_docker_repository(use_prefix=True), action_type=_action_daemon,
    ...
    def _deploy_job_image(self, source, func_name, global_code):
        ...
        try:
            ...
            self._client.deploy_action(self._name, self._kind, self._get_docker_repository(use_prefix=True), action_type=_action_job)
        finally:
            # cleanup temp
            shutil.rmtree(temp_dir, ignore_errors=True)
    ...
    def _deploy_model(self, dry_run=False):
        ...
        if self._action_type == _action_daemon:
            if dry_run:
                source = self._render_template(_daemon_main, action_globals=_globals, action_code=_predict_source, func_name='predict')
            ...
            if dry_run:
                source = self._render_template(_job_main, action_globals=_globals, action_code=_predict_source, func_name='predict')
                self._dump_source(source)
                return
            ...
            if dry_run:
                source = self._render_template(_function_main, action_globals=_globals, action_code=_predict_source, func_name='predict')
                self._dump_source(source)
                return
