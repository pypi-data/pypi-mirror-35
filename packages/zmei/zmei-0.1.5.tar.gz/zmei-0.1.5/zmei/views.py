...
class ZmeiReactViewMixin(ZmeiDataViewMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        for cmp in self.react_components:
            try:
                data[f'react_page_{cmp}'] = self.react_server.evaljs(f"R.renderServer(R.{cmp}, {data['react_state']});")
            except MiniRacerBaseException as e:
                data[f'react_page_{cmp}'] = f'Server rendering failed. See browser console for details. \n\n\n\n{e}'
