...
class DataHandler(object):
    def __init__(self):
        self._protocol_parse = protocol_handler.ProtocolHandler()
    ...
    def parse_data(self,ring_buffer):
        ...
        package = None
        try:
            self._protocol_parse.del_wrong_header(ring_buffer)
            package = self._protocol_parse.get_one_complete_package(ring_buffer)
            ...
            if ring_buffer.over_max_time():
                self._protocol_parse.pop_header(ring_buffer)
                self._protocol_parse.del_wrong_header(ring_buffer)
        ...
        parsed_data = self._protocol_parse.parse_data(package)
