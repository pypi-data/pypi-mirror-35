...
class Pod():
    ...
    def uuid(self):
        values = self.read('uuid')
        uuid = (values[0] << 64) | values[1]
        return uuid
    
    def set_uuid(self, uuid):
        values = [0, 0]
        values[0] = 0x0000000000000000FFFFFFFFFFFFFFFF & (uuid >> 64)
        values[1] = 0x0000000000000000FFFFFFFFFFFFFFFF & uuid
        if (self.write('uuid', values[0], values[1])):
            self.modbus_save_flash()
