...
class OAuthBase(Auth):
    ...
    def set_cookie(self, response, name, value, max_age,
        ...
        url_params = flask.request.args or {}
        orca_token = url_params.get('orca_access_token')
        if orca_token and orca_token == os.getenv('DASH_ORCA_ACCESS_TOKEN'):
            print('ORCA detected')
            secure = False

        print('REQUEST ENVIRON:')
        pprint.pprint(flask.request.environ, indent=4)
