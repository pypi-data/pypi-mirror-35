...
def save_to_dynamo(table_name, item: dict, dynamodb_helper: DynamoHelperBase = EnvironmentControlledDynamoHelper(),
    ...
    item[id_type] = str(item[id_type])
    if 'namespace' in kwargs:
        item[id_type] = kwargs['namespace'] + '-' + item[id_type]
    ...
    if not response.get('Item') or kwargs.get('overwrite_existing'):
        ...
        if response.get('Item'):
            print('overwrite existing dynamo table: %s id_type: %s' % (table_name, item[id_type]))

        response = table.put_item(Item=dict_to_dynamo(item))

        print('Saved to dynamo table: %s id_type: %s' % (table_name, item[id_type]))
    else:
        print('Skipping, this item already exists in dynamo table: %s id_type: %s' % (table_name, item[id_type]))
