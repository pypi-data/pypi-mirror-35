...
from connections.command import Command
from connections.data import Data
from patchers import fork_handler
from patchers.monkey import Patcher
...
class Agent:
    ...
    def __init__(self, acfg, icfg, comm_q, data_q):
        ...
        self.instid = os.getpid()
        self.icfg_mtime = icfg.pop()
        self.comm_q = comm_q
        self.data_q = data_q

        config.configure_logger(self.log, str(self.instid))
    ...
    def start_agent(self):
        config.agent_config_monitor(self)
        config.instrument_config_monitor()
