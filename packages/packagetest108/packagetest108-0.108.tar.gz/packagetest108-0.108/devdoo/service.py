...
class Service:
    ...
    '''
    def __init__(self, service_id, status, has_connection_backend=False):
    ...
    def __config_brokers(self, service_port):
        if self.brokers and self.network:
            self.brokers = Brokers(service_port, self.brokers, self.status, self.has_connection_backend)
        else:
