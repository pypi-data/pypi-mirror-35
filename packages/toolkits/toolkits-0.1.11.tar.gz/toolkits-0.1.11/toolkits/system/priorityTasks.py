...
from config_flow_mis import redis_info
...
class PriorityTasks:
    ...
    '''
    def __init__(self, queue_key=None, work_status_queue_name="txt_redis_working_flag"):
        ...
        self.redisCli = redis.StrictRedis(host=redis_info['host'], port=redis_info['port'], password=redis_info['password'], db=1)
    ...
    def send_task(self, priority, task_params):
    ...
    def worker(self, task_func):
        ...
        if work_flag == "on":
            work_status = True
            logger.debug("InitWork: Status is ON!!! ")
        else:
            work_status = False
            logger.debug("InitWork: Status is OFF!!!  Nothing to do....")
