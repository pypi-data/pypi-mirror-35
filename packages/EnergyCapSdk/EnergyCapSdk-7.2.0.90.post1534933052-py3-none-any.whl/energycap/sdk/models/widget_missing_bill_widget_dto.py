# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class WidgetMissingBillWidgetDTO(Model):
    """WidgetMissingBillWidgetDTO.

    :param gap_begin_date: The begin date of the gap in billing data
    :type gap_begin_date: datetime
    :param gap_end_date: The end date of the gap in billing data
    :type gap_end_date: datetime
    :param gap_days: The number of days in the gap in billing data
    :type gap_days: int
    :param account: The account that is missing billing data
    :type account: ~energycap.sdk.models.AccountsAccountChildDTO
    :param meter: The meter that is missing billing data
    :type meter: ~energycap.sdk.models.LogicalDeviceMeterChildDTO
    """

    _attribute_map = {
        'gap_begin_date': {'key': 'gapBeginDate', 'type': 'iso-8601'},
        'gap_end_date': {'key': 'gapEndDate', 'type': 'iso-8601'},
        'gap_days': {'key': 'gapDays', 'type': 'int'},
        'account': {'key': 'account', 'type': 'AccountsAccountChildDTO'},
        'meter': {'key': 'meter', 'type': 'LogicalDeviceMeterChildDTO'},
    }

    def __init__(self, gap_begin_date=None, gap_end_date=None, gap_days=None, account=None, meter=None):
        super(WidgetMissingBillWidgetDTO, self).__init__()
        self.gap_begin_date = gap_begin_date
        self.gap_end_date = gap_end_date
        self.gap_days = gap_days
        self.account = account
        self.meter = meter
