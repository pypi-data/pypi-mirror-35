...
class TestHttpClient(unittest.TestCase):
    ...
    def test_http_get(self):
        self.log.info('test_http_get')
        async def http_get():
            http_get = self.http_client.get(self.url)
            response = await http_get
            return response.data

        data = self.loop.run_until_complete(http_get())
        self.assertEqual(data, 'Welcome to tomato')

    def test_http_post(self):
        self.log.info('test_http_post')
        async def http_post():
            http_post = self.http_client.post(self.url)
            response = await http_post
            return response.data

        data = self.loop.run_until_complete(http_post())
        self.assertEqual(data, 'Welcome to tomato')
