...
class CompilerClangBase(BaseCompiler):
    ...
    clang_libcxx_env_variable = None
    ...
    def environment_filters(cls):
        ...
        clang_libcxx = get_env(cls.clang_libcxx_env_variable, [])
        r = {}
        if len(clang_versions):
            r['version'] = [(cls.id, v) for v in clang_versions]
        if len(clang_libcxx):
            r['libcxx'] = clang_libcxx
        return r
...
class CompilerClangLinux(CompilerClangBase):
    ...
    clang_libcxx_env_variable = "CONAN_CLANG_LIBCXXS"
...
class CompilerClangApple(CompilerClangBase):
    ...
    clang_libcxx_env_variable = "CONAN_APPLE_CLANG_LIBCXXS"
