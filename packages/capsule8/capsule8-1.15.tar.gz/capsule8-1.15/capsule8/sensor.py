...
class SensorClient(object):
    ...
    def telemetry(self, max_num_events=None, timeout=None, connection_timeout=2, asynchronous=False, loop=None, ignore_status=False):
        """Return an iterator yielding telemetry events.

        Note:
            Iterate through telemetry from the capsule8 sensor via gRPC

        Args:
            max_num_events: Maximum number of events iterator will iterate.
            timeout: Timeout of the gRPC connection with the capsule8 sensor.
            connection_timeout: Timeout of the initial connection to the capsule8 sensor
            asynchronous: Boolean to use telemetry as an async iterator
            loop: If asynchronous an event loop can be provided to aiogrpc.*_channel
            ignore_status: Boolean to filter status messages
