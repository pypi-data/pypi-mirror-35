...
from nf.utils import load_to_list, dict_fields_filter, takePriceFromDepth
...
def get_execution_reports(exchange=None):
    ...
    res = ExecRpts()
    res.ParseFromString(result)
    return [protobuf_to_dict(Execrpt, including_default_value_fields=True) for
            Execrpt in res.data]
...
def get_execution_reports_intraday():
    status, result = py_nfi_get_execution_reports_intraday()
...
def get_orders_intraday():
    status, result = py_nfi_get_orders_intraday()
...
def get_all_symbols_of_coin(market, currency, df=False):
    if not market or not currency:
        raise TypeError('get_all_symbols_of_coin() : argument missing or incorrect argument type.')

    status, result = py_nfi_get_all_symbols_of_coin(market, currency)
...
def history_n(symbol, frequency, n, end_time, fields='', skip_suspended=True, fill_missing='', df=False):
    if not symbol or not frequency or not n or not end_time:
    ...
    if frequency == 'tick':
        ...
        status, result = py_nfi_history_ticks_n(symbol, n, end_time, fields, skip_suspended,
                                                fill_missing, 1)
        ...
        status, result = py_nfi_history_bars_n(symbol, frequency, n, end_time, fields, skip_suspended,
                                               fill_missing, 1)
...
def get_unfinished_orders_intraday():
    status, result = py_nfi_get_unfinished_orders_intraday()
