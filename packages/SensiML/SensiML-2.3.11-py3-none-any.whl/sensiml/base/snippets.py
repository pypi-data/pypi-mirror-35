...
def function_help(func):
    ...
    if isinstance(func, PipelineSeed):
        return """results, summary = dsk.pipeline.auto({{'seed': '{}', 'params':{{{}}}}})""".format(func.name, ', \n\t\t\t\t\t\t\t\t'.join(params))
    ...
    if func.type == 'Feature Selector':
        return """dsk.pipeline.add_feature_selector([{{'name':'{}', 'params':{{{}}}}}], params={{'number_of_features':(int)}})""".format(func.name, ', '.join(params))

    if func.type in ['Segmenter', 'Transform', 'Sampler']:
        return  """dsk.pipeline.add_transform("{}", params={{{}}})""".format(func.name, ', \n\t\t\t\t\t\t\t\t'.join(params))

    if func.type == 'Training Algorithm':
        return """dsk.pipeline.set_training_algorithm("{}", params={{{}}})""".format(func.name, ', \n\t\t\t\t\t\t\t\t'.join(params))

    if func.type == 'Validation Method':
        return """dsk.pipeline.set_validation_method("{}", params={{{}}})""".format(func.name, ', \n\t\t\t\t\t\t\t\t'.join(params))

    if func.type == 'Classifier':
        return """dsk.pipeline.set_classifier("{}", params={{{}}})""".format(func.name, ', \n\t\t\t\t\t\t\t\t'.join(params))
...
def pipeline_function_help(function_list, step, filter_list  = ['group_columns', 'label_column','ignore_columns','passthrough_columns', "classifiers", 'validation_methods',  'input_data','return_segment_index']):
        ...
        if func.type == 'Feature Generator':
            return """{{'name':'{}', 'params':{{{}}}}}""".format(func.name, ', '.join(params))

        if func.type == 'Feature Selector':
            return """{{'name':'{}', 'params':{{{}}}}}, """.format(func.name, ', '.join(params))

        if func.type in ['Segmenter', 'Transform']:
            return  """dsk.pipeline.add_transform("{}", params={{{}}})""".format(func.name, ', '.join(params))

        if func.type == 'Training Algorithm':
            return """dsk.pipeline.set_training_algorithm("{}", params={{{}}})""".format(func.name, ', '.join(params))

        if func.type == 'Validation Method':
            return """dsk.pipeline.set_validation_method("{}", params={{{}}})""".format(func.name, ', '.join(params))

        if func.type == 'Classifier':
            return """dsk.pipeline.set_classifier("{}", params={{{}}})""".format(func.name, ', '.join(params))
