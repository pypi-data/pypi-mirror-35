...
can be provided via module options.
...
In addition or in place of the variable file(s), environments can be
specified via module options.
...
deployments of static websites. It can be used with a configuration like
the following:
...
This will build the website in ``conduit`` via the specified
build\_steps and then upload the contents of ``dist`` to a S3 bucket
created in the CloudFormation stack ``contoso-dev-conduit``.
