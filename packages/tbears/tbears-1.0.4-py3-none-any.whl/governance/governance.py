...
class Governance(IconScoreBase):
    ...
    @eventlog(indexed=0)
    def add_import_white_list_log(self, add_list: str, add_count: int):
        pass

    @eventlog(indexed=0)
    def remove_import_white_list_log(self, remove_list: str, remove_count: int):
        pass
    ...
    @external
    def add_import_white_list(self, import_stmt: str):
        ...
        # make eventlog
        if len(log_entry):
            self.add_import_white_list_log(add_list=str(log_entry), add_count=len(log_entry))

        # TODO send noti to SCORE engine

        if DEBUG is True:
            self._print_import_white_list('add_import_white_list')

    @external
    def remove_import_white_list(self, import_stmt: str):
        # only owner can add import white list
        if self.msg.sender != self.owner:
            self.revert('Invalid sender: not owner')
        ...
        # make eventlog
        if len(log_entry):
            self.remove_import_white_list_log(remove_list=str(log_entry), remove_count=len(log_entry))

        # TODO send noti to SCORE engine

        if DEBUG is True:
            self._print_import_white_list('remove_import_white_list')
    ...
    def _print_import_white_list(self, header: str):
        Logger.debug(f'{header}: list len = {len(self._import_white_list)}', TAG)
        for pkg in self._import_white_list:
            Logger.debug(f' --- {pkg}: {self._import_white_list[pkg]}', TAG)
