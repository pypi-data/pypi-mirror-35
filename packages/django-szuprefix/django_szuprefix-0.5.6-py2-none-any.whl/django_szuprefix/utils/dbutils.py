        ...
        for row in introspection.get_table_description(cursor, table_name):
            ...
            if field_type == 'DecimalField':
                if row[4] is None or row[5] is None:
                    ...
                    md = row[4]
                    dp = row[5]
                field_params['max_digits'] = md == 65535 and 1000 or md
                field_params['decimal_places'] = dp == 65535 and 1000 or md
...
def create_table(conn, table, fields, schema=None):
    try:
    ...
    with conn.schema_editor() as schema_editor:
        ...
        full_table_name = schema and "%s.%s" % (schema, schema_editor.quote_name(table)) or schema_editor.quote_name(table)
