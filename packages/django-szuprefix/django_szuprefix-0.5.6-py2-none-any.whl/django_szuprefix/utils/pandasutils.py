...
class AutoGrowTable(object):
    def __init__(self, db_name, table_name, primary_key, insert_timestamp_field=None, update_timestamp_field=None):
        ...
        self.full_table_name = self.schema and "%s.%s" % (self.connection.ops.quote_name(self.schema), self.connection.ops.quote_name(self.table_name)) or self.connection.ops.quote_name(self.table_name)
    ...
    def gen_sql_table(self, df):
        ...
        self.table = SQLTable(self.table_name, self.pd_sql, df, index=False, schema=self.schema).table.tometadata(self.pd_sql.meta)
    ...
    def insert_or_update(self, df):
        ...
        pks = [k.strip().lower() for k in self.primary_key.split(",")]
        efs = ['1 as a']
        ...
        for i in xrange(len(df)):
            try:
                ...
                where = " and ".join(["%s='%s'" % (quote_name(pk), d[pk]) for pk in pks])
