...
class AutoGrowTable(object):
    def __init__(self, db_name, table_name, primary_key, insert_timestamp_field=None, update_timestamp_field=None):
        ...
        self.table_name = self.connection.ops.quote_name(tps[-1])
        self.schema = len(tps) > 1 and self.connection.ops.quote_name(tps[0]) or None
        self.full_table_name = self.schema and "%s.%s" % (self.schema, self.table_name) or self.table_name
