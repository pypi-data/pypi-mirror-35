...
def group_by(df, dimension, measures=None, agg='count'):
    if not measures:
        return df.groupby(dimension)[dimension].agg(agg).to_frame(agg).reset_index()
    return df.groupby(dimension, as_index=False)[measures].agg(agg)
