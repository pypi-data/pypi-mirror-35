...
class NineAnime(BaseAnime):
    ...
    def _scarpe_episodes(self, soup):
        ...
        # Gomen'nasai
        api_url = "https://www8.9anime.is/ajax/film/servers-ping?ts={}".format(ts)
        ...
        print(r.json(), r.url)
        soup = BeautifulSoup(requests.get(api_url,params=params, headers=headers).json()['html'], 'html.parser')
