...
def ferry_function(func, device):
    replacements = [
        ...
        *[(r's*([a-zA-Z_][a-zA-Z_0-9]*)\s*=\s*%s\((.+)\)' % type_class.__name__,
        ...
        (r' or ', r'||'),
        (r' not ', r'!'),
        *[(r'self.%s.%s\((.*?)\)' % (member[0], funky[0]), r'%s(\1)' % funky[1]()) for member in get_members(device) for
          funky in
          get_functions(member[1])],
        *[(r'\s*self.%s.%s\((.*?)\)\s*$' % (member[0], funky[0]), r'%s(\1);' % funky[1]()) for member in get_members(device) for
          funky in
          get_functions(member[1])],
    ]
