...
def start_hosting():
    ...
    try:
        ...
        # ensuring host tests have been ran
        if not os.path.isfile('/var/vectordash/flags/tests_passed'):
            print("You have not ran host tests yet. Please run " +
                  stylize("vdhost run-tests", fg("blue")) + ' to continue.')
            return
        ...
        r = requests.post(urljoin(VECTORDASH_URL, "authenticate-host/"), data={'email': email, 'machine_key': machine_key})
