...
def get_merge_cells(index):
    ranges = [[(0, len(index.labels[0]))]]

    for ix, index_level in enumerate(index.labels[:]):
        index_ranges = []
        for rng in ranges[ix]:
            index_ranges.extend(get_contiguous_ranges(index_level, rng[0], rng[1]))
...
def get_contiguous_ranges(lst, lst_start, lst_end):
    ...
    for ix2, ele in enumerate(lst_section):
        if start < ix2 - 1 and prev_val != ele:
            index_ranges.append((lst_start + start, lst_start + ix2 - 1))
            start = ix2
        elif start != ix2 and ix2 == len(lst_section) - 1:
            index_ranges.append((lst_start + start, lst_start + ix2))
        prev_val = ele
    return index_ranges


def create_merge_cells_request(
    sheet_id, start_row, end_row, start_col, end_col, merge_type="MERGE_ALL"
):
    return {
        "mergeCells": {
            "range": {
...
def create_merge_labels_request(sheet_id, df, start, index=True, cols=True):
    request = []
    start = get_cell_as_tuple(start)

    if index and isinstance(df.index, pd.MultiIndex):
        merge_cells = get_merge_cells(df.index)
        request.append(
            [
    ...
    if cols and isinstance(df.columns, pd.MultiIndex):
        merge_cells = get_merge_cells(df.columns)
        request.append(
            [
                create_merge_cells_request(
                    sheet_id,
