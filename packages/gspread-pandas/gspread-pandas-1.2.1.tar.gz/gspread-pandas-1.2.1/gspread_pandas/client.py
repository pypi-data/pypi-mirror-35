...
class Spread:
    ...
    @_ensure_auth
    def merge_cells(
        self, start_row, end_row, start_col, end_col, merge_type="MERGE_ALL", sheet=None
    ):
        """Add filters to data in the open worksheet.
        ...
        self.spread.batch_update(
            {
                "requests": create_merge_cells_request(
                    self.sheet.id, start_row, end_row, start_col, end_col
                )
            }
        )

    @_ensure_auth
    def pmerge_cell_ranges(self, ranges, sheet=None):
        """Add filters to data in the open worksheet.
