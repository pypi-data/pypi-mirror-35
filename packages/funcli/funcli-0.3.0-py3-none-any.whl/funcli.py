...
def spec_to_argparse(spec: Spec, parser: argparse.ArgumentParser, converters: Converters) -> None:
	...
	for i, arg in enumerate(argspec.args):
		...
		if required or content is not bool:
			if required:
				...
				parser.add_argument(dest, default = default, type = get_converter(converters, content), nargs = ('*' if container else None))
				post[arg] = container
		else:
	...
	for arg in argspec.kwonlyargs:
		...
		dest = '--' + arg
		if required or content is not bool:
			parser.add_argument(dest, default = default, required = required, type = get_converter(converters, content), nargs = ('*' if container else None))
			post[arg] = container
		else:
...
def decompose_annotation(annot: Any) -> Tuple[Optional[type], Any]:
	...
	if origin is None or not annot.__args__:
		return decompose_annotation((origin or annot)[Any])
	arg = deoptionalize(annot.__args__[0])
...
def deoptionalize(annot: Any) -> Any:
	...
	if origin is Union:
		args = [a for a in annot.__args__ if a is not NoneType]
		if len(args) == 1:
