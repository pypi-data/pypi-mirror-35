...
class RoonApi():
    ...
    def register_source_control(self, control_key, display_name, callback, initial_state="selected", supports_standby=True):
        ...
        if not self._source_controls_request_id:
            LOGGER.error("Not yet registered, can not register source control")
            return False
        control_data = {
    ...
    def update_source_control(self, control_key, new_state):
        ...
        if control_key not in self._source_controls:
            ...
            return
        if not self._source_controls_request_id:
            LOGGER.error("Not yet registered, can not update source control")
            return False
    ...
    def update_volume_control(self, control_key, volume=None, mute=None):
        ...
        if control_key not in self._volume_controls:
            ...
            return
        if not self._source_controls_request_id:
            LOGGER.error("Not yet registered, can not update volume control")
            return False
        if volume != None:
    ...
    def _do_init(self):
        ...
        while not self._host and not self._exit:
            ...
            if not host and not self._exit:
                ...
                self._host = host
                self._port = port
        ws_address = "ws://%s:%s/api" %(self._host, self._port)
