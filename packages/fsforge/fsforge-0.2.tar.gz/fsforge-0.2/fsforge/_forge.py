...
def create_fs(fs_fixture, fs_tree_dict, root_mount_point):
    """
    ...
    assert type(fs_fixture).__name__ == "FakeFilesystem", "Expecting pyflakefs' fs object"
    assert isinstance(fs_tree_dict, dict), 'Expecting a dictionary as an argument.'
    assert isinstance(root_mount_point, str), 'Expecting root_mount_point to be a string.'

    if not os.path.exists(root_mount_point):
        fs_fixture.create_dir(root_mount_point)

    for directories, files in _walk_equivalent(root_mount_point, fs_tree_dict):
        for directory_path in directories:
            fs_fixture.create_dir(directory_path)

        for file_path, file_content in files.items():
            fs_fixture.create_file(file_path, contents=file_content or '')
