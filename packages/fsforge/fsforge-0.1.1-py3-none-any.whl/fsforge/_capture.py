...
class PP(object):
    ...
    def __contains__(self, other):
        """
            ...
            e.g. PP('file.txt').matches('that_file.txt') should give negative result,
            but ('file.txt').matches('*file.txt') is supposed to be True

        """
        if isinstance(other, (PP, str)):
...
def _matching_dir_masks(tree_masks, dir_name):
    def collect():
        for name_pattern, sub_masks_def in _masks(tree_masks):
            if dir_name in PP(name_pattern):
                if name_pattern == "**":
        ...
        for name_pattern, processor in _masks(tree_masks):
            if name_pattern == "**" and isinstance(processor, dict):
                for sub_name_pattern, sub_proccessor in processor.items():
                    if file_name in PP(sub_name_pattern):
                        yield sub_name_pattern, sub_proccessor
            else:
                if file_name in PP(name_pattern) and not isinstance(processor, dict):
                    yield name_pattern, processor

    result = list(zip(*collect()))
    if result:
        patterns, processors = result
        if len(processors) > 1:
