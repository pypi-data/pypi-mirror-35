...
class Database:
    ...
    def endpoint(self, method, source, status):
        ...
        print '========================================='
        print 'SELF.ACTIONS:'
        pprint(self.actions['put']['/1.0/gdi/products/mercado/editar'])
        #time.sleep(5)
        print '========================================='
        pprint(self.actions)
        self.method = method
        ...
        if self.id:
            ...
            if data_response.ready():
                ...
                self.status.error("ERROR_DATABASE_CONFIG", None,
                                  ["Nao obteve dados de configuracao do database", self.id])
        else:
            self.status.error("ERROR_DATABASE_CONFIG", None, ["Nao recebeu identificador de database", "--"])
    ...
    def __brokers(self, service_port):
        if self.brokers:
            ...
            self.status.error("ERROR_DATABASE_CONFIG", None,
                              ["Nao obteve dados de configuracao de brokers do database", self.id])
    ...
    def __output(self):
        if self.network:
            ...
            self.status.error("ERROR_DATABASE_CONFIG", None,
                              ["Nao obteve dados de configuracao do network no database", self.id])
    ...
    def fields_insert(self, body, endpoint):
        ...
        for item_body_field in body:
            ...
            item_name, item_scheme = self.__check_body_item_scheme_insert(body_field, endpoint.scheme)
    ...
    def fields_update(self, body, scheme, fields_default):
        ...
        for item_body_field in body:
            body_field = item_body_field.replace(' ', '_').lower().strip()
            item_name, item_scheme = self.__check_body_item_scheme_update(body_field, scheme)

            #print 'FIELDS_UPDATE', item_name, item_scheme
            #print 'BODY_FIELD>>>>>>>>>', body_field
            #print 'SCHEME>>>>>>>>>>>>>', scheme
    ...
    # --------------------------------
    # __check_body_item_scheme_insert
    # --------------------------------
    # Verifica se o campo recebido via body esta configurado no esquema
    @staticmethod
    def __check_body_item_scheme_insert(body_field, scheme):
    ...
    # --------------------------------
    # __check_body_item_scheme_update
    # --------------------------------
    # Verifica se o campo recebido via body esta configurado no esquema
    @staticmethod
    def __check_body_item_scheme_update(body_field, scheme):
        for item_scheme in scheme:
            if item_scheme['field'] == body_field:
                return body_field, item_scheme
        return None, None
