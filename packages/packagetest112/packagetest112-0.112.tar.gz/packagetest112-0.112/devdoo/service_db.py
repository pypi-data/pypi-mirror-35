...
class ServiceDB:
    ...
    # __init__
    # --------------------------------
    def __init__(self, message, mongo_server):
        self.mongo_server = mongo_server
        self.status = Status()
        self.datarest = DataRest(message, self.status, True)

        #self.datarest.show('SELF.DATAREST')
        #print "[[[[[[[[[[MESSAGE2222]]]]]]]]", message
    ...
    def send_result(self):
        return self.datarest.send_result()
    ...
    def find(self):
        ...
        db_fields = self.datarest.get_fields()
        db_filter = self.datarest.filter()
        db_limit = self.datarest.get_limit()
        db_offset = self.datarest.offset()
        db_sort = self.datarest.sort()

        # Consulta o base de dados
        find_result = collection.find(db_filter, db_fields).skip(db_offset).sort(db_sort.items())
        found_count = find_result.count()
        ...
        if "item" == self.datarest.get_result_type():
            if filtered_count == 1:
                data_found = find_result_limit[0]
            else:
                self.status.warn("WARN_FIND_ITEM", None, [])

        elif "list" == self.datarest.get_result_type():
            data_found = find_result_limit
    ...
    def insert(self):
        ...
        db_data = self.datarest.get_data()
        db_fields = self.datarest.get_fields()
    ...
    # --------------------------------
    def __action(self):
        action = self.datarest.action_db()
    ...
    def __collection(self):
        ...
        print "SELF.DATAREST.DATABASE()", self.datarest.database()
        print "SELF.CONFIG", self.mongo_server
        database = self.mongo_server[self.datarest.database()]
        # Recupera/Cria a coleção mongo db
        return database[self.datarest.get_collection()]
    ...
    def __prepare_data_changes(self, data_update, data_active):
        ...
        insert_data_update = {
            ...
            "collection": self.datarest.get_collection()
        }
