...
def corine_for_cutout(cutout, grid_codes, label=None, natura=False,
    ...
    if distance is not None and len(distance_grid_codes) > 0:
        ...
        with timer("Merging the two tiffs into one another"):
            merge_fn = os.path.join(tmpdir, 'merge.tif')
            ...
            # fn1 = 'tmpicfdze0l_proximity.tif'
            # fn2 = 'tmpicfdze0l.tif'

            # result_fn = 'test.tif'

            # with rasterio.open(fn1) as src1, rasterio.open(fn2) as src2:
            #     meta = src1.meta.copy()
            #     meta.update(compress='lzw')
            ...
            #     with rasterio.open(result_fn, 'w', **meta) as dst:
            #         for idx, window in windows:
            #             src_data1, = src1.read(window=window)
            #             src_data2, = src1.read(window=window)
            #             dst.write_band(1, src_data1 & src_data2, window=window)
