...
@cli.subparser('deploy_keys',help='List deploy keys')
def list_deploy_keys(cli,argv):
    api = cli.get_api(argv.name)
    rsp = api['/deploy_keys'].get(project=argv.project)
    cli.out(rsp,fields=list(argv.fields))
list_deploy_keys.add_argument('--fields',type=ChoicesList,choices=ChoicesList(deploy_key_fields),default=deploy_key_fields)
list_deploy_keys.add_argument('--project',type=int,required=False)
# END CLI DEPLOY KEYS
