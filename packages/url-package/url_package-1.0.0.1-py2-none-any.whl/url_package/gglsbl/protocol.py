    ...
    def wrapper(*args, **kwargs):
        ...
        while True:
            try:
                ...
                OutputDebugStringA('Call Failed for {} time(s). Retrying in {} seconds: {}'.format(_fail_count, wait_for, str(e)))
                time.sleep(wait_for)
            except socket.error as e:
                transient_error_wait = 2
                OutputDebugStringA('Socket error, retrying in {} seconds.'.format(transient_error_wait))
                time.sleep(transient_error_wait)
    return wrapper


class SafeBrowsingApiClient(object):
    def __init__(self, developer_key, client_id='SaniTrans MAIL', client_version='v1.0.1', discard_fair_use_policy=True):
        self.client_id = client_id
        self.client_version = client_version
        self.discard_fair_use_policy = discard_fair_use_policy
        if self.discard_fair_use_policy:
            OutputDebugStringA('Circumventing request frequency throttling is against Safe Browsing API policy.')
    ...
    def fair_use_delay(self):
        if self.next_request_no_sooner_than is not None:
            ...
            OutputDebugStringA('Sleeping for {} seconds until next request.'.format(sleep_for))
