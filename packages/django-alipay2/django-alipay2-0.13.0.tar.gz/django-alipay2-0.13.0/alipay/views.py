...
class AlipayRedirectView(View):
    ...
    def get(self, request, out_no):
        ua_mode = request.GET.get('ua', self.UA_WEB)
        ...
        # 默认用WEB方式支付，AUTO模式下根据UA来决定用WEB还是WAP
        ...
        if ua_mode == self.UA_WEB:
            service = AlipayClient.SERVICE_DIRECT
        else:
            service = AlipayClient.SERVICE_WAP

        pay_url = alipay_api.client.create_pay_url(
            service,
            payment.out_no,
            payment.subject,
            payment.body,
