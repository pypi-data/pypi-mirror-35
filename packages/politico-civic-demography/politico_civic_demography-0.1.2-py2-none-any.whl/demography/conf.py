...
Settings.AWS_ACCESS_KEY_ID = getattr(
    project_settings, 'DEMOGRAPHY_AWS_ACCESS_KEY_ID', None)

Settings.AWS_SECRET_ACCESS_KEY = getattr(
    project_settings, 'DEMOGRAPHY_AWS_SECRET_ACCESS_KEY', None)

Settings.AWS_REGION = getattr(
    project_settings, 'DEMOGRAPHY_AWS_REGION', 'us-east-1')

Settings.AWS_S3_BUCKET = getattr(
    project_settings, 'DEMOGRAPHY_AWS_S3_BUCKET', None)
...
Settings.API_AUTHENTICATION_CLASS = getattr(
    project_settings,
    'DEMOGRAPHY_API_AUTHENTICATION_CLASS',
    'rest_framework.authentication.BasicAuthentication'
)

Settings.API_PERMISSION_CLASS = getattr(
    project_settings,
    'DEMOGRAPHY_API_PERMISSION_CLASS',
    'rest_framework.permissions.IsAdminUser'
)

Settings.API_PAGINATION_CLASS = getattr(
    project_settings,
    'DEMOGRAPHY_API_PAGINATION_CLASS',
    'demography.pagination.ResultsPagination'
)
