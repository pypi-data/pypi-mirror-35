...
class BackgroundSprite(GameSprite):
    def __init__(self, next=False):
        super().__init__('bg_img_5.jpg', speed =3)
        # 输出完第一张背景图片，第二张图片接力
        if next:
...
class Hero_bulletSprite(GameSprite):
    def __init__(self, x, y):
        super().__init__('zidan.png', speed = -28)
        self.rect.x = x
        self.rect.y = y
    # 发射子弹
    def update(self):
...
class Hero_bullet2Sprite(GameSprite):
    def __init__(self, x, y):
        super().__init__('dazhao.png', speed = -20)
        self.rect.x = x
        self.rect.y = y
    # 发射子弹
    def update(self):
...
class EnemySprite(GameSprite):
    def __init__(self):
        # 初始化敌方飞机图片，速度
        super().__init__('diji.png', speed = random.randint(4, 7))
        # 初始化敌方飞机的位置
...
class Enemy_BulletSprite(GameSprite):
    def __init__(self, x, y):
        super().__init__('34-1.png', speed = 20)
        self.rect.x = x
        self.rect.y = y
    # 敌方发射子弹
    def update(self):
...
class WarSprite(GameSprite):
    def __init__(self):
        super().__init__('kongtou.png', speed = 7)
        self.rect.x = random.randint(0, screen_rect.width)
        self.rect.y = -self.rect.height
    def update(self):
        self.rect.y += self.speed
