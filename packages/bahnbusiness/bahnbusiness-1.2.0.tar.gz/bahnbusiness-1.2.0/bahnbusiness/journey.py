...
class Journey:
  ...
  def __init__(self, id, booking_date, name_booking, journey_date, name_travel, description, route, canceled, *args):
    self.id = id
  ...
  def __repr__(self):
    return "<Journey id={id} name_booking='{name_booking}' route='{route}'>".format(
      id=self.id,
      name_booking=self.name_booking,
      route=self.route,
    )
