...
def load_state(fn,key) :
    ...
    try :
        ...
        state_end = len(state)-1-state[::-1].index('u[\033')
        state = state[state_start+3:state_end]
        # remove all control codes from the substring
        state = state.replace('\033[u','').replace('\033[s','')
...
def save_state(state,fn,key,decoy_text='') :
    with open(fn,'wt') as f :
        ...
        # if the encoded string is longer than the terminal width it
        # line breaks and restoring the cursor position doesn't work
        # as desired
        # split the encoded state into short blocks that are repeatedly
        # overwritten by cursor moves
        block_len = 30
        for i in range(0,len(encoded),block_len) :
            f.write('\033[s')
            f.write(encoded[i:i+block_len])
            f.write('\033[u')
        f.write(decoy_text)
...
class PuzzleMaster(object) :
    ...
    def set_key(self,key) :
        ...
        random.seed(key)
        # +1 because the first 'puzzle' is unlock and not stored in self.puzzles
        answers = [hex(random.randint(0x10000,0x100000))[2:] for _ in range(len(self.puzzles)+1)]
        self.unlock_answer, self.answers = answers[0], answers[1:]
    def get_puzzle(self,puzzle,path) :
        puz_ind = list(self.puzzles).index(puzzle)
        return self.puzzles[puzzle](self.key,self.answers[puz_ind],path)
    def unlock(self,answer=None) :
        state = {
                'opened': set(),
                'solved': set()
        }
        ...
        if os.path.exists(shadowy_figure) :
            prev_state = load_state(shadowy_figure,self.key)
            if prev_state is None : # they messed with the reaper
                print(bold(red('You should have feared the reaper,\n'
                    'but you messed with him/her/them,\n'
                    'and now you will pay the ultimate,\n'
                    'some might even say terminal,\nprice.'))
                )
                # reset all puzzles
        ...
        if answer is not None :
            if answer == self.unlock_answer : # first unlock

                # open all puzzles
                for i,puz in enumerate(self.puzzles.values()) :
                    puzzle = puz(self.key,self.answers[i])
                    puzzle.create()
                    state['opened'].add(str(puz.name))

            elif answer in self.answers:
                index = self.answers.index(answer)
                state['solved'].add(list(self.puzzles)[index])
            else :
            ...
            save_state(state,shadowy_figure,self.key,reaper)

        num_solved = len(set(self.puzzles).intersection(state['solved']))
        if num_solved == 0 :
            unlock(self.key,self.unlock_answer,self.key).run()
        elif num_solved == 1 :
        ...
        elif num_solved == len(set(self.puzzles)) :
            dazzle()
            banner()
        ...
        if len(state['opened']) != 0 :
            print('BEHOLD! The puzzles:')
            for i, (pid, puz) in enumerate(self.puzzles.items()) :
                puzzles = puz.scan(self.key)
                if puzzles :
                    if pid in state['solved'] :
                        opened_answer = bold(yellow(self.answers[i]))
                    else :
                        opened_answer = red('LOCKED')
                    print(pid,opened_answer)

# each of these puzzles has a class with class, a classy class
class Puzzle(object) :
    def __init__(self,key,answer,path=None) :
        self.key = key
        self.answer = answer
    ...
    def create_dir(self,path=None) :
        ...
        with open(script_path,'wt') as f :
            sha = base64.b64encode(
                '{} {} {}'.format(self.name,'.',self.key).encode()
            )
            f.write(
                'terminal_temple sha {} $@\n'.format(
                sha.decode()
