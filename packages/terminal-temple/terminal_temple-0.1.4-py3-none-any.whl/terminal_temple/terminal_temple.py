...
def main() :
    ...
    Usage: terminal_temple create <puzzle> <key> [<path>] [<guess>] [<args>...]
                 terminal_temple run <puzzle> <key> [<path>] [<guess>] [<args>...]
    ...
    if len(args) == 0 :
        ...
        try :
            ...
            print(bold('\n\nOk ok ok, needn\'t be hasty, geez, I made your stupid\n'
                  'puzzles Mr./Ms./Mx./Misc./Ind./Dr. Impatient Pants.'))
            print(bold('They are yon ')+magenta(path))
    ...
    # otherwise we're running a puzzle op
        ...
        if args['sha'] :
            ...
            puzzle_name, path, key = dec.split()
        else :
            puzzle_name = args['<puzzle>']
            path = args['<path>']
            key = args['<key>']

        # this sets the key for the puzzles and generates answers
        puz_mas.set_key(key)

        # unlock is a special case
        if puzzle_name == 'unlock' :
            puz_mas.unlock(args.get('<guess>'))
            ...
            if not path :
                path = '.'
            puzzle = puz_mas.get_puzzle(puzzle_name,path)
