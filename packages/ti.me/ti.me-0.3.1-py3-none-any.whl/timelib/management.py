...
class Manager:
    ...
    Examples:
        ...
        >>> Manager(create_session('sqlite://'), 'stranger')  # doctest: +ELLIPSIS
    ...
    def complete_task(
        ...
        if all(task.completed for task in task.blocking_tasks):
            if task.completed:
                warnings.warn(
    ...
    def create_tasks_from_update(
        ...
        for planned_task in update.tasks:
            tasks[planned_task.id] = self.create_task(
                ...
                blocking_task_ids=[
                    tasks[blocking_task.id].id
                    for blocking_task
                    in planned_task.blocking_tasks
                ],
                target_task_ids=[
                    tasks[target_task.id].id
                    for target_task
                    in planned_task.target_tasks
                ],
            )
