...
class MySql:
    ...
    def read_keyword_from_db(self):
         self.cursor = self.db.cursor()
         self.cursor.execute("""SELECT Text, id from Periodic_keyword""")
         result_set = self.cursor.fetchall()
         return result_set

    def update_data_base(self, result, keyword_id):
        for items in result:
            display_name = items.get_user().get_display_name()
            display_picture = items.get_user().get_display_picture()
            follower_count = items.get_user().get_follower_count()
            ...
            self.cursor.execute(
                """Insert into Periodic_post(Keyword_id, PostUser_id, statusID,Content,CreatedAt, Source, ResharerCount) VALUES (%s,%s,%s,%s,%s,%s,%s)""",
