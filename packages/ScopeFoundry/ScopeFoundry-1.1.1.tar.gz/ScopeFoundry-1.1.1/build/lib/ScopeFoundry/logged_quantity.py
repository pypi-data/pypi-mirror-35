...
class LQRange2(LQCircularNetwork):
...
class LQRange(LQCircularNetwork):
    ...
    def __init__(self, min_lq, max_lq, step_lq, num_lq, center_lq=None, span_lq=None, preserve_num=True):
        QtCore.QObject.__init__(self)
        self.log = get_logger_from_class(self)
        self.min = min_lq
        self.max = max_lq
    ...
    def calc_num(self, min_, max_, step):
        ...
        new_step = span/n
        num = n+1
        return int(num),new_step
    
    def calc_num_preserve_step(self,min_, max_, step):
        span = max_-min_
        if step==0:
            n = 10 
        else: 
        ...
        num = n+1
        new_min = min_
        new_max = new_min + num*step
        
        return int(num), new_min, new_max
    ...
    def on_change_min_max(self):
        ...
        if self.preserve_num:
            num,new_step = self.calc_num(min_, max_, step)
            self.update_values_synchronously(num=num, span=span, center=center, step=new_step)
        else:
            num, new_min, new_max = self.calc_num_preserve_step(min_, max_, step)
            self.update_values_synchronously(num=num, min=new_min, max=new_max, step=step)
...
#        return self._logged_quantities.items()
    ...
    def New_Range(self, name, include_center_span=False, preserve_num=True, **kwargs):
        ...
        if include_center_span:
            ...
            lqrange = LQRange(min_lq, max_lq, step_lq, num_lq, center_lq, span_lq, preserve_num=preserve_num)
        else:
            lqrange = LQRange(min_lq, max_lq, step_lq, num_lq, preserve_num=preserve_num)
