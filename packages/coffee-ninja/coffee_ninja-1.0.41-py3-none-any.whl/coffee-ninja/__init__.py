...
class TogglerProcessor(Preprocessor):
    ...
    def run(self, lines):
        ...
        content = self.getTogglerHTML(toggler_p, tabs_p, content)
    ...
    def getTogglerHTML(self, toggler_p, tabs_p, content):
    ...
    def getNestedTogglerDiv(self, content):
        ...
        nested_toggler_p = r'(^/{3}[\s\S]*?/{3}[\s\S]*?[ ]{2,}[\S].*?(?=\n\n+.*?(?!/)^[\S]))|' \
                           r'(^/{3}[\s\S]*?/{3}[\s\S]*?[ ]{2,}[\S].*?(?=\n\n+.*?(?!/)[\S]))|' \
                           r'(^/{3}[\s\S]*/{3}[\s\S]*?[ ]{2,}[\S].*?(?=\n\n+))'
        content = self.getTogglerHTML(nested_toggler_p, tabs_p, content)
