    ...
    def __init__(self, name, value_or_values, update_memory=False):
        ...
        if update_memory:
            if self.Values is None:
                ...
                os.environ[self.Name] = CurrentShell.EnvironmentVariableDelimiter.join(self.Values) # <Class '<name>' has no '<attr>' member> pylint: disable = E1101
...
class Augment(object):
    ...
    def __init__(self, name, value_or_values, update_memory=False):
        ...
        if update_memory:
            ...
            os.environ[self.Name] = CurrentShell.EnvironmentVariableDelimiter.join(itertools.chain(new_values, existing_values)) # <Class '<name>' has no '<attr>' member> pylint: disable = E1101
