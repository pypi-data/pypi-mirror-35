...
class Stream:
	...
	#itemDetails = retrieveWithAddress(stream, address, count)	#call to retrieveWithAddress() function
	...
	#items = retrieveWithKey(stream, key, count)	#call to retrieveWithKey() function
	...
	#result = verifyData("root","test data to check", 20)	#call to verifyData() function
	...
	def retrieveItems(self, stream, count):		#retrieveItems() function definition
		...
		if check is None:
			...
			itemsDetails = response_json[0]['error']['message']
