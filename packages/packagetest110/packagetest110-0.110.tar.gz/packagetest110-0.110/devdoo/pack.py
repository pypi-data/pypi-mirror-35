...
class Pack:
    ...
    def __init__(self, message, status):
        ...
        self.length_in = 0  # Tamanho do pacote de entrada
        self.length_out = 0  # Tamanho do pacote de saída
        self.service = None  # Identificador do serviço
        self.service_id = None  # Id do serviço a ser executado
        self.database_id = None  # Id numérico da base de dados
    ...
    def __decode(self):
        ...
        if Check.is_string(self.message):
            ...
            if Check.is_object(pack):
                ...
                self.body = pack["body"]
                self.id = pack["id"] # id da conexão (criado no connection - rest)
                self.length_in = sys.getsizeof(self.message)
                self.length_out = pack["length_out"]
    ...
    def __check_params(self, pack):
        ...
        if "length_in" not in pack.keys():
            errors.append("LENGTH_IN")

        if "length_out" not in pack.keys():
            errors.append("LENGTH_OUT")
