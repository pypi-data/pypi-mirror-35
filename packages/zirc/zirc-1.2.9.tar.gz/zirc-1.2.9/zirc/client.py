...
class Client(object):
    ...
    def main_job(self, event):
        ...
        # CTCP and ACTION Replies
        if event.type == "PRIVMSG":
            if event.arguments[0].startswith("\x01ACTION"):
                event.type = "ACTION"
                event.text_type = "ACTION"
            elif event.arguments[0].startswith("\x01"):
                event.type = "CTCP"
                event.text_type = "CTCP"
                if hasattr(self, "ctcp"):
