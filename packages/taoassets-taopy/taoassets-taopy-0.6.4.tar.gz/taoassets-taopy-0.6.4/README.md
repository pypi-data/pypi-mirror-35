...
needed parameters. They can be found in `taopy.structs.script`. All the constructors of these classes can take an input of type `Script`.
...
This library offers `Solver`s to spend a previous transaction's output. Solvers can be found in `taopy.structs.sig` and 
