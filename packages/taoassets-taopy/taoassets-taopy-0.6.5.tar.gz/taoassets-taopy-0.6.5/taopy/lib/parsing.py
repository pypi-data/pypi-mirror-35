...
class TaoTxParser(TransactionParser):
    ...
    def _data(self):
        '''get transaction data (tao specific)'''
        import base64
        data=base64.b64decode(str.from_bytes(Parser.from_hex(self)))
        return str(data)

    def _txout(self, n):
        from ..structs.script import ScriptBuilder
        from ..structs.transaction import TaoTxOut

        value = int.from_bytes(self >> 8, 'little')
        script = ScriptBuilder.identify(self >> self.parse_varint())
        return TaoTxOut(value, n, script, self.network)
    ...
    def get_next_tx(self, mutable=False):
        ...
        if segwit:
            ...
            result = TaoTx(version, tstamp, txins, txouts, locktime, data, self.network)
