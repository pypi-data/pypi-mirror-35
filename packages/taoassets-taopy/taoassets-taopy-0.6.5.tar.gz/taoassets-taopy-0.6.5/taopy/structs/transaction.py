...
from ..lib.parsing import Parser, PeercoinTxParser, TaoTxParser, TransactionParser, Stream
from taopy.constants import TaoMainnet, BitcoinMainnet, PeercoinMainnet
...
class TaoTxOut(TxOut):
    def __init__(self, value: int, utxoid: int, n: int, script_pubkey: ScriptPubKey, network):
        object.__setattr__(self, 'value', value)
        object.__setattr__(self, 'utxoid', utxoid)
        object.__setattr__(self, 'n', n)
        object.__setattr__(self, 'script_pubkey', script_pubkey)
        object.__setattr__(self, 'network', network)
    ...
    def serialize(self):
        result = Stream()
        result << self.value.to_bytes(8, 'little')
        result << self.utxoid.to_bytes(8, 'little')
        result << Parser.to_varint(len(self.script_pubkey))
        result << self.script_pubkey
        return result.serialize()

    def __str__(self):
        return "TxOut(value={}, utxoid={}, n={}, scriptPubKey='{}')".format(self.value, self.utxoid, self.n, self.script_pubkey)
...
class TaoTx(Transaction):

    def __init__(self, version: int, timestamp: int, ins: list, outs: list,
                 locktime: Locktime, data:str, network, txid: str=None) -> None:
    ...
    @classmethod
    def unhexlify(cls, string, network=TaoMainnet):
        return cls.deserialize(bytearray(unhexlify(string)), TaoTxParser, network)
...
class TaoMutableTx(TaoTx, MutableTransaction):

    def __init__(self, version: int, timestamp: int, ins: list,
                 outs: list, locktime: Locktime, data:str, network) -> None:

        super().__init__(version, timestamp, ins, outs, locktime, data, network)
        ins = []
        for txin in self.ins:
            if isinstance(txin, MutableTxIn):
                ins.append(txin)
    ...
    def to_segwit(self):
        raise NotImplementedError("Tao doesn't have SegWit.")
