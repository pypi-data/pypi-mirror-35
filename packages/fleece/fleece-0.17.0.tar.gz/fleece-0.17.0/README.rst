...
.. _fleece-config-edit-[--json]-[--editor-editor]:
...
.. _fleece-config-render-[--json]-[--encrypt]-[--python]-<environment>:
...
There are four output options: YAML plaintext (the default), JSON
plaintext (with ``--json``), JSON encrypted (with ``--encrypt``) and an
encrypted Python module (with ``--python``).
