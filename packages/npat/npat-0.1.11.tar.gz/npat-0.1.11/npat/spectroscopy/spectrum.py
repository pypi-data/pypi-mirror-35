import sqlite3, os, matplotlib.pyplot as plt, numpy as np, datetime as dtm
from scipy.optimize import curve_fit
from scipy.special import erfc
from ast import literal_eval
from ..reactions import isotope
from ..misc import pallate
...
class spectrum(object):
	...
	def write_db(self,calibrate=False):
		...
		self.db_connection.commit()
	def get_fmt_pks(self,calibrate=False,cutoff=4.5):
		pks = []
		for pk in self.fit_peaks(calibrate,cutoff):
			...
			for m in range(N):
				...
				pks.append({'istp':info[3],'E':info[0],'I':info[1],'unc_I':info[2],'N':info[4],'unc_N':info[5],'A':info[7],'unc_A':info[8],'eff':self.map_efficiency(info[0]),'unc_eff':self.map_unc_efficiency(info[0]),'eff_corr':self.map_efficiency_correction(info[0]),'chi2':info[6],'fit':fit,'unc':unc})
		return pks
	...
	def filter_peaks(self,pks,cutoff=4.5):
		...
		for pk in pks:
			...
			for m in range(len(pk['pk_info'])):
				...
				itp = isotope(pk['pk_info'][m][3])
				Ac = (self.real_time/self.live_time)*itp.decay_const()*N_cts/((1.0-np.exp(-itp.decay_const()*self.real_time))*self.map_efficiency(pk['pk_info'][m][0])*pk['pk_info'][m][1]*0.01)
				unc_Ac = Ac*np.sqrt(((unc_N**2+N_cts)/N_cts**2)+(itp.decay_const(unc=True)[1]/itp.decay_const())**2+(self.map_unc_efficiency(pk['pk_info'][m][0])/self.map_efficiency(pk['pk_info'][m][0]))**2+(pk['pk_info'][m][2]/pk['pk_info'][m][1])**2)
				if sig>cutoff and chi2*cutoff<N_cts:
					peaks.append({'l':x[0],'h':x[-1],'gm':pk['pk_info'][m]+[N_cts,unc_N,chi2,Ac,unc_Ac],'p0':pk['p0'][N*m:N*m+N],'fit':pk['fit'][N*m:N*m+N],'unc':[u.tolist()[N*m:N*m+N] for u in pk['unc'][N*m:N*m+N]]})
		self.peak_fits = self.group_peaks(peaks)
		return self.peak_fits
	def fit_peaks(self,calibrate=False,cutoff=4.5):
		...
		for p in self.get_p0():
			try:
				...
				print Exception, err
		return self.filter_peaks(fits,cutoff)
	def plot(self,logscale=True,wfit=False,bg=False,calibrate=False,subpeak=False,printout=False,saveas=None,p0=False):
		...
		if wfit:
			for n,pk in enumerate(self.fit_peaks(calibrate)):
				if printout:
					for p in pk['pk_info']:
						print ''.join(map(str,[p[3],': E(keV)=',p[0],', ','I(%)=',p[1],'(',p[2],'), ','N=',p[4],'(',int(p[5]),'), ','A(Bq)=',str(int(p[7])),'(',str(int(p[8])),'), ','chi^2_nu=',round(p[6],2)]))
