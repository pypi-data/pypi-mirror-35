...
class phasePrecession():
	...
	def plotPPRegression(self, regressorDict, regressor2plot='pos_d_cum', ax=None):
		...
		idx = self.TETRODE[self.tetrode].getClustIdx(self.cluster)
		t = self._getClusterPhaseVals(self.tetrode, self.cluster)
		x = self._getClusterXPos(self.tetrode, self.cluster)
		label, xe, _ = self._getFieldLims(self.tetrode, self.cluster)
