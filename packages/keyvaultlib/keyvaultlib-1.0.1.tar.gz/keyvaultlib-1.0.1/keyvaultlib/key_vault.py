...
class KeyVaultOAuthClient(KeyVaultClient):
    ...
    def __init__(self, client_id=None, client_secret=None, tenant_id=None, use_msi=False, logger=None, verbose=False,
                 *args, **kwargs):
        # type: (str, str, str, bool, Logger|logging, bool) -> None
        """
        ...
        :param client_secret:   An optional (When using MSI) client secret - Of a user or an application that is
                                authorized with your KeyVault resources
        ...
        :param verbose:         If set, will log which authentication mechanism the KV client will be using (MSI/ADAL)
        :param args:            Additional arguments to pass to KeyVault client
        :param kwargs:          Additional named arguments to pass to KeyVault client
        """
        ...
        if verbose:
            self._logger.info('Will be using {} for KeyVault authentication'.format('MSI' if use_msi else 'ADAL'))
