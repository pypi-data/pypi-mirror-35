...
class Attention(keras.layers.Layer):
    ...
    def _attention_regularizer(self, attention):
        ...
        if K.backend() == Attention.BACKEND_TYPE_TENSORFLOW:
            import tensorflow as tf
            return self.attention_regularizer_weight * K.square(K.batch_dot(
                attention,
                K.permute_dimensions(attention, (0, 2, 1))) - tf.eye(input_len))
        if K.backend() == Attention.BACKEND_TYPE_THEANO:
            ...
            return self.attention_regularizer_weight * K.square(K.batch_dot(
                attention,
                K.permute_dimensions(attention, (0, 2, 1))) - K.expand_dims(eye, 0))
