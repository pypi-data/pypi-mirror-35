...
class AbstractArray(np.ndarray):
    ...
    def from_fits(cls, file_path, hdu, pixel_scale):
        ...
        hdu : int
            The HDU number in the fits file containing the basic weighted_data.
        pixel_scale: float
    ...
    def trim(self, new_dimensions):
        ...
        if self.shape[0] != new_dimensions[0]:
            logger.debug(
                'basic.weighted_data.trim_data - Your specified x_size was odd (even) when the basic x dimension is '
                'even (odd)')
            logger.debug(
                'The method has automatically used x_size+1 to ensure the basic is not miscentred by a half-pixel.')
        elif self.shape[1] != new_dimensions[1]:
            logger.debug(
                'basic.weighted_data.trim_data - Your specified y_size was odd (even) when the basic y dimension is '
                'even (odd)')
            logger.debug(
                'The method has automatically used y_size+1 to ensure the basic is not miscentred by a half-pixel.')
...
class ScaledArray(AbstractArray):
    """
    Class storing the grids for 2D pixel grids (e.g. basic, PSF, signal_to_noise_ratio).
    """
    ...
    def central_pixel_coordinates(self):
        ...
        central_pixel_coordinates:
            The coordinates of the central pixel in the basic. If a dimension of the basic are odd then the
    ...
    def pixel_coordinates_to_arc_second_coordinates(self, pixel_coordinates):
        ...
        left corner of the basic whilst the arc second coordinate origin is at the centre. This means that the original
    ...
    def shape_arc_seconds(self):
        """
        The shape of the basic in arc seconds
        """
    ...
    def sub_pixel_to_coordinate(self, sub_pixel, arcsec, sub_grid_size):
        """Convert a coordinate on the regular basic-pixel grid_coords to a sub-coordinate, using the pixel scale and
