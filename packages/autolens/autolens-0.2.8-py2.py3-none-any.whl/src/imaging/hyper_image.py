...
class HyperImage(object):
    ...
    def __init__(self, background_sky_scale=0.0, background_noise_scale=0.0):
        """Class for scaling the noise of different components in an basic, primarily the different galaxies (e.g. the \
        lens, source).
    ...
    def sky_scaled_image_from_image(self, image):
        """Compute a new basic with the background sky level scaled. This can simply multiple by a constant factor \
        (assuming a uniform background sky) because the basic is in units electrons per second.

        Parameters
        -----------
        image : ndarray
            The basic before scaling (electrons per second).
        """
