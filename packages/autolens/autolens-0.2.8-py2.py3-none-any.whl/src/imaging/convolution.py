...
class Convolver(object):
    """Class to setup the 1D convolution of an basic / mapping matrix.

    IMAGE FRAMES:
    ------------

    For a masked basic, in 2D, one can compute for every unmasked pixel all other unmasked pixels it will blur light \
    ...
    image_frame_indexes - The indexes of all basic pixels it will blur light into.
    image_frame_psfs - The psf values that overlap each basic pixel it will blur light into.
    image_frame_length - The number of basic-pixels it will blur light into (because unmasked pixels are excluded)
    ...
    Once we have set up all these quantities, the convolution routine simply uses them to convolve a 1D array of basic
    data / a mapping matrix basic.
    ...
    blurring_frame_indexes - The indexes of all unmasked basic pixels it will blur light into.
    bluring_frame_kernels - The psf values that overlap each unmasked basic pixel it will blur light into.
    blurring_frame_length - The number of basic-pixels it will blur light into (because unmasked pixels are excluded)
    ...
    For pixel 0 above, when we overlap the 3x3 psf above only 1 unmasked basic pixels overlap the psf, such that:

    blurring_frame_indexes = [0]
    blurring_frame_psfs = [0.9]
    blurring_frame_length = 1

    For pixel 1 above, when we overlap the 3x3 psf above 2 unmasked basic pixels overlap the psf, such that:

    blurring_frame_indexes = [0, 1]
    blurring_frame_psfs = [0.8, 0.9]
    blurring_frame_length = 2

    For pixel 3 above, when we overlap the 3x3 psf above 3 unmasked basic pixels overlap the psf, such that:
    ...
    def __init__(self, mask, psf):
        """
        Class to create basic frames and blurring frames used to convolve a psf with a 1D basic of non-masked \
        values.
