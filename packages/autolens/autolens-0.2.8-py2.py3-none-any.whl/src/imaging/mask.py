...
class Mask(scaled_array.ScaledArray):
    ...
    def unmasked(cls, shape_arc_seconds, pixel_scale):
        """
        Setup the mask such that all values are unmasked, thus corresponding to the entire basic.
    ...
    def coordinate_grid(self):
        """
        Compute the basic grid_coords grids from a mask, using the center of every unmasked pixel.
        """
    ...
    def map_to_1d(self, grid_data):
        """Compute a data grid, which represents the data values of a data-set (e.g. an basic, noise, in the mask.
    ...
    def border_pixel_indices(self):
        """Compute the border basic data_to_pixels from a mask, where a border pixel is a pixel inside the mask but on
    ...
    def blurring_mask_for_kernel_shape(self, kernel_shape):
        ...
        fraction of their light will be blurring in the basic.
                    ...
                    for x1 in range((-kernel_shape[0] + 1) // 2, (kernel_shape[0] + 1) // 2):
                        if 0 <= x + x1 <= self.shape[0] - 1 and 0 <= y + y1 <= self.shape[1] - 1:
                            ...
                            raise exc.MaskException(
                                "setup_blurring_mask extends beyond the sub_grid_size of the mask - pad the basic"
                                "before masking")
    ...
    def map_to_2d(self, data):
        """Use mapper to map an input data-set from a *GridData* to its original 2D basic.
        Parameters
        -----------
        data : ndarray
            The grid-data which is mapped to its 2D basic.
        """
...
class SparseMask(Mask):
    ...
    def index_image(self):
        """
        Setup an basic which, for each *False* entry in the sparse mask, puts the sparse pixel index in that pixel.

         This is used for computing the image_to_cluster vector, whereby each basic pixel is paired to the sparse
         pixel in this basic via a neighbor search."""
    ...
    def sparse_to_image(self):
        """
        Compute the mapping of each sparse basic pixel to its closest basic pixel, defined using a mask of basic \
        data_to_pixels.

        Returns
        -------
        cluster_to_image : ndarray
            The mapping between every sparse clustering basic pixel and basic pixel, where each entry gives the 1D index
            of the basic pixel in the self.
        """
    ...
    def image_to_sparse(self):
        """Compute the mapping between every basic pixel in the mask and its closest sparse clustering pixel.

        This is performed by going to each basic pixel in the *mask*, and pairing it with its nearest neighboring pixel
        ...
        image_to_cluster : ndarray
            The mapping between every basic pixel and its closest sparse clustering pixel, where each entry give the 1D
...
class ImageGrid(np.ndarray):
    ...
    basic (e.g. [0,0]) have a negative x-value and positive y-value in arc seconds. The basic pixel indexes are \
    ...
    to the basic pixel index, and second element to its (x,y) arc second coordinates. For example, the value \
    [3,1] gives the 4th basic pixel's y coordinate.

    Below is a visual illustration of a regular grid, where a total of 10 data_to_image are unmasked and therefore \
    included in the grid.

    |x|x|x|x|x|x|x|x|x|x|
    |x|x|x|x|x|x|x|x|x|x|     This is an example basic.Mask, where:
    |x|x|x|x|x|x|x|x|x|x|
    ...
    This basic pixel index's will come out like this (and the direction of arc-second coordinates is highlighted
    around the basic.
...
class SubGrid(ImageGrid):
    ...
    element maps to the basic pixel index, the second element to the sub-pixel index and third element to that
    sub pixel's (x,y) arc second coordinates. For example, the value [3, 6, 1] gives the 4th basic pixel's
    ...
    However, we now go to each basic-pixel and derive a sub-pixel grid for it. For example, for pixel 0,
    ...
    def sub_to_image(self):
        """ Compute the pairing of every sub-pixel to its original basic pixel from a mask. """
...
class GridCollection(object):
    ...
    def __init__(self, image, sub, blurring):
        """
        A collection of grids which contain the coordinates of an basic. This includes the basic's regular grid,
        sub-grid, blurring region, etc. Coordinate grids are passed through the ray-tracing module to set up the basic,
        lens and source planes.

        Parameters
        -----------
        image : GridCoordsImage
            A grid of coordinates for the regular basic grid.
        sub : GridCoordsImageSub
            A grid of coordinates for the sub-gridded basic grid.
