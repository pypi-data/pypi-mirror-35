...
class InterpolationScheme(object):
    ...
    def __init__(self, shape, image_coords, image_pixel_scale):
        ...
        the basic-grid and sub-grid deflection angle values.
        ...
        The interpolation scheme can be performed on basic and sub grids that are non-uniform (e.g. after tracing them \
        from the basic-plane to a source-plane). The x_pixel and y_pixel bounds of the new grids are used to scale the \
        ...
        image_coords : ndarray
            The basic-plane coordinates of each pixel on the interpolation-grid.
        """
    ...
    def from_mask(cls, mask, shape):
        """Determine the interpolation scheme from an basic-mask. This uses the x / y_pixel bounds of the mask to setup the \
