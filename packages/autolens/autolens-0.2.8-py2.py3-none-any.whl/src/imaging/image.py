...
# TODO : The idea is that we don't need functions to estimate the noise or the exposure time once we set up an basic
...
class PrepatoryImage(ScaledArray):
    ...
    def __init__(self, array, pixel_scale, psf, noise=None, background_noise=None, poisson_noise=None,
        ...
        """
        A 2d array representing a real or simulated basic.

        Parameters
        ----------
        array: ndarray
            An array of basic pixels in gray-scale
        noise: ndarray
            An array describing the noise in the basic
        effective_exposure_time: Union(ndarray, float)
            A float or array representing the effective exposure time of the whole basic or each pixel.
        pixel_scale: float
    ...
    def simulate(cls, array, pixel_scale, effective_exposure_time, psf=None, background_sky_map=None,
        ...
        """
        Create a realistic simulated basic by applying effects to a plain simulated basic.
        ...
        include_poisson_noise: Bool
            If True poisson noise is simulated and added to the basic
        seed: int
            A seed for random noise generation

        Returns
        -------
        basic: PrepatoryImage
            A simulated basic
        """
        ...
        # The final basic is background subtracted.
        if background_sky_map is not None:
            array -= background_sky_map
    ...
    def electrons_per_second_to_counts(self, array):
        ...
        ----------
        array : ndarray
            The basic from which the Poisson signal_to_noise_ratio map is estimated.
        """
    ...
    def counts_array(self):
        ...
        counts_array: ndarray
            An array representing the basic in terms of counts
        """
    ...
    def background_noise_from_edges(self, no_edges):
        """Estimate the background signal_to_noise_ratio by binning data_to_image located at the edge(s) of an basic
...
class PSF(Array):
    ...
    def convolve(self, array):
        ...
        ----------
        array: ndarray
            An array representing an basic

        Returns
        -------
        convolved_array: ndarray
            An array representing an basic that has been convolved with this PSF
...
def generate_poisson_noise(image, exposure_time, seed=-1):
    """
    Generate a two-dimensional background noise-map for an basic, generating values from a Gaussian
    distribution with mean 0.0.

    Parameters
    ----------
    image : ndarray
        The 2D basic background noise is added to.
