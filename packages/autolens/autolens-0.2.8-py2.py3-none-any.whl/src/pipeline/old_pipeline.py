...
def source_only_pipeline(image, mask):
    """
    Fit a source-only basic (i.e. no lens light component)

    Parameters
    ----------
    image: Image
        An basic of a lens galaxy including metadata such as PSF, background noise and effective exposure time
    mask: Mask
        A mask describing which parts of the basic should be excluded from the analysis
    ...
    logger.info(
        ...
        PURPOSE - Fit a source-only basic (i.e. no lens light component)
        ...
        Image: Observed basic used throughout.
    ...
    # Create an analysis object for the basic and mask
...
def lens_and_source_pipeline(image, lens_mask, source_mask, combined_mask):
    ...
    PURPOSE - Fit a lens light + source basic (mass model does not decomposed the light and dark matter)
    ...
    - Draw annulus for mask containing the source (excludes central regions of basic where lens is).
    """
    ...
    1) Image: Observed basic, includes lens+source.
       Mask: Circle / Ellipse containing entire lens galaxy.
       Light: Sersic (This phase simply subtracts the lens light from the basic - the source is present and disrupts the 
       NLO: LM        fit but we choose not to care)
       Purpose: Provides lens subtracted basic for subsequent phases.

    """
    # Create an analysis object for the basic and a mask that means only lens light is visible
    ...
    """
    2) Image: The lens light subtracted basic from phase 1.
    ...
    """
    3) Image: The lens light subtracted basic from phase 1.
       Mask: Circle / Ellipse containing entire lens galaxy.
       Light: None
    ...
    """
    4) Image: Observed basic, includes lens+source.
       Mask: Circle / Ellipse containing entire lens galaxy.
...
def profiles_pipeline(paths, image, mask):
    """
     Fit an basic with profiles, gradually adding extra profiles.
    ...
    logger.info(
        ...
        PURPOSE - Fit profiles to an basic.
...
# def profiles_pipeline(paths, basic, mask):
#     """
#      Fit an basic with profiles, gradually adding extra profiles.
#
#      Parameters
...
#
#     # Create an analysis object for the basic and mask
#     analysis = an.Analysis(basic, mask)
#
#     logger.info(
#         """
#         1) Light: Elliptical Sersic x1
...
4H) Hyper-parameters: All included in model (most priors broad and uniform, but use previous phase regularization as well)
    Image: The lens light subtracted basic from phase 1.
    Mask: Circle / Ellipse containing entire lens galaxy.
    Mass: SIE+Shear (Fixed to highest likelihood model from phase 2)
    Source: 'noisy' pixelization
    NLO: MN.
