...
class Pixelization(object):
    ...
    def __init__(self, pixels=100, regularization_coefficients=(1.0,)):
        """
        Abstract base class for a pixelization, which discretizes a set of basic and sub grid grid into \
        pixels. These pixels fit an basic using a linear inversion, where a regularization matrix
        enforces smoothness between pixel values.

        A number of 1D and 2D arrays are used to represent mappings betwen basic, sub, pix, and cluster pixels. The \
        nomenclature here follows grid_to_grid, such that it maps the index of a value on one grid to another. For \
        example:

        - pix_to_image[2] = 5 tells us that the 3rd pixelization-pixel maps to the 6th basic-pixel.
...
class Rectangular(Pixelization):
    ...
    def __init__(self, shape=(3,3), regularization_coefficients=(1.0,)):
        ...
        Like an basic grid, the indexing of the rectangular grid begins in the top-left corner and goes right and down.
    ...
    def grid_to_pix_from_grid_jit(self, grid, geometry):
        """Compute the mappings between a set of basic pixels (or sub-pixels) and pixels, using the basic's
    ...
    def reconstructor_from_pix_grids(self, grids, borders, cluster_mask=None):
        ...
        grid.
        2) Pair basic and sub-basic pixels to the rectangular grid using their traced grid and its geometry.
...
class Voronoi(Pixelization):
    ...
    def __init__(self, pixels=100, regularization_coefficients=(1.0,)):
        ...
        all of the nearest-neighbor pix-grid (i.e. traced basic-pixels) are mapped to them.
    ...
    def image_to_pix_from_pixelization(self, grids, pix_centers, pix_neighbors, cluster_to_pix, cluster_mask):
        """ Compute the mappings between a set of basic pixels and pixels, using the basic's traced \
        ...
        These provide the mappings between only a sub-set of sub-pixels / basic-pixels and pixels.
        ...
        pixel that we have paired to a sub_coordinate using the cluster_coordinate of an basic coordinate. \
        ...
        sub_to_pix : [int, int]
            ...
            basic-pixel maps to the 3rd pixel, sub_to_pix[2,4] = 2).
    ...
    def sub_to_pix_from_pixelization(self, grids, pix_centers, pix_neighbors, cluster_to_pix, cluster_mask):
        """ Compute the mappings between a set of sub-basic pixels and pixels, using the basic's traced \
        ...
        grids: mask.GridCollection
            A collection of coordinates for the masked basic, subgrid and blurring grid
        cluster_mask: mask.SparseMask
            A mask describing the basic pixels that should be used in pixel clustering
        pix_centers: [[float, float]]
            The coordinate of the center of every pixel.
        pix_neighbors : [[]]
            The neighboring pix_pixels of each pix_pixel, computed via the Voronoi grid_coords. \
...
class Cluster(Voronoi):
    ...
    def __init__(self, pixels, regularization_coefficients=(1.0,)):
        ...
        pix-grid (i.e. traced basic-pixels) are mapped to them.

        For this pixelization, a set of cluster-pixels (defined in the basic-plane as a cluster uniform grid of \
        basic-pixels) determine the pixel centers .
    ...
    def reconstructor_from_pix_grids(self, grids, borders, cluster_mask):
        ...
        4) Compute the mapping between all basic sub-grid and pixels.
...
class Amorphous(Voronoi):
    ...
    def __init__(self, pixels, regularization_coefficients=(1.0, 1.0, 2.0)):
        ...
        nearest-neighbor pix-grid (i.e. traced basic-pixels) are mapped to them.

        For this pixelization, a set of cluster-pixels (defined in the basic-plane as a cluster uniform grid of \
        basic-pixels) are used to determine a set of pix-plane grid. These grid are then fed into a \
