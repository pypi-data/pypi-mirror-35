...
# TODO : Should galaxy basic and minimum value be in the constructor (they aren't free parameters)?

class HyperGalaxy(object):
    _ids = count()

    def __init__(self, contribution_factor=0.0, noise_factor=0.0, noise_power=1.0):
        """Class for scaling the noise in the different galaxies of an basic (e.g. the lens, source).
    ...
    def contributions_from_model_images(self, model_image, galaxy_image, minimum_value):
        ...
        -----------
        model_image : ndarray
            The model basic of the observed weighted_data (from a previous analysis phase). This tells us the total light \
            attributed to each basic pixel by the model.
        galaxy_image : ndarray
            A model basic of the galaxy (e.g the lens light profile or source reconstruction) computed from a
            previous analysis.
        minimum_value : float
