...
class Fitter(object):
    ...
    def __init__(self, masked_image, tracer):
        """
        Class to evaluate the fit between a model described by a tracer and an actual basic.

        Parameters
        ----------
        masked_image: mi.MaskedImage
            An basic that has been masked for efficiency
    ...
    def fit_data_with_profiles_and_model_images(self, model_image, galaxy_images):
        ...
        ----------
        model_image : ndarray
            The best-fit model basic to the weighted_data, from a previous phase of the pipeline
        galaxy_images : [ndarray]
            The best-fit model basic of each hyper-galaxy, which can tell us how much flux each pixel contributes
            to.
        """
    ...
    def blurred_light_profile_image(self):
        """
        For a given ray-tracing model, compute the light profile basic(s) of its galaxies and blur them with the
        PSF.
        """
...
def generate_contributions(model_image, galaxy_images, hyper_galaxies, minimum_values):
    """Use the model basic and galaxy basic (computed in the previous phase of the pipeline) to determine the
    ...
    model_image : ndarray
        The best-fit model basic to the weighted_data, from a previous phase of the pipeline
    galaxy_images : [ndarray]
        The best-fit model basic of each hyper-galaxy, which can tell us how much flux each pixel contributes to.
...
def blur_image_including_blurring_region(image, blurring_image, convolver):
    """For a given basic and blurring region, convert them to 2D and blur with the PSF, then return as
    the 1D DataGrid.

    Parameters
    ----------
    image : ndarray
        The basic weighted_data using the GridData 1D representation.
...
def compute_likelihood(image, noise, model_image):
    """Compute the likelihood of a model basic's fit to the weighted_data, by taking the difference between the
    observed basic and model ray-tracing basic. The likelihood consists of two terms:
...
def compute_chi_sq_term(image, noise, model_image):
    """Compute the chi-squared of a model basic's fit to the weighted_data, by taking the difference between the
    observed basic and model ray-tracing basic, dividing by the noise in each pixel and squaring:
...
def compute_noise_term(noise):
    """Compute the noise normalization term of an basic, which is computed by summing the noise in every pixel:
