...
data = scaled_array.ScaledArray.from_fits_with_scale(file_path=data_dir + '/basic', hdu=0, pixel_scale=0.1)
noise = scaled_array.ScaledArray.from_fits_with_scale(file_path=data_dir + '/noise', hdu=0, pixel_scale=0.1)
exposure_time = scaled_array.ScaledArray.from_fits_with_scale(file_path=data_dir + '/exposure_time', hdu=0,
                                                              pixel_scale=0.1)
...
def test_analysis_1():
    ...
    source_galaxy = g.Galaxy(light_profile=light_profiles.EllipticalSersicLightProfile())
