...
def find_references(file_path):
    """ Search through the markdown file at the given 'file_path' for references to assets and links.
    """
    references = []
    ...
    # Return filepaths relitive to the root of the site..
    return [os.path.join(pathlib.Path(file_path).parent, a) for a in list(set(references))]
...
def find_orphans(path):
    ...
    # If it's a markdown file append it to a pages list
    for i in files:
        if i.endswith('.md'):
            pages[i] = find_references(i)
        ...
        if i.endswith('.md'):
            with open(i) as f:
                if has_nav(f.read().lower()):
                    nav_markdown.append(i)

    files = [x for x in files if x not in nav_markdown]
    ...
    unused_assets = [i for i in files if i not in all_references]
    print(f'{len(unused_assets)} Unused assets (orphans):\n\t' + '\n\t'.join(unused_assets))
...
def main():
    ...
    parser.add_argument('--find-orphans', action="store", dest='find_orphans_path',
                        default=False,
    ...
    if args.find_orphans_path:
        find_orphans(args.find_orphans_path)
        return 0
    ...
    # Flip the flask debug to true if the watch is enabled.
    if flask_watch:
        flask_debug = TRUE
    ...
    extra_files = build_reload_files_list([__name__, dir_documents, dir_style]) if flask_watch else []
