...
def generate_static_html(root_dir, output_dir='build'):
...
def check_pdf_generation_cap():
    ...
    try:
        subprocess.check_output(f'{WKHTMLTOPDF_BINARY} --version', stdout=DEVNULL, shell=True)
        return True
    except Exception:
        return False
    finally:
...
def copy_local_project():
    ...
    pairs = [(os.path.join(source_root, path), os.path.join(target_root, path)) for path in targets]
...
def main():
    ...
    TRUE = 'TRUE'
    FALSE = 'FALSE'
    flask_debug = os.environ.get('DS_FLASK_DEBUG', TRUE) == TRUE
    ...
    if not os.path.exists(dir_documents) and not os.path.isdir(dir_documents):
        print(f'Error: Documents directory "{dir_documents}" does not exist.  Create one called `docs` and fill it with your documentation.', file=sys.stderr)
        exit()
    ...
    if args.html_output_dir:
        PDF_GENERATION_ENABLED = False
        try:
            generate_static_html(dir_documents, args.html_output_dir)
