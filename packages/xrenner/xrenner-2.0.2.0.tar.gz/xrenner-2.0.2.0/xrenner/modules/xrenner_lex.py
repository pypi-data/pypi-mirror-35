...
class LexData:
	...
	def __init__(self, model, xrenner, override=None, rule_based=False):
		...
		# Add all isa entries to similar too
		#keys = set(self.similar).union(self.isa)
		#no = []
		#self.similar = dict((k, self.similar.get(k, no) + self.isa.get(k, no)) for k in keys)
	...
	def read_delim(self, filename, mode="normal", atom_list_name="atoms", add_to_sums=False, sep=","):
		...
		with filename as csvfile:
			...
			if mode == "low":
				...
				#if "א טיבה" in out_dict:
				#	print out_dict["א טיבה"]
				return out_dict
