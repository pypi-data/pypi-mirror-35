...
class DWhoInoEventPlugBase(DWhoInoPlugBase, DWhoInotifyEventBase):
    ...
    def _get_path_options(self):
        if not self.cfg_path \
           ...
           or self.PLUGIN_NAME not in self.inopaths[self.cfg_path.path] \
           or not isinstance(self.inopaths[self.cfg_path.path][self.PLUGIN_NAME], dict):
            return

        return self.inopaths[self.cfg_path.path][self.PLUGIN_NAME]
