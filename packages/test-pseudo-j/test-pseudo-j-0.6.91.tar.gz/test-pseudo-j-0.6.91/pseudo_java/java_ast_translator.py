...
class JavaASTTranslator:
			...
			if (node.qualifier,self.name_scope[node.qualifier][-1]) in self.ass_store:
				...
				if BUILTIN_ARG_FUNCTIONS[node.member] == 'same':
					if pt != temp['pseudo_type']:
						raise errors.type_check_error('The type of the arguments is not fit for this particular function and its receiver: Arg : %s, Receiver : %s' % (temp['pseudo_type'],pt),node.position,self.lines[node.position[0]])
				...
				if pt != BUILTIN_ARG_FUNCTIONS[node.member]:
					raise errors.type_check_error('The type of the arguments is not fit for this particular function and its receiver: Arg : %s, Receiver : %s' % (pt,BUILTIN_ARG_FUNCTIONS[node.member]),node.position,self.lines[node.position[0]])	
	...
	def translate_BIOP(self,node,parent):
		...
		if(str(node.operandl) == 'MemberReference'):
			...
			raise errors.type_check_error('Operand left not yet treated by Pseudo-Java',node.position,self.lines[node.position[0]])
		...
		if(str(node.operandr) == "Literal"):
			...
			raise errors.type_check_error('Operand right not yet treated by Pseudo-Java',node.position,self.lines[node.position[0]])
			...
			if (str(node.type) == 'ReferenceType'):
				if(node.type.dimensions == []):
					...
					if(str(d.initializer) == 'ArrayInitializer'):
						...
						raise errors.type_check_error('Type %s not covered by Pseudo-Java' % str(d.initializer), node.position,self.lines[node.position[0]])
			elif(str(node.type) == 'BasicType'):
				if(node.type.dimensions == []):
					if(str(d.initializer) == 'Literal'):
						...
						if(node.type.name == 'int' or node.type.name == 'float'):
							...
							raise errors.type_check_error('Type not covered by Pseudo-Java: %s' %node.type.name,node.position,self.lines[node.position[0]])
							print('Type not supported '+node.type.name)
							return None
						if d.name in self.name_scope:
							self.name_scope[d.name].append(self.actual_scope)				
