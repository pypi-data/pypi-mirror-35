    ...
    def delete(self
        ...
        for name, path in self._yield_folder_info():
            for movie in path.iterdir():
                ...
                if is_difference:
                    self.log.info(f"{movie.name} is remove after {difference :.2f} {self.threshold_type} of existance", extra={"folder" : name})
                    shutil.rmtree(movie.resolve())
                    n_remove += 1
                    list_remove.append(movie.name)
        if not n_remove:
            self.log.info("Nothing remove", extra={"folder" : name})
        else:
              self.log.info(f"{n_remove} files remove", extra={"folder" : name})
